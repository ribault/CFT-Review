...
\begin{defn}[Liouville theory]
 ...
 \begin{align}
  S^\mathrm{Liouville} 
= \int_{\frac{Q}{2}+i\mathbb{R}_+}  dP\ \mathcal V_P \otimes 
   \bar{\mathcal V}_P\ , 
   \label{eq:sl}
 \end{align}
...
\begin{align}
 \Delta \in \mathbb{R} \iff P\in \mathbb{R} \cup \left(\frac{Q}{2} + i\mathbb{R}\right)\ ,
  \qquad
   \begin{tikzpicture}[scale = .4, baseline=(current  bounding  box.center)]
  \draw[-latex, very thin] (-2, 0) -- (0, 0) node[below right] {$0$} -- (2, 0) node[below right] {$\frac{Q}{2}$} -- (4, 0)  -- (6, 0) node[above] {$P$};
...
\begin{align}
 \Delta_\text{min}=\Delta\left(P=\frac{Q}{2}\right) = \frac{Q^2}{4}=\frac{c-1}{24}\ .
\end{align}
This leads to $P \in \frac{Q}{2}+i\mathbb{R}$. Assuming that each allowed representation appears only once in the spectrum, we actually restrict the momentums to $P \in \frac{Q}{2}+i\mathbb{R}_+$, due to the reflection symmetry \eqref{eq:refm}.
...
\begin{align}
 ...
 \label{eq:vvv}
 \\
 V_{P_1}V_{P_2} &= \int_{\frac{Q}{2}+i\mathbb{R}_+} dP\, \frac{C_{P_1,P_2,P}}{B(P)} \Big( V_P + \cdots\Big)\ ,
 \label{eq:v1v2}
\end{align}
...
$P \in \frac{Q}{2}+i\mathbb{R} \centernot\implies (P\pm\frac{b}{2}) \in \frac{Q}{2}+i\mathbb{R}$.
We resort to the assumption in Definition \ref{def:liou} that correlation functions are smooth functions of $P$, and take $V_P$ to actually be defined for $P\in\mathbb{C}$ by analytic continuation. (Or for $P$ in a dense open subset of $\mathbb{C}$, if there are singularities.) This allows us to write the OPE
\begin{align}
...
Let us determine the coefficients $c^{(s)}_{\epsilon}$ in terms of OPE coefficients and three-point structure constants. Using the degenerate OPE \eqref{degope} and the three-point function \eqref{eq:vvv}, we find
\begin{align}
...
\label{cs}
\end{align}
Then eq. \eqref{eq:coc} becomes a shift equation for the dependence of $C_{P_1,P_2,P_3}$ on $P_1$,
\begin{align}
 \frac{C_+(P_1) C_{P_1+\frac{b}{2},P_2,P_3}}{C_-(P_1) C_{P_1-\frac{b}{2},P_2,P_3} } 
 = \frac{\gamma(b(2P_1-Q)) \gamma(1+b(2P_1-Q))}{\prod_{\pm,\pm} \gamma(\frac12 +b(P_1-\frac{Q}{2}) \pm b(P_2-\frac{Q}{2}) \pm b(P_3-\frac{Q}{2}))}\ .
 \label{eq:shift}
\end{align}
In order to find the three-point structure constant $C_{P_1,P_2,P_3}$, we need to constrain the degenerate OPE coefficients $C_\pm(P)$. To do this, we consider the special case where the last field is degenerate too, i.e. the four-point function $\Big\langle V_{\langle 2,1 \rangle}(z) V_P(0) V_{P}(\infty) V_{\langle 2,1 \rangle}(1)\Big\rangle$.
...
\begin{align}
 ...
 =  \frac{\gamma(b(2P-Q))}{\gamma(-b(2P-Q))}
 \frac{\gamma(-b^2-b(2P-Q))}{\gamma(-b^2+b(2P-Q))} \ .
 \label{eq:shiftd}
\end{align}
...
\subsection{Determining three-point structure constants}\label{sec:sol}
...
\begin{align}
 C_{P_1,P_2,P_3} =  \frac{N_0 \prod_{i=1}^3 N(P_i)}{\Upsilon_b(P_1+P_2+P_3-Q) \Upsilon_b(P_1+P_2-P_3)\Upsilon_b(P_2+P_3-P_1)\Upsilon_b(P_3+P_1-P_2)} \ ,
\end{align}
...
\begin{align}
 \frac{C_-(P_1)N(P_1-\frac{b}{2})}{C_+(P_1)N(P_1+\frac{b}{2})} = \frac{b^{4b(2P_1-Q)}}{ \gamma(b(2P_1-Q)) \gamma(1+b(2P_1-Q))}\ .
\end{align}
...
\begin{align}
 \frac{\left(N^2B^{-1}\right)(P-\frac{b}{2})}{\left(N^2B^{-1}\right)(P+\frac{b}{2})} = b^{8b(2P-Q)} \frac{\gamma(-b(2P-Q))}{\gamma(b(2P-Q))} \frac{\gamma(-b^2-b(2P-Q))}{\gamma(-b^2+b(2P-Q))}\ .
\end{align}
Together with its image under $b\to b^{-1}$, this equation has the solution
\begin{align}
 \left(N^2B^{-1}\right)(P) = \Upsilon_b(2P)\Upsilon_b(2P-Q)\ .
 \label{eq:ntbm}
\end{align}
Therefore, we have only determined the combination $N^2B^{-1}$, and not the individual functions $B$ and $N$ that appear in the 
two- and three-point functions. This is because we still have the freedom of performing changes of field normalization $V_P(z) \to \lambda(P)V_P(z)$. Under such changes, we have $B\to \lambda^2B$ and $N\to \lambda N$, while
...
the normalization such that $B(P)=1$, which would cause $N(P)$ (and therefore $C_{P_1,P_2,P_3}$) to have square-root branch cuts.
We could rather adopt the field normalization such that $N(P)=\Upsilon_b(2P)$, which makes the three-point structure constant meromorphic as a function of the momentums.
We also choose $N_0=1$, and  
% Field normalization such that C_+ = 1, and B(P)B(Q-P)=1.
we obtain the DOZZ formula (for Dorn, Otto, A.
Zamolodchikov and Al. Zamolodchikov),
\begin{align}
 C_{P_1,P_2,P_3} =  \frac{\Upsilon_b(2P_1) \Upsilon_b(2P_2) \Upsilon_b(2P_3)}{\Upsilon_b(P_1+P_2+P_3-Q) \Upsilon_b(P_1+P_2-P_3)\Upsilon_b(P_2+P_3-P_1)\Upsilon_b(P_3+P_1-P_2)} \ .
\label{caaa}
\end{align}
...
Our shift equations for three-point structure constants, and their solutions, are valid not only for Liouville theory, but also for (generalized) minimal models. In such models, the momentums $P_{\langle r,s\rangle}$ belong to a lattice with periods $b$ and $\frac{1}{b}$, so they are uniquely determined by the shift equations. The solution is given by $C$ or $\hat C$, which coincide. (Actually $C$ has poles when $P_i$ take degenerate values, one should take the residues.) This shows that (generalized) minimal models are unique, but not yet that they exist.
...
\node at (-2.5, -4) {$\Big< V_{P_1}(z) V_{P_2}(0) V_{P_3}(\infty) V_{P_4}(1)\Big> = {\displaystyle\int_{\frac{Q}{2}+i\mathbb{R}_+}} dP\ \colorboxed{red}{\frac{C_{P_1,P_2,P}}{B(P)}}\, \colorboxed{red}{ C_{P,P_3, P_4}}\, \mathcal{F}_{P}^{(s)}(z) \mathcal{F}_{P}^{(s)}(\bar z)$};
...
In order to investigate the analytic properties of the integral, let us first extend the integration half-line to a line, $\int_{\frac{Q}{2}+i\mathbb{R}_+} \to \frac12 \int_{\frac{Q}{2}+i\mathbb{R}}$. This is possible because the integrand is invariant under $P \to Q-P$: the conformal blocks are invariant because they are functions of the conformal dimension \eqref{eq:refm}, and the combination $B(P)^{-1}C_{P_1,P_2,P}C_{P,P_3, P_4}$ of structure constants is invariant as a consequence of $\Upsilon_b(P)=\Upsilon_b(Q-P)$. 
...
\begin{tikzpicture}[scale = .4, baseline=(current  bounding  box.center)]
 ...
 \draw[ultra thick, blue, opacity = .3] (2,0) -- (6,0);
  \draw[-latex] (-4,0) -- (0, 0) node[above left] {$0$} -- (6,0) node [above] {$P$};
  \draw (0, -4.5) -- (0, 4.5);
...
\node[above right] at (2,0) {$Q$};
 ...
 &
 \begin{tikzpicture}[scale = .4, baseline=(current  bounding  box.center)]
  \draw[-latex] (-3,0) -- (0, 0) node[above left] {$0$} -- (4,0) node [above] {$P$};
