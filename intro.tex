...
\end{defn}
Until Section \ref{sec:dmm}, we will assume that all primary fields are diagonal, and write  $V_\Delta(z) = V_{\Delta,\Delta}(z)$.
...
\begin{exo}[Closure of minimal model spectrums under fusion]
 ~\label{exo:cmm}
 Show that $S_{p,q}$ is closed under fusion. If you are brave, show that the fusion products of the representations that appear in $S_{p,q}$ are
 \begin{align}
...
\label{rrmm}
\end{align}
 If you are very brave, show that $p,q\geq 2$ is a necessary condition for the existence of a finite, nontrivial set of multiply degenerate representations that closes under fusion.
\end{exo}
...
\subsection{Questions to be raised}

\begin{enumerate}
 \item It would be more logical for me to do affine symmetrie at the end. But this might come too late for the course on WZW models. Should two lectures be swapped? 
 \item Does anyone need the Verlinde formula?
 \item Who needs characters and torus partition functions? What for?
\end{enumerate}

\subsection{To be done}

\begin{enumerate}
 \item Discuss correlation functions of minimal models?
\end{enumerate}
...
Modular invariance neither necessary nor sufficient. Main advantage: technical simplicity, since characters are simpler than four-point blocks.

We first have to justify the definition of the torus partition function, by explaining why it is modular invariant. 

This comes from the geometry: the modular transformation from the plane to the cylinder $z=e^{\tau + i\sigma}$ shifts $L_0+\bar{L}_0$ by $-\frac{c}{12}$. (This comes from the $TT$ OPE.) Then we build a torus from a finite size cylinder of length $\tau-\bar{\tau}$, while doing a rotation by $\tau + \bar\tau$ before gluing. This explains why the torus partition function is 
\begin{align}
 Z = \operatorname{Tr} e^{2\pi i(\tau-\bar\tau)(L_0+\bar L_0-\frac{c}{12})} e^{2\pi i(\tau+\bar \tau)(L_0-\bar L_0)} = \operatorname{Tr} y^{L_0-\frac{c}{24}} \bar y^{\bar L_0-\frac{c}{24}}
\end{align}
where $y = e^{2\pi i\tau}$. We can then rewrite the torus as $\frac{\mathbb{C}}{\mathbb{Z} + \tau\mathbb{Z}}$ and justify modular invariance. 

Start with Liouville theory: the partition function does not involve $c$ but $c_\text{eff}=1$. 

Modular invariance of diagonal partition function always true if $S=S^T$. (We have $S^2=1$ by construction.)

Do not talk about the Verlinde formula? Or just state it? It has a general proof in \cite{fms97} using manipulations of torus two-point functions. But what are exactly the assumptions? Naively using the formula in Liouville theory gives fusion rules that enforce momentum conservation!

$T$-modular transformation requires $\Delta\equiv\bar\Delta \bmod \mathbb{Z}$. So there is room for non-diagonal invariants due to $\Delta_{\langle q-r,s\rangle} -\Delta_{\langle r,s\rangle}= \left(r-\frac{q}{2}\right)\left(s-\frac{p}{2}\right)$.
But guessing the spectrum is better done with fusion rules? How do we guess conservation of diagonality? Parity of $r$ is given in $N$ sector, so $q\equiv 0\bmod 4 \implies N\times N = D$.

\subsection{Modular transformation of degenerate characters}

The character of a Verma module is 
\begin{align}
 \chi_p (y) = \frac{y^{p^2}}{\eta(y)}
\end{align}
(Explain!)
The modular transformation is 
\begin{align}
 \chi_p(y) = \sqrt{2}\int_\mathbb{R} dp'\ e^{4\pi ipp'} \chi_{p'}(y) 
\end{align}
The character of a doubly degenerate representation is 
\begin{align}
 \chi_{\langle r,s\rangle}(y) = \sum_{k\in\mathbb{Z}} \left( \chi_{p_{\langle r,s\rangle} + k\sqrt{pq}} - \chi_{p_{\langle r,-s\rangle} + k\sqrt{pq}}\right)
\end{align}
Modular transforming, and using the Poisson summation formula,
\begin{align}
 \sum_{k\in\mathbb{Z}} e^{2\pi ikx} = \sum_{\ell\in\mathbb{Z}} \delta(x+\ell)
\end{align}
we obtain
\begin{align}
 \chi_{\langle r,s\rangle}(y) = 
 -\sqrt{\frac{2}{pq}}\sum_{\ell\in\mathbb{Z}} 
 \sin\left(\pi\frac{r}{q}\ell\right) 
 \sin\left(\pi\frac{s}{p}\ell\right)
 \chi_{\frac{\ell}{2\sqrt{pq}}}(\tilde{y})
\end{align}
Let us check that the modular $S$-matrix is
\begin{align}
 S_{\langle r,s\rangle, \langle r', s'\rangle} = -\sqrt{\frac{8}{pq}}(-1)^{rs'+r's} \sin\left(\pi\frac{p}{q}rr'\right)\sin\left(\pi\frac{q}{p}ss'\right)
\end{align}
We compute 
\begin{align}
 \frac12\sum_{r=1}^{q-1}\sum_{s=1}^{p-1} S_{\langle r,s\rangle, \langle r', s'\rangle} \chi_{\langle r,s\rangle}(y) = \chi_{\langle r',s'\rangle}(\tilde{y})
\end{align}
where we used
\begin{align}
 \sum_{r=1}^{q-1} (-1)^{rs'}\sin\left(\pi\frac{p}{q}rr'\right)\sin\left(\pi\frac{r}{q}\ell\right) 
 = -\frac{q}{2}\sum_{\epsilon =\pm} \epsilon \delta_{\ell\equiv\epsilon pr'+qs'\bmod 2q}
\end{align}
