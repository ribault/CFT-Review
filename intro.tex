...
\vspace{2mm}
 ...
 \textit{Lecture notes for the ``Young Researchers Integrability School'', Wien 2019}
...
Actually, modular invariance reduces to the invariance under two particular modular transformations, $Z(\tau)=Z(\tau+1)=Z(-\frac{1}{\tau})$. The condition $Z(\tau)=Z(\tau+1)$ amounts to $L_0-\bar{L}_0$ having integer eigenvalues, in other words all states having integer conformal spins.
...
\end{align}
For a fully degenerate representation in the Kac table of the $(p, q)$ minimal model, the structure is a bit more complicated: we have to subtract the two null vectors and their descendents, but add again the intersection of their two Verma submodules, which was subtracted twice. This intersection has a complicated structure, and the result is 
\begin{align}
...
\end{align}
The modular $S$-matrix turns out to be symmetric, showing that the A-series minimal models have modular invariant partition functions. Actually, the D-series minimal models too. Furthermore, these minimal models exist on arbitrary Riemann surfaces, and so does Liouville theory.

\begin{exo}[Modular $S$-matrices of minimal models]
 Show that the modular transformation of the character $\chi_{P}(\tau)$ of a Verma module is 
 \begin{align}
  \chi_P(\tau) = \sqrt{2}i\int_{i\mathbb{R}} dP'\ e^{4\pi iPP'} \chi_{P'}(-\tfrac{1}{\tau})\ . 
 \end{align}
Using the Poisson resummation formula $\sum_{k\in\mathbb{Z}} e^{2\pi ikx} = \sum_{\ell\in\mathbb{Z}} \delta(x+\ell)$, deduce the modular transformation of the characters of fully degenerate representation in the Kac table of the $(p, q)$ minimal model,
\begin{align}
...
\end{align}
show that the modular $S$-matrix is 
\begin{align}
...
\begin{enumerate}
 ...
 \item How are tutorials organized? Should lecture notes be adapted to tutorials? For instance, by listing all exercises, or choosing which exercies to do?
\end{enumerate}
