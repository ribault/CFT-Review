...
\end{align}
where $\widetilde{\mathcal{G}}^{(x)}_{\Delta,\bar\Delta}$ is now an interchiral block \eqref{gtrs}, and $\widetilde{\mathcal{S}}=\left.\mathcal{S}\right|_{-1<s\leq 1}$ is a reduced spectrum. In loop CFTs, the reduced spectrum is still infinite, but for any given value of $r\in\frac12\mathbb{N}^*$ it contains only $2r$ non-diagonal fields $V_{(r,s)}$ with integer spins, and their list Eq. \eqref{vex} reduces to 
\begin{subequations}
\label{vexr}
\begin{align}
 & V_{(\frac12, 0)},
...
Or just refer to \cite{nrj23}

Taking interchiral symmetry into account

forget interchiral for simplicity, no big deal to add it, just one layer of complexity
