...
Even then, solvable CFTs are very special cases. Starting from a simple CFT and following a renormalization group flow, we can easily end up at a fixed point whose CFT is intractable.


\subsubsection*{Few exact results in $d\geq 3$}
...
\begin{itemize}
 ...
 % \item Harmonic analysis on groups or group quotients provides a generic way of constructing CFTs, and generalizations thereof. The resulting theories are in general not exactly solvable \cite{kmp21}, but solvable examples with global conf. sym. although in d=2... not very exciting
 % \item Fishnet: better than N=4 SYM but still no exact results? References? Papers by Korchemsky with the word 'exact' in the title. But exact results in the planar limit only: do we have a CFT in that limit? Anyway these CFTs are non-unitary toy models for unitary N=4 SYM. 
