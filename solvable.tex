...
If $\Delta=\Delta_{(r,s)}$, there is a singular vector $L_{\langle r,s\rangle} V_{\Delta_{(r,s)}}$ at level $rs$ in the Verma module $\mathcal{V}_{\Delta_{(r,s)}}$. This singular vector manifests itself as a simple pole at $\Delta=\Delta_{(r,s)}$ of the vectors $(f^L)_{L\in\mathcal{L}_N}$ for any $N\geq rs$. Nevertheless, the Shapovalov form is invertible provided the singular vector vanishes (and any other singular vectors vanish as well). Then the primary field is fully degenerate, let us call it $V^f_{\langle r,s\rangle}$.
