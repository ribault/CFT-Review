...
Let us compute $s$-channel interchiral blocks for a 4-point function of the type $\left<\prod_{i=1}^4 V_{(r_i,s_i)}\right>$, remembering that this also covers diagonal fields thanks to Eq. \eqref{sP}. In terms of Virasoro blocks, the interchiral block for the $s$-channel interchiral representation $\widetilde{\mathcal{V}}_{(r,s)}$ \eqref{wvrs} reads 
\begin{align}
 \widetilde{\mathcal{G}}_{(r,s)} = \sum_{k\in \mathbb{Z}} \frac{D^{(s)}_{(r,s+2k)}}{D^{(s)}_{(r,s)}} \mathcal{F}_{P_{(r,s+2k)}} \bar{\mathcal{F}}_{P_{(-r,s+2k)}}\ . 
\end{align}
To make this explicit, it remains to compute the ratios of 4-point structure constants. This is in principle a straightforward consequence of the shift equations for 3-point structure constants \eqref{sh-mp}. However, for certain values of the parameters, these shift equations 
are finite after cancelling poles from the numerator and denominator. This problem can be avoided by writing the shift equations differently. In fact, in shifts of the reference 3-point structure constant $C^\text{ref}$ \eqref{cref}, no cancelling poles occur, thanks to the absolute values. The price to pay is that $C^\text{ref}$ only solves shift equations up to signs, see Eq. \eqref{tpt}. 
