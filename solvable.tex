...
If $\Delta=\Delta_{(r,s)}$, there is a singular vector $L_{\langle r,s\rangle} V_{\Delta_{(r,s)}}$ at level $rs$ in the Verma module $\mathcal{V}_{\Delta_{(r,s)}}$. This singular vector manifests itself as a simple pole at $\Delta=\Delta_{(r,s)}$ of the inverse Shapovalov form \eqref{smo}, so that $(f^L)_{L\in\mathcal{L}_{N\geq rs}}$ also have a simple pole at $\Delta=\Delta_{(r,s)}$.

TO BE COMPLETED

Nevertheless, the Shapovalov form is invertible provided the singular vector vanishes (and any other singular vectors vanish as well). Then the primary field is fully degenerate, let us call it $V^f_{\langle r,s\rangle}$.
The basis of descendant fields that appear in the OPE is a subset of $(LV^f_{\langle r,s\rangle})_{L\in\mathcal{L}}$. Their coefficients are again universal quantities. However, as we will see in Section \ref{sec:dope}, a degenerate field can appear in the OPE $V_{\Delta_1}V_{\Delta_2}$, only under certain conditions on $\Delta_1$ and $\Delta_2$.

In the case of the OPE coefficients $C^{\Delta,L}_{\Delta_1,\Delta_2}(z_1,z_2)$, we have just seen that the primary field determines the contributions of its descendant fields, provided there is no singular vector among these descendants, i.e. provided the primary field generates an irreducible representation. In Section \ref{sec:csc}, we will also see that correlation functions of primary fields determine the correlation functions of their descendant fields, with no caveats about singular vectors this time. This property of Virasoro symmetry would  not necessarily hold if we had a larger chiral symmetry algebra such as a W-algebra.
...
This definition works very well in \myindex{rational CFTs}\index{CFT!rational---}\index{rational CFT}, i.e. CFTs whose spectrums are generated by finitely many primary fields and their infinitely many descendants.
...
- At the end of Section 1.4, summarized notations and clarified the relation with reducibility.
