...
The \textbf{space of states} $\mathcal{S}$, also called spectrum or space of fields, is a vector space, whose elements $V(z)$ depend on a position $z\in\overline{\mathbb{C}}$ in the Riemann sphere. The Virasoro algebra acts on $\mathcal{S}$; depending on the context, we may use the following two notations for this action:
\begin{align}
 L_n^{(z)} V(z) = L_n V(z) \ . 
\end{align}
Primary states and descendant states that belong to $\mathcal{S}$ are now called \textbf{primary fields} and \textbf{descendant fields}. By definition, a primary field $V_\Delta(z)$ of conformal dimension $\Delta$  obeys 
\begin{align}
...
\begin{align}
  ...
  \label{pvlv}
 \end{align}
 Applying this to $L_n^{(z)} V(z)$, we find how the Virasoro generator $L_n^{(z)}$ depends on $z$,
 \begin{align}
...
The fundamental axiom of conformal field theory, which underlies the conformal bootstrap approach, is the existence of \textbf{operator product expansions (OPEs)}. This axiom states that there exists a bilinear product on the space of fields. Given a basis $(V_i)_i$ of the space of states $\mathcal{S}$, this reads 
\begin{align}
  \boxed{V_1(z_1)V_2(z_2) = \sum_i C^i_{12}(z_1,z_2) V_i(z_2)}\ ,
  \label{ope}
 \end{align}
...
We also assume that there exists an \textbf{identity field} $I(z)$ whose OPE with any other field is trivial, namely $I(z_1)V(z_2) = V(z_2)$. Let us consider the energy-momentum tensor $T$ as a field, then we have 
$
...
\begin{align}
 \Big< V_1(z_1) V_2(z_2) \cdots V_{|w_N\rangle}(z_N)\Big>\ . 
\end{align}
