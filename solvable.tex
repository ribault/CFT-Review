...
\begin{align}
 z = \frac{\theta_2^4(q)}{\theta_3^4(q)} \quad , \quad 1-z = \frac{\theta_4^4(q)}{\theta_3^4(q)}\ ,
\end{align}
where $\theta_k(q)$ are 
\myindex{Jacobi theta functions}\index{Jacobi theta function} 
\begin{align}
 \theta_2(q) = 2q^{\frac14}\sum_{n=0}^\infty q^{n(n+1)} \quad , \quad \theta_3(q) = \sum_{n=0}^\infty q^{n^2}\quad , \quad \theta_4(q) = \sum_{n=0}^\infty (-1)^n q^{n^2} \ ,
\end{align}
which obey the identity 
\begin{align}
 \sum_{k=2,3,4}(-1)^k \theta_k^4(q) = 0 \ . 
\end{align}
...
Let us describe the behaviour of the Virasoro block $\mathcal{F}_\Delta(z)$ as $\Delta\to \infty$, before explaining how this behaviour can be derived.
The Virasoro block may be rewritten as a prefactor, times a power series $H_\Delta(q)$, which has a trivial asymptotic behaviour:
\begin{align}
...
See Appendix D of \cite{msz15} for the pillow geometry that underlies the variable $q$.
...
In practice, instead of studying singular values, we count solutions by a rather pedestrian method, which does not require high values of $\Lambda$. To begin with, we can find whether $\mathcal{N}>0$ by examining how a solution $\left(D_k^{(x)|\Lambda,Z_\Lambda}\right)_{k,x}$ of the truncated system \eqref{fziz} behaves at $z_0\notin Z_\Lambda$. If $\mathcal{N}=0$, we expect a rather large discrepancy between the three channels, of the order $\max_{k\in \mathcal{S}_\Lambda^{(s)}} \left(\epsilon_k^{(s)|\Lambda} D_k^{(s)|\Lambda,Z_\Lambda}\mathcal{G}^{(s)}_{\Lambda|\Delta_k\bar\Delta_k}(z_0)\right) $. This means that in this case the deviation $\epsilon_k^{(s)|\Lambda}$ measures the failure of crossing symmetry. If on the other hand $\mathcal{N}>1$, then the large deviation only reflects the existence of several solutions, and the three channels should agree up to a small discrepancy that is due to our truncation of the system, typically $\ll \max_{k\in \mathcal{S}_\Lambda^{(s)}} \left(D_k^{(s)|\Lambda,Z_\Lambda}\mathcal{G}^{(s)}_{\Lambda|\Delta_k\bar\Delta_k}(z_0)\right) $.
