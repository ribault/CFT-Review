...
More generally, there is an infinite family of conformal dimensions $(\Delta_{\langle r,s\rangle})_{r,s\in\mathbb{N}^*}$ such that $\mathcal{V}_{\Delta_{\langle r,s\rangle}}$ has a null vector $|\chi_{\langle r,s\rangle}\rangle $ at level $N=rs$. For $N=1,2$ we have found 
\begin{align}
...
\end{align}
To write $\Delta_{\langle r,s\rangle}$ for any level $N\in\mathbb{N}^*$, we rewrite the conformal dimension in terms of the momentum $P$ defined by 
\begin{align}
...
\label{eq:ars}
\end{align}
For $\beta^2\in\mathbb{C}-\mathbb{Q}$, the Verma module $\mathcal{V}_\Delta$ has a null vector if and only if $\Delta=\Delta_{\langle r,s\rangle}$ for some $r,s\in\mathbb{N}^*$, in which case it has only one null vector. For $\beta^2\in\mathbb{Q}$ however, there are coincidences of the type $\Delta_{\langle r,s\rangle}=\Delta_{\langle r',s'\rangle}$ with $(r,s)\neq (r',s')$, and Verma modules can have multiple null vectors. In fact, they can have null vectors that are not of the type $|\chi_{\langle r,s\rangle}\rangle= L_{\langle r,s\rangle} |v\rangle$, bur rather  $L_{\langle r'',s''\rangle}L_{\langle r,s\rangle} |v\rangle$.

The structure of Verma modules with multiple null vectors depends a lot on the sign of $\beta^2$. This is because the set $\{\Delta_{\langle r,s\rangle}\}_{r,s\in\mathbb{N}^*}$ is bounded from above if $\beta^2<0$, but not if $\beta^2>0$. However, the null vector always has a larger dimension than its primary state. This results in infinitely many null vectors if $\beta^2>0$ (null vectors of null vectors of null vectors...), and finitely many if $\beta^2<0$. To summarize:
\begin{align}
 \begin{array}{|c||c|c|c|}
  \hline 
  ...
  \\
  \hline 
  \#\text{null vectors in } \mathcal{V}_{\Delta_{\langle r,s\rangle}} & 1 & \infty & \text{finite} 
  \\
  \hline 
 \end{array}
\end{align}
