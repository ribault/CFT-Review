...
\begin{align}
 ...
 \label{dnmono}
\end{align}
where the phase is $e^{2\pi i(S_{(r,s\pm 1)}-S_{(r,s)})} =e^{2\pi ir}\in \{1,-1\}$. In a CFT where $r$ can be half-integer, the existence of $V^d_{\langle 1,2\rangle}$ therefore requires that we slightly relax the axiom of single-valuedness, which would imply that the phase is $1$.
In any case, 
