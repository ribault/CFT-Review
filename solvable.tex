...
In this section we derive the spectrums and fusion rules of exactly solvable CFTs. We begin with a general discussion of CFTs, their spectrums, and the consequences of having degenerate fields. 
...
Let us start with the case $\beta^2=-\frac{q}{p}<0$ with $p,q\in\mathbb{N}^*$ coprime. Let us repeatedly fuse $\mathcal{R}^f_{\langle 2, 1\rangle}$ with itself, and assume that all resulting representations are fully degenerate. We therefore obtain a subset of $\left\{\mathcal{R}^f_{\langle r,1\rangle}\right\}_{r\in\mathbb{N}^*}$. To see whether this subset is finite, we examine the restrictions on fusion rules that come from the existence of multiple vanishing null vectors. The first representation with two null vectors is $\mathcal{R}^f_{\langle p+1,1\rangle}=\mathcal{R}^f_{\langle 1,q+1\rangle}$. Due to the second null vector, there is only one term in 
\begin{align}
\mathcal{R}^f_{\langle 2, 1\rangle}\times \mathcal{R}^f_{\langle p+1,1\rangle} = \mathcal{R}^f_{\langle p+2,1\rangle}=\mathcal{R}^f_{\langle 2,q+1\rangle}\ ,
\end{align}
i.e. the term $\mathcal{R}^f_{\langle p,1\rangle}$ from Eq. \eqref{rrrs} is absent. But this does not prevent repeated fusion with $\mathcal{R}^f_{\langle 2, 1\rangle}$ from generating representations with ever higher $r$. Therefore, there are no diagonal, rational CFTs with $\beta^2<0$. 

We therefore write $\beta^2 = \frac{q}{p}>0$ with $p,q\in\mathbb{N}^*$ coprime.


If positive get Kac table by argument of \cite{rib14}. Do we take arbitrary $r,s$ in argument, or restrict to Kac table? 



Fusion rules. 

Trivial rational diagonal CFTs: identity CFT, submodels. 
