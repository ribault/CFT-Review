...
Can no longer write book on CFT which would be only about 2d. But exact solvability still only 2d. Known techniques rely on local conformal symmetry (infinite-dimensional) and degenerate fields (finite OPEs). Bootstrap, analytic and numerical. Exact solution means analytic expression for 3-point structure constants. Here: CFTs that are presumed to be solvable, not necessarily solved. Low central charge, good reason for that (half-plane). Plenty of applications including \cite{jcpb23}. High $c$ a challenge, applications to gravity.  

Focus on CFTs whose chiral symmetry algebra is only Virasoro. How do we see chiral symmetry? In spectrum: current may or may not appear, what matters is integer gaps in spectrum: if we knew only about global conf. sym. we would detect local sym. by seeing infinite families of integer-spaced primaries. If we had higher that Virasoro we would see Virasoro primaries whose dimensions differ by integers, or simple fractions (parafermionic symmetry). Not only in spectrum: we also want symmetry in interactions, selection rules from Ward identities. Cf Coulomb gas methos: affine abelian algebra but no conserved momentum. \textbf{Extended chiral algebra}: our methods would apply. 

Methods: forget Coulomb gas, a dead end. For correl fct, applies only if we have two deg. fields? But used to find spectrum of $O(n)$ etc.  Historically, the spectrums and fusion rules of solvable CFTs have been derived by studying the torus partition function. The torus partition function is a simple object that only depends and the spectrum, and not on the structure constants. It can be computed either by solving modular invariance equations, or constructively via a path integral or a lattice sum. And in rational CFTs, the modular invariance equations involve the modular S-matrix, which also allows us to determine the fusion rules via the Verlinde formula \cite{fms97}.
Here, we will follow a simpler approach, based on exploiting the fusion rules of degenerate fields. We do not lose in generality, because all known exactly solvable CFTs have degenerate fields. And we gain in flexibility: in many cases, adding fields to the spectrum is consistent with fusion rules, but breaks modular invariance. For example, in the Ising model, the modular invariant torus partition function only involves three primary fields, and excludes disorder fields, as well as the fields that are needed for computing connectivities of clusters. Focus on correlation functions: notion of extended spectrum, incompatible with modular invariance. Spectrum not so fundamental, can get it otherwise. 

This review: simple derivations of results preferred to conceptual aspects. Calculations not done in detail but all doable with a few exceptions. Not formulated as exercises unlike \cite{rib16} and \cite{rib14}. Main difference with these texts: focus on non-diagonal CFTs, particularly loop models. 


\subsection*{Acknowledgements}

Course organizers. Collaborators. 
...
\subsubsection{Various flavours of numerics}

Assuming that we exactly know the spectrum, the situation depends on what we know about structure constants (inputs), and what we want to determine (outputs). 
\begin{enumerate}
 \item \textbf{We know the structure constants.} All that remains is to test crossing symmetry of 4-point functions, by numerically computing them in various channels. This has been done in Liouville theory \cite{zz95, rs15}, generalized minimal models \cite{rs15}, Runkel--Watts-type CFTs \cite{rs15}, and generalized D-series minimal models  with $c\leq 1$ \cite{rib19}. 
 \item \textbf{We do not know the structure constants, and only want to check the spectrum.} 
 We view crossing symmetry as a system of linear equations for 4-point structure constants. The very existence of solutions is a strong test on the spectrum. The OPE spectrums that are relevant to 4-point connectivities in the Potts model have been tested in this way \cite{hjs20, nr20}, after having been predicted using lattice methods \cite{js18}. This also works for computing the 4-point functions themselves \cite{prs16, prs19}, or checking individual structure constants that may be known \cite{nr20}, spaces of solutions. Combinatorial maps paper also an example. 
 ...
 \item Using the factorization of 4-point structure constants into 3-point structure constants (see Table \eqref{stu}), crossing symmetry is a system of nonlinear equations for the 3-point structure constants.
\end{enumerate}

Various styles of numerical bootstrap, all assuming we know (or guess) spectrum analytically:  
\begin{itemize}
 \item Checking crossing sym. when we know struct. csts. Can be reassuring. Allows us to actually compute 4pt functions. 
 \item Counting solutions: their existence, dimension of space. Check spectrum, deduce fusion rules. 
 \item Determining struct. csts numerically or even analytically. 
\end{itemize}
...
Factorization of structure constants. Pb of field multiplicities, of fusion multiplicities. 
...
Predict conformal spin? Or just match sets of parameters? 

Loop weight invariant under shifts, but CFT not invariant, discussion of criticality 

\subsection{Analytic properties of structure constants}

Poles and their cancellations. Degrees of polynomials 
