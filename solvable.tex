...
As we have seen in Section \eqref{sec:ab}, degenerate fields play a crucial role by constraining structure constants via shift equations. This has led to the idea of extending the conformal algebra into an  \textbf{interchiral algebra}, which would also include the degenerate fields \cite{grs12}. In the same way as the conformal algebra is generated by the energy-momentum tensors $T,\bar T$, leading to conformal Ward identities, an interchiral algebra could be generated by $T,\bar T,V^d_{\langle 1,2\rangle}$ or $T,\bar T,V^d_{\langle 2,1\rangle},V^d_{\langle 1,2\rangle}$, and we would consider shift equations as interchiral Ward identities. 

\subsubsection{Interchiral symmetry}

Algebra not very well defined.

Representations also not so clear in CFT but somewhat useful in lattice. 

Blocks useful in bootstrap if discrete spectrum. Not really in CFTs with 2 deg. fields such as MM: full correlator. Will focus on loop CFTs.

\subsubsection{Interchiral blocks in loop CFTs}
