...
\begin{align}
 \boxed{\forall z\in \overline{\mathbb{C}}\backslash\{0,1,\infty\} \ , \quad \sum_{k\in\mathcal{S}} D_k^{(s)} \mathcal{G}^{(s)}_{\Delta_k,\bar\Delta_k}(z) = \sum_{k\in\mathcal{S}} D_k^{(t)} \mathcal{G}^{(t)}_{\Delta_k,\bar\Delta_k}(z) = \sum_{k\in\mathcal{S}} D_k^{(u)} \mathcal{G}^{(u)}_{\Delta_k,\bar\Delta_k}(z)} \ . 
 \label{seteu}
\end{align}
...
\begin{align}
 \begin{array}{|c|c|l|}
  ...
  \\
  \hline 
  \widehat{\text{CFT}} & \text{Extended spectrum } \widehat{\mathcal{S}} & \mathcal{S}\subset \widehat{\mathcal{S}}\ , \quad i,j\in\widehat{\mathcal{S}}\implies \mathcal{S}_{i,j}\subset \widehat{\mathcal{S}}
  \\
  \hline 
 \end{array}
 \label{specs}
...
Together with the closure of the spectrum under OPEs, this leads to constraints on non-diagonal fields. If indeed $V^d_{\langle 1,2\rangle}\in\widehat{\mathcal{S}}$, then $V_{(r,s)}\in\widehat{\mathcal{S}}\implies V_{(r,s\pm 1)}\in\widehat{\mathcal{S}}$. 
...
\end{align}
If $V^d_{\langle 1,2\rangle}$ exists, then the OPE \eqref{vdvrs} increments $s$ by one unit, therefore $s$ is unbounded in real part i.e. $\sup_{V_{(r,s)}\in \widehat{\mathcal{S}}} \left(\Re s\right) = \infty$. Assuming that this holds not only for the extended spectrum $\widehat{\mathcal{S}}$ but also for some OPE spectrum $\mathcal{S}_{12}$, we deduce $\Re\beta^{-2}>0$, or equivalently 
\begin{align}
...
crossing symmetry equations \eqref{seteu} form a system of linear equations for the 4-point structure constants $D^{(x)}_k$. There are infinitely many unknowns if the spectrum is infinite. There are also infinitely many equations: two for each value of the cross-ratio $z\in \overline{\mathbb{C}}\backslash\{0,1,\infty\}$. For numerical calculations on a computer, we need a truncation to a system of finitely many equations with finitely many unknowns. Symmetries, starting with interchiral symmetry, can also be used for reducing the number of unknowns. 
...
As we saw in Section \ref{sec:icb}, the interchiral symmetry associated to the degenerate field $V^d_{\langle 1,2\rangle}$ determines how 4-point structure constants $D^{(x)}_{(r,s)}$ behave under $s\to s+2$. This allows us to rewrite the crossing symmetry equations \eqref{seteu} as 
\begin{align}
 \forall z\in \overline{\mathbb{C}}\backslash\{0,1,\infty\} \ , \quad \sum_{k\in\widetilde{\mathcal{S}}} D_k^{(s)} \widetilde{\mathcal{G}}^{(s)}_{\Delta_k,\bar\Delta_k}(z) = \sum_{k\in\widetilde{\mathcal{S}}} D_k^{(t)} \widetilde{\mathcal{G}}^{(t)}_{\Delta_k,\bar\Delta_k}(z) = \sum_{k\in\widetilde{\mathcal{S}}} D_k^{(u)} \widetilde{\mathcal{G}}^{(u)}_{\Delta_k,\bar\Delta_k}(z)\ ,
\end{align}
where $\widetilde{\mathcal{G}}^{(x)}_{\Delta,\bar\Delta}$ is now an interchiral block \eqref{gtrs}, and $\widetilde{\mathcal{S}}=\left.\mathcal{S}\right|_{-1<s\leq 1}$ is a reduced spectrum. In loop CFTs, the reduced spectrum is still infinite, but the list \eqref{vex} of non-diagonal fields $V_{(r,s)}$ with integer spins reduces to $2r$ fields for any given value of $r\in\frac12\mathbb{N}^*$:
\begin{subequations}
\label{vexr}
\begin{align}
 & V_{(\frac12, 0)},
...
In addition to interchiral symmetry, some 4-point functions $\left<\prod_{i=1}^4 V_{(r_i,s_i)}\right>$ also have discrete $\mathbb{Z}_2$ symmetries that reduce the number of unknowns. We will sketch the ideas here, and refer the reader to \cite[Section 2.4]{nrj23} for technical details. 

The \textbf{parity transformation} $z\to \bar z$ exchanges the left- and right-moving Virasoro algebras, and therefore acts on non-diagonal fields as $V_{(r,s)}\to V_{(r,-s)}$. If our 4-point function is parity-invariant i.e. if $s_i=0$, then the space of solutions of crossing symmetry equations decomposes into a parity-even subspace (with $D^{(x)}_{(r,s)}=D^{(x)}_{(r,-s)}$) and a parity-odd subspace (with $D^{(x)}_{(r,s)}=-D^{(x)}_{(r,-s)}$). More generally, if our 4-point function is parity-invariant modulo interchiral symmetry i.e. $s_i\in\mathbb{Z}$, then the parity induces a $\mathbb{Z}_2$ action on the space of solutions. This action is in general more complicated than $D^{(x)}_{(r,s)}\mapsto D^{(x)}_{(r,-s)}$, and its eigenvalues do not have a simple behaviour (even or odd) under $z\to \bar z$. Nevertheless, focussing on an eigenspace still reduces the number of unknowns by roughly half. 

The permutation $z_1\leftrightarrow z_2$ is a symmetry of the crossing symmetry equations provided $(r_1,s_1)=(r_2,s_2)$. Again, this allows us to decompose the space of solutions into permutation-even and permutation-odd subspaces. Of course, if the fields $V_{r_1,s_1)$ and $V_{(r_2,s_2)}$ are actually identical, then only permutation-even solutions are relevant. However, if we allow non-trivial field multiplicities, then these fields may differ while having the same conformal dimensions, and permutation-odd solutions may also be relevant. In practice, the action of our permutation on 4-point structure constants is sensitive on



It may also happen that our two fields have the same dimensions modulo interchiral symmetry, in which case 

Linear relations from symmetry.
...
\subsection{What about global symmetry groups?}
...
Surely this is because they are all formulated in terms of similar loop ensembles, see Section \ref{sec:coup}.
Moreover, if we decompose the spectrum of the $O(n)$ CFT into of $O(n)$ representations, as we sketched in Eq. \eqref{von}, we find that a primary field $V_{(r,s)}$ with a given conformal dimension generally does not transform in an irreducible representation, but in a larger representation. This is a hint that the model's global symmetry is larger than the group $O(n)$. 
...
When it comes to a 4-point function $\left<\prod_{i=1}^4 V_{(r_i,s_i)}\right>$, the number of 4-point invariant tensors is larger than the number of solutions of crossing symmetry (= the number of combinatorial maps). If $\sum r_i = 2, 3$ these numbers actually coincide, while if $\sum r_i\geq 4$ the inequality becomes strict in most cases (for example, $6>5$ for $\big\langle V_{(\frac32,0)}^2V_{(\frac12,0)}^2\big\rangle$), as the number of solutions grows much slower with $\sum r_i$ \cite{gjnrs23}. The reason for this slower growth is not only that the symmetry is larger than $O(n)$, but also that an OPE of 2 primary fields has no reason to include all the primary fields that are allowed by $O(n)$ symmetry, 

In $d\geq 3$ dimensions, the $O(n)$ and $U(n)$ models can still be formulated in terms of loops, but the question of whether two loops intersect makes no sense macroscopically. The case $d\geq 3$ is therefore similar to the case $d=2$ with intersecting loops, and we expect no larger symmetry. For any conformal dimension that appears in the spectrum, we expect that there exists one primary field that transforms in an irreducible representation of $O(n)$ or $U(n)$. On the other hand, for any irreducible representation of $O(n)$ or $U(n)$, we expect that there exists infinitely many primary fields, whose conformal dimensions are all different.
Moreover, in the Potts model, the boundaries of clusters are no longer loops, and we do not expect the Potts model to share non-trivial conformal dimensions with the $O(n)$ or $U(n)$ model. 
