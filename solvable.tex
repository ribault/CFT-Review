...
\end{tikzpicture} 
 ...
 \label{stu}
\end{align}
In order to write these decompositions in greater generality, let us define the $x$-channel \myindex{conformal blocks}\index{block!conformal---} as 
\begin{align}
...
Together with the closure of the spectrum under OPE, this leads to constraints on non-diagonal fields. If indeed $V^d_{\langle 1,2\rangle}\in\widehat{\mathcal{S}}$, then $V_{(r,s)}\in\widehat{\mathcal{S}}\implies V_{(r,s\pm 1)}\in\widehat{\mathcal{S}}$. 
...
For $\beta^2\in\mathbb{C}^*\backslash\mathbb{Q}$, we define the \myindex{generalized minimal model (GMM)}\index{minimal model!generalized---} as the CFT whose spectrum of primary fields is made of all diagonal degenerate fields:
\begin{align}
...
\end{align}
and we call the corresponding CFT a \myindex{generalized D-series minimal model}\index{minimal model!generalized D-series---}. Due to the lack of $r\leftrightarrow s$ symmetry in the non-diagonal sector, this theory depends on $\beta^2$ rather than the central charge $c$, i.e. it is not invariant under $\beta \to \beta^{-1}$. 
...
\begin{align}
 ...
 \label{sh-mp}
\end{align}
These are called \myindex{shift equations}, because they determine how structure constants behave when the momentums $P_1,\bar P_1$ of fields are shifted by $\beta$, under the condition $s_i\in \frac12 \mathbb{Z}$ \eqref{ssiz}. If $s_i\in\mathbb{Z}$, then not only $S_{i^-}-S_{i^+}\in\mathbb{Z}$ but also $S_{i}-S_{i^+}\in\mathbb{Z}$, and we also have the more refined shift equation, deduced from Eq. \eqref{dpdp},
\begin{align}
...
In the loop CFTs' extended spectrum $\widehat{S}^\text{loop}$ \eqref{sloop}, it is clear that shift equations do not determine the structure constants:
\begin{itemize}
...
\begin{align}
 ...
 \label{resf}
\end{align}
where the coefficient $R_{r,s}$ is called a \myindex{Virasoro block residue}\index{block!---residue}, and we use the momentum $P$ rather than the conformal dimension $\Delta$. We will derive an expression of the type 
\begin{align}
...
\begin{align}
 b^{r,s} = \frac{-(-)^{rs}}{P_{(0,0)}} \prod_{j=1-r}^r\prod_{k=1-s}^s 2P_{(j,k)} = 
 \frac{-2\prod_\pm \Gamma_\beta\left(\beta \pm 2P_{(r,s)}\right)}{\Gamma_\beta\left(\beta+2P_{(r,-s)}\right)\operatorname{Res}_{\beta-2P_{(r,-s)}}\Gamma_\beta}\ .
 \label{brs}
\end{align}
...
\begin{align}
 \mathcal{F}_\Delta(q) = (16q)^\Delta \times \cdots 
 \label{fdq}
\end{align}
...
$S_3$ subgroup of global conformal group that permutes $z=0,1,\infty$. 
...
\begin{align}
 ...
 \begin{array}{|c||c|c|c|c|c|c|c|}
  ...
  \\
  \hline 
  q & 0,\pm i & 1 & -1 & \pm ie^{-\frac{\pi}{2}} & -e^{-\pi} & e^{-\pi} & \pm ie^{-\frac{\pi}{2}\sqrt{3}}
  \\
  \hline 
 \end{array}
 \label{zqval}
...
\begin{align}
 ...
 \label{fziz}
\end{align}
After choosing a normalization, for example $D_{k_0}^{(s)}=1$, this system would generically have a unique solution if it had as many equations as unknowns, i.e. if $|Z_\Lambda|=\sum_{x\in\{s,t,u\}}\left|\mathcal{S}^{(x)}_\Lambda\right|-1$. For numerical stability, it does not hurt to make the system slightly overdetermined.

\subsubsection{Studying one solution}


Deviations. Detection of solutions. 

Relative, absolute deviations. Explicit example of a table of deviations. Vanishing structure constants. 

\subsubsection{Counting solutions}

Singular values method: not very systematic in practice! 
...
\begin{align}
 ...
 \label{sxs}
\end{align}
where we define the map's $x$-channel \myindex{signature}\index{signature} $\sigma^{(x)}\in\frac12\mathbb{N}$ as half the minimum number of lines in $M$ that are crossed by an $x$-loop. For example, the map 
$
...
\end{align}
This is a combination of the blocks for the Verma modules of $V_{(r,s)}$ and $V_{(r,-s)}$. Their relative coefficient $\frac{R_{r,s}}{\vphantom{\tilde{|}}\bar{R}_{r,s}}$ can be determined either by taking a limit in $s_i$ with $r_i$ fixed \cite{gnjrs21}, or by using the shift equations \eqref{cshift}, \eqref{bshift}. 
