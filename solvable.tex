...
\subsubsection{OPE Ward identities and their solutions}

Let us work out the consequences of conformal symmetry for OPEs. For technical simplicity, we focus on an OPE of two primary fields. In that OPE, we single out the contributions of a primary field $V_\Delta$ and its descendants:
\begin{align}
...
\end{align}
\end{itemize}
This leads to a system of linear equations for the OPE coefficients $C^{\Delta,L}_{\Delta_1,\Delta_2}$, called \textbf{OPE Ward identities}. It turns out that for $\Delta\notin \left\{\Delta_{(r,s)}\right\}_{r,s\in\mathbb{N}^*}$, this system has a unique solution $f^{\Delta,L}_{\Delta_1,\Delta_2}$, assuming that the coefficient of the primary field is normalized as
$f^{\Delta,1}_{\Delta_1,\Delta_2}=1$. Let us illustrate this claim by deriving the first few coefficients $f^{\Delta,L}_{\Delta_1,\Delta_2}$ with $L\in \{L_{-1},L_{-1}^2,L_{-2}\}$. Let us write the term of order $O(z_{12})$ of the $n=1$ OPE Ward identity, i.e. $L=1$ on the left and $L=L_{-1}$ on the right:
\begin{align}
 (\Delta+\Delta_1-\Delta_2)V_\Delta(z_2) = f^{\Delta,L_{-1}}_{\Delta_1,\Delta_2} L_1L_{-1}V_\Delta(z_2)\ .
\end{align}
This determines $f^{\Delta,L_{-1}}_{\Delta_1,\Delta_2}$, except if its coefficient vanishes, which occurs if and only if there is a null vector at level one in the Verma module $\mathcal{V}_\Delta$, i.e. if and only if $\Delta=\Delta_{(1,1)}=0$. In the resulting expression, the null vector manifests itself as a simple pole at $\Delta=0$:
\begin{align}
 f^{\Delta,L_{-1}}_{\Delta_1,\Delta_2} = \frac{\Delta+\Delta_1-\Delta_2}{2\Delta}\ . 
\end{align}
Similarly, the level two coefficients $f^{\Delta,l_{-1}^2}_{\Delta_1,\Delta_2},f^{\Delta,L_{-2}}_{\Delta_1,\Delta_2}$ are determined by the $O(z_{12}^2)$ terms in the $n=1,2$ OPE Ward identities.


summary OPE structure with OPE struct cst... simple pole for generic $c$. 
