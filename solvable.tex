...
\end{itemize}
For example, in the case $N=4$, the $s$-channel spectrum is 
\begin{align}
\renewcommand{\arraystretch}{1.5} 
 \mathcal{S}^{(s)} = 
 \left\{\begin{array}{ll} 
...
The crossing symmetry equations for $\left<\prod_{i=1}^N V_{(r_i,s_i)}\right>$, with the spectrums that we just sketched, define a vector space $\mathcal{Z}$ of solutions. 
We conjecture that each combinatorial map $M$ corresponds to a solution $Z_M = \lim_\text{critical} Z(\mathcal{E}_M)$, and that these solutions form a basis of the space of solutions \cite{gjnrs23}:
\begin{align}
...
\begin{multline}
  ...
  \ ,
\end{multline}
where the regularized Virasoro block at its pole $P=P_{(r,s)}$ was defined in Eq. \eqref{freg}.
...
\end{align}
where $d_{(r,s)}$ is a holomorphic function of $w$, and the residue of the pole has been computed using Eq. \eqref{rdp}, and expressed in terms of the quantity
\begin{align}
