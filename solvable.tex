...
\begin{align}
 ...
 \label{degsets}
\end{align}
In particular, the set that is generated by $\{V^d_{\langle 2,1\rangle},V^d_{\langle 1,2\rangle}\} $ is the full set of degenerate fields $\{V^d_{\langle r,s\rangle}\}_{r,s\in\mathbb{N}^*}$. For our purposes, it is enough to distinguish 3 types of CFTs:
\begin{itemize}
 \item CFTs with two degenerate fields, in the sense that both $V^d_{\langle 2,1\rangle}$ and $V^d_{\langle 1,2\rangle}$ exist. 
 \item CFTs with one degenerate field $V^d_{\langle 1,2\rangle}$. Because the invariance under $\beta\to \beta^{-1}$ is broken by choosing $V^d_{\langle 1,2\rangle}$ rather than $V^d_{\langle 2,1\rangle}$, such CFTs depend on $\beta^2$ \eqref{cb} rather than on the central charge $c$. 
 \item CFTs with no nontrivial degenerate field, where only $V^d_{\langle 1,1\rangle}$ exists.
\end{itemize}
We will not consider CFTs where $V^d_{\langle 1,3\rangle}$ exists but not $V^d_{\langle 1,2\rangle}$. 
When it comes to the spectrum, the situation is more diverse, and pretty much all sets of degenerate fields \eqref{degsets} can occur. 

For $\beta^2\in\mathbb{Q}$, the situation is more complicated, because we have to distinguish fully degenerate fields from partly degenerate fields. And the fully degenerate fields $V^f_{\langle 2,1\rangle}$ and $V^f_{\langle 1,2\rangle}$ are not independent, as follows from the identities
\begin{align}
...
Motivated by the single-valuedness of correlation functions, we have made the assumption \eqref{siz} that conformal spins are integer. A simple way to satisfy this assumption is to have fields of spin zero, i.e. \textbf{spinless fields}. However, whenever degenerate fields exist, we will use the slightly different notion of a \textbf{diagonal field}: a spinless field whose OPEs with degenerate fields only produces spinless fields. Conversely, a \textbf{non-diagonal field} is a field that either has nonzero spin, or is related to fields with nonzero spins by taking OPEs with degenerate fields. A \textbf{diagonal CFT} is a CFT whose spectrum only involves diagonal fields.  

We assume that the degenerate fields that we discussed in Section \ref{sec:hmdf} are diagonal. (In minimal models, there can also exist non-diagonal degenerate fields.) For diagonal primary fields that are not degenerate, we will use the notation $V_P$, where $P\in\mathbb{C}$ is the momentum. The OPE of a diagonal degenerate field with $V_P$ is determined by the fusion rules \eqref{rvvp}, and the requirement that only spinless primary fields can appear:
\begin{align}
 ...
 \label{vpope}
\end{align}
Non-diagonal fields are in principle parametrized by their left and right momentums $P,\bar P$. We rewrite these momentums as $P_{(r,s)}, P_{(r,-s)}$, where $r,s\in\mathbb{C}$ are the Kac indices defined by Eq. \eqref{prs}, and call $V_{(r,s)}$ our non-diagonal primary field. According to Eq. \eqref{drms}, the conformal spin of this field is 
\begin{align}
 \boxed{S_{(r,s)}= rs} \ . 
\end{align}
Using Kac indices as parameters will simplifies OPEs with degenerate fields. According to the fusion rules \eqref{rvvp}, the primary fields that can appear in the OPE $V^d_{\langle 2,1\rangle}V_{(r,s)}$ and $V^d_{\langle 1,2\rangle}V_{(r,s)}$ are 
\begin{align}
...
\begin{align}
 ...
 \label{vdvrs}
\end{align}
While they were derived under the assumption $\beta^2\notin \mathbb{Q}$, we will assume that these OPEs are valid for any value of the central charge. The OPEs \eqref{vdvrs} and \eqref{vpope} indicate which primary fields can appear: we now assume that in each one of these OPEs, the two fields that can appear do appear. Since 
the spectrum must be closed under OPEs, this assumption allows degenerate fields to constrain the spectrum. This assumption may not hold if one of the involved fields has a null vector, in addition to the level $2$ null vector of $V^d_{\langle 2,1\rangle}$ or $V^d_{\langle 1,2\rangle}$. In particular, a vanishing null vector can further constrain the OPE. 

Let us summarize the primary fields that we have been considering, together with their conformal dimensions, and the representations of the conformal algebra that they generate: 
\begin{align}
 \begin{array}{|r|l|l|l|l|}
  ...
  \\
  \hline 
  \text{Non-diagonal} & V_{(r,s)} & rs\in \tfrac12\mathbb{Z} & \left(\Delta_{(r,s)},\Delta_{(r,-s)}\right) & 
 ...
 \label{fields}
\end{align}
The non-diagonal primary field $V_{(r,s)}$ generates the Verma module $\mathcal{V}_{P_{(r,s)}} \otimes \bar{\mathcal{V}}_{P_{(r,-s)}}$, unless that module has null vectors, which occurs if $r,s\in\mathbb{Z}^*$, and also for other values of $r,s$ if $\beta^2\in\mathbb{Q}$. If there are null vectors, $V_{(r,s)}$ may generate a degenerate representation, or belong to a logarithmic representation. We will discuss such cases as they arise.  

\subsection{CFTs with 1 degenerate field}

\subsubsection{General results}

Let us first constrain the non-diagonal fields that can belong to a spectrum $\mathcal{S}$ or extended spectrum $\widetilde{\mathcal{S}}$, depending on assumptions on conformal spins and degenerate fields. The constraints come from computing differences of spins \eqref{sdiff2} in the OPE $V^d_{\langle 1,2\rangle}V_{(r,s)}$ \eqref{vdvrs} or $V^d_{\langle 1,3\rangle}V_{(r,s)}=V_{(r,s-2)}+V_{(r,s)}+V_{(r,s+2)}$:
\begin{align}
 \begin{array}{|c|c|c|c|}
 \hline 
  \text{Spectrum} & \text{Degenerate field} & \text{Spins} & \text{Condition} 
  \\
  \hline \hline 
  \mathcal{S} & V^d_{\langle 1,3\rangle} &  S\in \mathbb{Z} & r\in\frac12\mathbb{Z} 
  \\
  \hline 
  \mathcal{S}\text{ or }\widetilde{\mathcal{S}}  & V^d_{\langle 1,2\rangle} &  S\in \mathbb{Z} & r\in\mathbb{Z} 
 \\
 \hline 
 \widetilde{\mathcal{S}}  & V^d_{\langle 1,2\rangle} &  S\in \frac12\mathbb{Z} & r\in\frac12\mathbb{Z} 
 \\
 \hline 
 \end{array}
 \label{rcon}
\end{align}
The first Kac index must therefore be half-integer. Let us show that it is conserved modulo integers. Since $V^d_{\langle 1,2\rangle}$ is always assumed to exist, we may consider the four-point function $\left<V^d_{\langle 1,2\rangle}(z)V_{(r_1,s_1)}(0)V_{(r_2,s_2)}(\infty)V_{(r_3,s_3)}(1)\right>$. 

first write deg. OPE monodromy indep. of 4pt! 

$r=0$ same as diagonal. 

Study monodromies of 4-point function with $V^d_{\langle 1,2\rangle}$ (not single valued) leads to $r_1+r_2+r_3\in\mathbb{Z}$. Applies also to D-series MM, explains conservation of diagonality.

\subsubsection{Loop CFT}

\subsubsection{$O(n)$ and Potts models}
...
Get D-series, can we get E-series? Maybe yes, with vanishing of struct. csts explaining disappearance of some fields? 
