...
Starting from a statistical model on a finite lattice, the large lattice limit may give rise to a conformal field theory, provided the lattice couplings are set to certain critical values. 

Plenty of it is non-universal: lattice, interaction strength, rules for drawing loops on lattice.

Focus on universal features and values of central charge: criticality lines. 


Loop CFTs are defined for the whole range of values of $\beta^2$ \eqref{rbp} that are allowed for non-diagonal CFTs with one degenerate fields. However, the corresponding statistical lattice models exist on a smaller range. This is first because the relation between $\beta^2$ and the models' parameters $n,Q$ is not bijective, and maps the strip $0<\Re \beta^2\leq 1$ to the full complex $n$-plane. Furthermore, not all complex values of $n,Q$ are possible, if we want the models to have a critical limit. However, the CFT gives us hints about which values are possible.

To reach the critical limit from a lattice model, we must pay attention to which fields are relevant $\Re(\Delta+\bar\Delta)<2$, marginal $\Re(\Delta+\bar\Delta)=2$, or irrelevant $\Re(\Delta+\bar\Delta)>2$.
Only relevant fields can influence whether the renormalization group flow reaches a critical limit. Therefore, the boundary of the region of criticality is expected to correspond to some field becoming relevant. A subtlety is that the appearance of a relevant field is not necessarily fatal: depending on the model's precise formulation, and in particular on which symmetries are preserved, the relevant field may or may not influence the flow. 

Some universal results: plot marginality lines. Focus on deg. fields because most studied and also do not generate the others so not destabilization of the flow by dangerously irrelevant opeartors. Also, non-diag. fields often forbidden by this or that symmetry/assumption? 
