...
In 1997, di Francesco, Mathieu and Sénéchal wrote a book called Conformal field theory \cite{fms97}, which dealt almost exclusively with \textit{two-dimensional} conformal field theory. Nowadays, fortunately, a book with that title could say a lot on CFT in higher dimensions. However, when it comes to CFTs that we can solve exactly, we are back to two dimensions, because of two main reasons:
\begin{itemize}
 \item In two dimensions, the algebra of conformal transformations is infinite-dimensional. 
 \item In some two-dimensional CFTs, there exist degenerate fields: fields whose OPEs with other fields have finitely many terms, modulo conformal symmetry. 
\end{itemize}
...
Even in two dimensions, solvable CFTs are very special cases. Starting from a simple CFTs and following a renormalization group flow, you can easily end up at a fixed point whose CFT is completely intractable. We will deal first of all with CFTs that are considered solved, in that we exactly know the three-point structure constants, and can compute any correlation function to arbitrary precision:
\begin{itemize}
 \item Liouville theory,
 \item (generalized) minimal models, and limits thereof. 
\end{itemize}
We will also propose a unified treatment of loop models, which are not solved but are believed to be solvable, because we have sufficiently many exact results. By loop models we mean a large class of CFTs, with various global symmetry groups, which describe critical limits of various statistical models in various phases, and have various applications: percolation, polymers, and more recently active hydraulics \cite{jcpb23}. 
Solvable CFTs have fewer applications to gravity: in particular, any CFT dual to three-dimensional gravity should exist in the large central charge limit, which corresponds to the classical limit of gravity; but there are strong technical reasons why non-trivial solvable CFTs must have small central charges. (See Section \ref{sec:dotc}.) 

There are other solved or solvable CFTs that we will not mention, because they have extended chiral algebras, which means extra local symmetries in addition to conformal symmetry. Some CFTs of that type can be solved with the methods that we will explain, for example certain Wess--Zumino--Witten models. 
...
We are not claiming that our approach is always mathematically rigorous and conceptually transparent: our main concern is to provide simple derivations of non-trivial results, without introducing too many auxiliary objects that are later discarded. The calculations are not all done in detail but they should be doable with reasonable effort. Readers who want formal exercises are directed to earlier texts in the same spirit: the minimal review \cite{rib16}, and the more extended review \cite{rib14}. However, these texts do not cover loop models.


\subsection*{Acknowledgements}

Course organizers. Collaborators. Jesper Jacobsen.
...
\end{align}
Let us define the \textbf{Witt algebra} as the Lie algebra of infinitesimal conformal transformations on $\mathbb{C}^*= \mathbb{C}\backslash \{0\}$. This algebra describes local conformal symmetry for quantum field theory on the Riemann sphere, as can be argued in two alternative ways:
\begin{itemize}
...
\end{align}
If the basis of fields is such that $B_{ij}=\delta_{ij}B_i$, the sum over $k$ reduces to one term, and the relation reduces to
\begin{align}
 \boxed{C_{123} = C_{12}^3B_{3}} \ .
 \label{ccb}
\end{align}
This is the case in particular if \textbf{field multiplicities} are trivial. i.e. if two different primary fields never share the same conformal dimensions.
...
Let us consider the spectrum of a generalized minimal model with $\beta^2\in\mathbb{R}_{>0}\backslash\mathbb{Q}$. The momentums $P_{(r,s)}$ \eqref{prs} are dense in the real line, i.e. $\overline{\left\{ P_{(r,s)}\right\}_{r,s\in\mathbb{N}^*}} = \mathbb{R}$. Any diagonal field with a real momentum can therefore be obtained as a limit of degenerate fields,
\begin{align}
...
Since non-diagonal fields have $s\in \mathbb{Z}+\frac12$, the conservation of $s\bmod \mathbb{Z}$ \eqref{ssiz} (with $s=0$ for diagonal fields) reduces to the \textbf{conservation of diagonality}:
\begin{align}
D\times D = D \quad , \quad D\times N = N \quad , \quad N\times N = D \ ,
\label{ddd}
\end{align}
where $D$ stands for diagonal and $N$ for non-diagonal fields. 
Moreover, we assume that any representation that appears in the diagonal sector can be obtained by fusing two representations from the non-diagonal sector. By the degenerate fusion rules \eqref{rrsr}, the diagonal sector involves representations $\mathcal{R}^f_{\langle r,s\rangle}$ with $r\in 2\mathbb{N}+1$, and the spectrum is 
\begin{align}
...
\end{itemize}
The basic expectation is that in any OPE, any field that is not excluded by these constraints appears. This will turn out to be true for non-diagonal fields. For diagonal fields, the situation is more subtle, as we will see in Section \ref{sec:cloop}.
...
On the other hand, the Potts spectrum has no simple explanation in our formalism. We can only remark that it is obtained from the $U(n)$ spectrum by removing the non-diagonal fields $\left\{V_{(1,s)}\right\}_{s\in\mathbb{Z}}$ and adding the diagonal fields $\left\{ V_{P_{(0,s)}}\right\}_{s\in \mathbb{N}+\frac12}$. 

Each one of these models has
\begin{itemize}
 \item a modular-invariant torus partition function,
 \item a group of global symmetries that commutes with conformal symmetry: the orthogonal group $O(n)$, the unitary group $U(n)$, or the symmetric group $S_Q$ in the case of the Potts model. The parameters of these groups can take arbitrary complex values \cite{gnjrs21, br19}
 \begin{align}
  n = -2\cos\left(\pi \beta^2\right) \quad , \quad Q = 4\cos^2\left(\pi \beta^2\right)\ .
  \label{nQ}
 \end{align}
\end{itemize}
Degenerate fields are invariant under global symmetries, while other fields can transform in non-trivial representations \cite{jrs22}. For example, in the $O(n)$ model, the fields $V_{(\frac12,0)},V_{(1,0)}$ and $V_{(1,1)}$ transform under $O(n)$ as a vector, a symmetric 2-tensor and an antisymmetric 2-tensor respectively. There are two fields of the type $V_{(\frac32,0)}$, which transform as symmetric and antisymmetric 3-tensors. And for $r\geq \frac52$, it also happens that several different fields have the same conformal dimensions and the same behaviour under $O(n)$. The situation is similar in the $U(n)$ and Potts models: let us only mention that the diagonal non-degenerate fields $V_{P_{(0,s)}}$ of the Potts model transform in the standard  representation of $S_Q$, whose dimension is $Q-1$. 
...
In the loop models' extended spectrum $\widetilde{S}^\text{loop}$ \eqref{sloop}, it is clear that shift equations do not determine the structure constants:
\begin{itemize}
...
\begin{align}
 ...
 \label{ncb}
\end{align}
(This is the same $n$ as in the $O(n)$ model \eqref{nQ}.) In Section \ref{sec:cloop} we will formulate this conjecture in more detail. 

Even if the conjecture only applies to loop models, it is interesting to compute the normalized 3-point structure constant associated to $C_{P_1,(r_2,s_2),(r_3,s_3)}$ \eqref{cdnn} from (generalized) D-series minimal models:
\begin{align}
...
\begin{center}
 \begin{tikzpicture}[baseline=(base), scale = .5]
% \coordinate (base) at (0, -4);
\node [right] at (7.5, -15) {$\bullet$ Potts connectivities \cite{nr20}, loop models \cite{rib22, nrj23}};
...
\node [right] at (4.5, -8) {Yes};
 ...
 \node [right] at (11, -7) {$\bullet$ $O(n)$ model \cite{gnjrs21}, Potts model \cite{niv22}, loop models \cite{gjnrs23}};
...
in practice it may be enough to study a few 4-point functions for guessing a given multiplicity. This has been done in a few examples of diagonal 4-point functions in loop models \cite{rib22}.
...
We will now take a short break from CFT, and build correlation functions as statistical sums over loop ensembles. This will motivate the introduction of combinatorial maps, which give rise to a basis of solutions of crossing symmetry. We can then forget about the statistical sums, which are not fundamental or universal quantities, while keeping the combinatorial maps, which are probably intrinsic features of the CFT.

\subsection{Statistical sums over loop ensembles}

\subsubsection{Sums over configurations of non-intersecting loops}

Let us consider an ensemble $\mathcal{E}$ of non-intersecting closed loops on a Riemann surface $\Sigma$. 
There are several possible constructions of such ensembles that give rise to a conformal field theory in some limit:
\begin{itemize}
 \item We could discretize $\Sigma$, and draw the loops on the resulting graph. The resulting ensemble $\mathcal{E}$ is then finite. Conformal symmetry emerges in the limit where the graph size tends to infinity. 
 \item We could use a conformal loop ensemble, 
\end{itemize}




Loop ensembles: lattice or probabilistic, we are agnostic. Both approaches have advantages: lattice can get finite sums, nice algebras, some exact results at finite $L$. Probabilitic approach has manifest conformal symmetry. 

Non-intersecting loops with certain weights. In partition function, all loops have same weight by conformal symmetry. 


\subsubsection{Open loops and combinatorial maps}



Then introduce fields, diagonal or not. Weight factors as allowed by conformal symmetry. Combinatorial maps. 

\subsubsection{Cases of the $O(n)$, $U(n)$ and Potts models}

Case of the $O(n)$ model (just restrict possible diagonal fields), $U(n)$ model (just assume even numbers of loops, 2 diagonal fields), Potts model (focus on clusters, unnatural constraints). 


\subsection{Conformal field theory interpretation}


Correl fct of fields with integer spins, not deg.

\subsubsection{Parameters / Fields}

Find relation of parameters. Historic arguments give correct relations but are complicated and not very convincing. See for example Appendix A of \cite{grz18}, which debate an old argument by Kondev based on a map to free bosonic fields, and try to patch it with the help of the torus partition function. 

Our argument is to first identify the parameters of $V_{(r,s)}$ with Kac indices, then deduce the relation between loop weights and momentums for diagonal fields, and finally get the central charge. 

Focus on $V_{(r,s)}$ as defined on the lattice. Under a rotation $\delta\theta$ of the reference direction, the weight of this operator takes a factor $e^{-irs\delta\theta}$. Compare with the behaviour of a primary field \eqref{zgc}: for $\left[\begin{smallmatrix} a & b \\ c & d \end{smallmatrix}\right] = \left[\begin{smallmatrix} e^{\frac{i}{2}\delta\theta} & 0 \\ 0 & e^{-\frac{i}{2}\delta\theta} \end{smallmatrix}\right]\in SL_2(\mathbb{C})$, a field of spin $S$ picks a factor $e^{-iS\delta\theta}$. (Say it there??) Therefore, $V_{(r,s)}$ has conformal spin $rs$. 

Next we assume it is primary. \textcolor{red}{It would be good to prove it.} Argument about not generating fields with lower dimension by conformal transformations? Show that local conformal dimension on some domain only produces geometrical prefactor in correlation functions? But this prefactor depends on left and right dimensions, how could we determine them? 

Now $V_{(r,s)}$ is parametrized by $r\in \frac12 \mathbb{N}^*$, which is conserved modulo integers (cf connecting half-segments), and has spin $rs$. We deduce that it is a non-diagonal field with Kac indices $r,s$ in a CFT where $V^d_{\langle 1,2\rangle}$ exists. 

Next we consider a field that gives a weight $w$ to loops around it. We also assume that it is primary. Since $w$ is a continuous parameter, this must be a diagonal field: we want to determine the relation between $w$ and $P$. 
From the case of $V_{(r,s)}$ we know that under a rotation by $\delta\theta$, an individual loop strand picks a factor $e^{\frac{i}{2} s\delta\theta}$. For a closed loop, i.e. $\delta\theta=2\pi$, we therefore find $e^{i\pi s}$. But we have to add the two possible orientations, so the weight is $w=2\cos(\pi s)$. Accepting the identification $V_P = V_{(0, 2 \beta P)}$ from CFT, we deduce $w(P)$. 

It remains to determine the parameter $n$, which is the weight of contractible loops. This is simply $n=2\cos(2\pi P_{(1,1)})$. 

\subsubsection{Correlation functions}

Weights of closed loops, and channel fields. 

Loop weight invariant under shifts, but CFT not invariant, discussion of criticality and values of the central charge and loop weights. 

Given some fields, space of bootstrap solutions and combinatorial maps. Combinatorial defects interpretation: preserves decomposition into blocks, explains why no OPE in usual sense. 

\subsubsection{Specific loop models}
