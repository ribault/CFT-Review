...
\subsubsection{Structure constants and their invariant combinations}
...
\label{cs}
\end{align}
Notice that the OPEs $V^d_{\langle 2,1\rangle}V_3$ and $V_3V^d_{\langle 2,1\rangle}$ are related by an odd permutation, under which the OPE structure constant picks a factor $(-)^{S_3+S_{3^\pm}} = (-)^{s_3}$, according to Eq. \eqref{b21}. This explains the sign prefactor in $d^{(t)}_\pm$.
...
normalization, reflection invariance, reflection coef, analyticity

norm inv. implies reflection inv.

combine two shifts when needed. 

careful study in Liouville theory \cite{rib14}

More practical: solve for dependence on 3 fields, the rest follows. 
