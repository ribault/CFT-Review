...
In order to deduce equations for the structure constants $B_i,C_{ijk}$, it may seem that we need to know the degenerate OPE structure constants $c^\pm_i$. This is not a big problem, because in the bootstrap approach, we are free to perform field renormalizations using arbitrary constant coefficients $\lambda_i$,
\begin{align}
...
It is however not practical to only work with renormalization-invariant combinations of structure constants. In particular, the invariant $\frac{C_{123}}{\sqrt{B_1B_2B_3}}$ involves a square root, and suffers from the corresponding sign ambiguity. Instead, we will use field renormalizations to choose $c_i^+$, from which $c_i^-$ is then deduced using Eq. \eqref{cbcb}. While the choice of $c_i^+$ is in principle arbitrary, in practice it is guided by the simplicity of the resulting equations for $B_i,C_{ijk}$. In a CFT with two degenerate fields $V^d_{\langle 2,1\rangle}$ and $V^d_{\langle 1,2\rangle}$, the degenerate OPE coefficients for the two fields can however not be chosen independently. (See \cite[Exercise 3.2]{rib14} for the case of Liouville theory.)  

Having chosen $c^+_i$, it can be convenient to determine $c^-_i$ explicitly, instead of eliminating it in favour of $B_i$. To do this, notice that Eq. \eqref{cbcb} implies $\frac{B_{i^-}}{B_{i^+}} = \frac{c_i^+}{c_i^-}\frac{c_{i^-}^+}{c_{i^+}^-}$. In the case of the 4-point function $\left<V_{\langle 2,1\rangle}^d V_iV_{\langle 2,1\rangle}^d V_i\right>$, Eq. \eqref{cs} gives us $\frac{d^{(s)}_-}{d^{(s)}_+}=\frac{(c_i^-)^2}{(c_i^+)^2} \frac{B_{i^-}}{B_{i^+}}$. Eliminating the 2-point structure constants, we obtain the relation
\begin{align}
 \frac{d^{(s)}_-}{d^{(s)}_+}=\frac{c_i^-}{c_{i^+}^-} \frac{c_{i^-}^+}{c_i^+}\ ,  
 \label{ddcccc}
\end{align}
whose left-hand side is explicitly known by Eq. \eqref{ddb}. If we know $c_i^+$, this relation constrains $c_i^-$, without determining it. But any $c_i^-$ is OK due to leftover field normalization?? no, leftover normalization is periodic, would not change $c_i^-$ at all. 



\subsubsection{Choice of normalization and shift equations}

We choose the degenerate OPE coefficients 
\begin{align}
 c^+ = \beta^{2\beta(P+\bar P)} \frac{\Gamma(-2\beta P)}{\Gamma(1+2\beta \bar P)} \quad , \quad c^- = \beta^{-2\beta(P+\bar P)} \frac{\Gamma(2\beta P)}{\Gamma(1-2\beta \bar P)}\  . 
 \label{cpcm}
\end{align}
While the choice of $c^+$ is arbitrary, 


Give generic coefs $c_\pm$, some modifs in some cases.
