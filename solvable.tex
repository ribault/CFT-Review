...
Combining the crossing symmetry equation \eqref{dmdp}, \eqref{dpdp} and \eqref{ddb} with the determination \eqref{cs} of degenerate 4pt structure constants and the choice \eqref{cpcm} of degenerate OPE structure constants, we obtain the shift equations 
\begin{align}
...
\beta^{8\beta (P_1+\bar P_1)} 
 ...
 \label{sh-b}
\end{align}
These are called \textbf{shift equations}, because they determine how structure constants behave when the momentums $P_i,\bar P_i$ of fields are shifted by $\pm \frac{\beta}{2}$. The second equation makes sense provided $s_i\in \mathbb{Z}$, whereas the other equations are valid under the more general condition $s_i\in \frac12 \mathbb{Z}$ \eqref{ssiz}. With our choice of degenerate OPE structure constants, the shift equations do not involve Gamma functions that depend on only one of the momentums. 
...
\begin{align}
 ...
 \label{bc}
\end{align}
These expressions indeed obey the shift equations \eqref{sh-mp}-\eqref{sh-b} with $s_i=0$ and $P_i=\bar P_i$. And they are invariant under $\beta \to \beta^{-1}$, so they also obey the shift equations from $V^d_{\langle 1,2\rangle}$. 
...
The expressions \eqref{bc} for 2-point and 3-point structure constants are also valid in generalized minimal models, and in A-series minimal models. 


AMM: source of confusion that fusion rules not automatic. 
