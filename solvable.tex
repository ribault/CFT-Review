...
It is sometimes useful to consider representations that are not highest-weight. This is in particular the case of \textbf{logarithmic representations}\index{logarithmic representation}\index{representation!logarithmic---}, which we define as indecomposable representations where $L_0$ is bounded from below but not diagonalizable. A logarithmic representation is always reducible, because the highest-weight representation generated by the $L_0$-eigenvector with the smallest eigenvalue is a nontrivial subrepresentation. A simple way to build a logarithmic representation is to generate it from the formal $\Delta$-derivative of a primary state $V_\Delta$, which obeys in particular $L_0 V'_\Delta = \Delta V'_\Delta + V_\Delta$.
...
\end{align}
\end{subequations}
Therefore, the conditions $L_1X{}=L_2 X{}=0$ for $X{}$ to be a singular vector boil down to a system of 2 linear equations for the coefficients $(\lambda,\mu)$. The vanishing of that system's determinant determines the conformal dimension $\Delta$ as a function of the central charge $c$,
\begin{align}
...
\label{ars}
\end{align}
In these cases as in the general case, the dimensions $\Delta_{(r,s)}$ and singular vectors $L_{\langle r,s\rangle}$ have simple behaviours under the transformations $\beta\to -\beta$ and $\beta\to \beta^{-1}$, which leave $c$ \eqref{cb} invariant:
\begin{align}
...
When a representation $\mathcal{R}$ has a singular vector $X{}=LV{}$, this allows us to define a quotient representation $\frac{\mathcal{R}}{\mathcal{R}_{X{}}}$ by setting the singular vector to zero.
...
\end{itemize}
What about OPEs that involve $V_0$ or $V^d_{\langle 1,1\rangle}$? The OPE $V_0V_{\Delta_2}$ is just a special case of the OPE \eqref{prope}, where arbitrary primary fields may appear.
...
Technical assumptions are needed for constructing the fusion product, and deriving its properties. We will use the fusion product in cases where it is commutative and associative, with multiplicities $m_{i,j}^k\in \{0,1\}$ that are  invariant under permutations of the 3 indices. These cases involve either degenerate representations and Verma modules at generic $\beta^2\in \mathbb{C}\backslash\mathbb{Q}$ (Section \ref{sec:fpdr}), or irreducible Kac table representations at $\beta^2 \in \mathbb{Q}_{>0}$ (Section \ref{sec:amm}). The construction and properties of the fusion product are well-established in the second case \cite{gab99}, from which the first case can be obtained by taking limits and doing analytic continuation in $\beta^2$. However, to actually compute fusion products, it will be easier to start with the first case, and deduce the second.
