...
\section{Basic objects of two-dimensional CFT}
...
\begin{align}
  \boxed{L_0 |v\rangle = \Delta |v\rangle \quad , \quad L_{n>0} |v\rangle = 0}\ .
 \end{align}
Any nonzero state that obeys these conditions is called a \textbf{primary state}. Let us introduce a basis of \textbf{creation operators},
\begin{align}
...
A primary state $|v\rangle \in \mathcal{R}$ generates a subrepresentation $\mathcal{R}_{|v\rangle}= \text{Span}(L|v\rangle)_{L\in\mathcal{L}}\subset\mathcal{R}$, which is the space of its descendant states. If the states $(L|v\rangle)_{L\in\mathcal{L}}$ are linearly independent, then $\mathcal{R}_{|v\rangle}$ is called the \textbf{Verma module} $\mathcal{V}_\Delta$. More generally, $\mathcal{R}_{|v\rangle}$ is a quotient of $\mathcal{V}_\Delta$ by a subrepresentation (trivial or not): such quotients are called \textbf{highest-weight representations}. In such a representation, $L_0$ is diagonalizable. 
...
Let us look for null vectors in the Verma module $\mathcal{V}_\Delta$. This can be done level by level, starting with level $1$. That level is generated by a single state $L_{-1}|v\rangle$. To determine whether that state is primary, let us determine how it behaves under the action of $L_{n\geq 1}$:
\begin{align}
...
\begin{align}
 ...
 \label{drms}
\end{align}
We will provide a derivation of $P_{(r, s)}$ using operator product expansions in Section \ref{sec:dope}. On the other hand, the expressions of the null vectors $|\chi_{\langle r,s\rangle}\rangle$ are complicated and not particularly useful. 
...
Whenever $r_1,s_1>0$, the Verma module $\mathcal{V}_{\Delta_{(r,s)}}$ has a null vector $|\chi_{\langle r_1,s_1\rangle}\rangle= L_{\langle r_1,s_1\rangle} |v\rangle$. 
Moreover, if we apply the identity \eqref{rpsq} to the dimension $\Delta_{(r_1,-s_1)}$ \eqref{drms} of that null vector, we find that it is itself of the type $\Delta_{\langle r_2,s_2\rangle}$. If $r_2,s_2>0$, this leads to another null vector $L_{\langle r_2,s_2\rangle}L_{\langle r_1,s_1\rangle} |v\rangle$. It turns out that all null vectors of Verma modules are of the type $L_{\langle r_1,s_1\rangle} |v\rangle$ or $L_{\langle r_2,s_2\rangle}L_{\langle r_1,s_1\rangle} |v\rangle$. In particular, $L_{\langle r_3,s_3\rangle}L_{\langle r_2,s_2\rangle}L_{\langle r_1,s_1\rangle} |v\rangle$ is of the type $L_{\langle r_4,s_4\rangle} |v\rangle$. To see this, let $\epsilon,\nu\in\{+,-\}$ be the signs such that $P_{(-r_1,s_1)}=\epsilon P_{(r_2,s_2)}$ and $P_{(r_2,-s_2)}=\eta P_{(r_3,s_3)}$, and let $r_4=|\epsilon r_1-r_2+\eta r_3|$ and $s_4=|\epsilon s_1+s_2+\eta s_3|$, then $r_4s_4=\sum_{i=1}^3 r_is_i$, which means that the null vector $L_{\langle r_4,s_4\rangle} |v\rangle$ has the right level. 
...
\begin{align}
 \mathcal{R}_{\langle r,s\rangle} = \frac{\mathcal{V}_{\Delta_{\langle r,s\rangle}}}{\mathcal{V}_{\Delta_{\langle r_1,-s_1\rangle}}+ \mathcal{V}_{\Delta_{\langle r_2,-s_2\rangle}}}\ .
\end{align}
...
\begin{enumerate}
 \item We know the \textbf{CFT data}: the spectrm $\mathcal{S}$ and the 2-point and 3-point structure constants $B,C$. This determines the OPEs, and crossing symmetry of arbitrary 4-point functions is equivalent to the associativity of the OPE, and therefore to the consistency of the CFT on the sphere. In this scenario, we have to check that the crossing symmetry equations are obeyed, not to solve them.
...
\subsection{Spectrum and symmetries}

Use deg. fields rather than torus partition fct. We lose Verlinde formula for fusion rules but we get them otherwise. Our method is more elementary and flexible, can extend CFTs. Degeneracy of fields can be hard to deduce from partition fct. 
...
\subsubsection{How many degenerate fields?}

If only one: depends on $\beta^2$, not $c$.
...
Study monodromies of 4-point function with $V_{\langle 1,2\rangle}$ (not single valued) leads to $r_1+r_2+r_3\in\mathbb{Z}$. Applies also to D-series MM, explains conservation of diagonality.


Maximal spectrum is $O(n)$. Cannot really argue for Potts, but does not matter much. Anyway, at this stage, we know nothing about global symmetry.
...
Ashkin--Teller does have higher symmetry, solved wrt Virasoro nevertheless. Not really 2 degenerate fields, but no fractional Kac indices either. 
