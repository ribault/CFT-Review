...
\newcommand{\myindex}[1]{\textbf{\boldmath #1}\index{#1}}
...
\begin{align}
 ...
 \label{zy}
\end{align}
Furthermore, Eq. \eqref{tinf} implies the vanishing of the coefficients of $y^{-1},y^{-2},y^{-3}$ in the Laurent expansion of $Z(y)$ near $y=\infty$, leading to the three \myindex{global Ward identitites} 
\begin{align}
...
\begin{align}
 ...
 \label{vdi}
\end{align}
The global Ward identities are valid not only for primary fields, but also for \myindex{global primary fields} (also called quasi-primary fields): $L_0$-eigenvectors that are annihilated by $L_1$ but not necessarily by $L_{n\geq 2}$. According to the OPE $T(y)T(z)$ \eqref{tt}, the energy-momentum tensor is a global primary field of dimension $\Delta=2$, and the resulting behaviour at infinity \eqref{vdi} is consistent with our assumption \eqref{tinf}. 
...
\begin{align}
 ...
 \label{epbe}
\end{align}
In fact we want the algebra of conformal transformations to act on a complex vector space: the space of complex-valued functions on $\overline{\mathbb{C}}$ for the Witt algebra, the space of states of the CFT for the Virasoro algebra. We should therefore complexify our symmetry algebra, and we obtain two copies of the Virasoro algebra: a \myindex{left-moving or holomorphic} Virasoro algebra $\mathfrak{V}$ with generators $L_n$, and a \myindex{right-moving or antiholomorphic} Virasoro algebra $\bar{\mathfrak{V}}$ with generators $\bar{L}_n$. In the case of the algebra of global conformal transformations, we start with the 6-dimensional real Lie algebra $\mathfrak{sl}_2(\mathbb{C})$, with generators \eqref{epbe} with $n=-1,0,1$, and the complexification yields $\mathfrak{sl}_2(\mathbb{C})^\mathbb{C} = \mathfrak{sl}_2(\mathbb{C})\times \mathfrak{sl}_2(\mathbb{C})$.
...
\end{tikzpicture} 
 ...
 \label{stu}
\end{align}
In order to write these decompositions in greater generality, let us define the $x$-channel \myindex{conformal blocks} as 
\begin{align}
...
\begin{align}
 ...
 \label{specs}
\end{align}
The \myindex{physical spectrum} is made of the fields whose correlation functions we are interested in. Our constraint on the physical spectrum is called \myindex{closure under OPEs}. This constraint means that if a field appears in an OPE (or a fortiori in a channel spectrum), then its correlation functions exist. 
We also introduce \myindex{extended spectrums} as larger spectrums that are also closed under OPEs. There are various reasons for considering extended spectrums:
\begin{itemize}
...
\end{align}
This is called a \myindex{Belavin--Polyakov--Zamolodchikov equation} or \myindex{BPZ equation}.  
...
\begin{align}
 ...
 \label{detf}
\end{align}
The degenerate fusing matrix is built from \myindex{Euler's Gamma function} $\Gamma(x)$: a meromorphic function of $x\in\mathbb{C}$ with simple poles for $x\in -\mathbb{N}$ and no zeros, which obeys $\Gamma(x+1)=x\Gamma(x)$ and $\Gamma(x)\Gamma(1-x)=\frac{\pi}{\sin(\pi x)}$. 
...
Let us introduce \myindex{degenerate OPE structure constants} $c_i^\pm$ such that 
\begin{align}
...
Since shift equations produce Gamma functions, their solutions should be written in terms of \myindex{Barnes' double Gamma function} $\Gamma_\beta$, which obeys
\begin{align}
...
\begin{align}
 ...
 \label{resf}
\end{align}
where the coefficient $R_{r,s}$ is called a \myindex{Virasoro block residue}, and we use the momentum $P$ rather than the conformal dimension $\Delta$. We will derive an expression of the type 
\begin{align}
...
\end{align}
where $n$ is the \myindex{loop weight}. 
...
\begin{align}
 ...
 \label{sxs}
\end{align}
where we define the \myindex{$x$-channel signature} $\sigma^{(x)}\in\frac12\mathbb{N}$ as half the minimum number of lines in $M$ that are crossed by an $x$-loop. For example, the map 
$
...
In fact, in the critical limit, zeros of the partition function condense not only on criticality lines, but also inside critical regions, on lines where the ground state changes. The \myindex{ground state}, i.e. the state with the lowest dimension (in real part), can be $V^d_{\langle 1,1\rangle}, V_{(1, 0)}, V_{(2,0)}$ or $V_{P_{(0,\frac12)}}$, depending on $\beta^2$ and on the model. On a line where the two lowest states have the same dimension, the free energy is singular. 
