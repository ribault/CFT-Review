...
\end{align}
Now this differs from Eq. \eqref{sh-mp} with $\bar P_1=P_1$ by an overall sign. This sign is not fatal to our ansatz, which we now save by flipping one of the degenerate OPE structure constants for $V^d_{\langle 2,1\rangle}V_{P_i}$, i.e. we do $c^-_i \to -c^-_i$ in Eq. \eqref{cpcm}. This corrects ths sign of Eq. \eqref{sh-mp}, but leaves the other shift equations unchanged. (Eq. \eqref{sh-bb} would be changed, but it does not make sense in a D-series minimal model.) This leads to sign prefactors in the diagonal 3-point structure constant for D-series minimal models:
\begin{align}
...
Let us nevertheless look for simple solutions of the shift equations. As an ansatz, we introduce the \textbf{reference structure constants} $B^\text{ref}$ and $C^\text{ref}$, 

write them, 2-point a bit tricky

3-point: diag. special case $r=0$. 2-point not really. 

solve shift eqn modulo signs: normalized structure constants

Arbitrary function of the \textbf{loop weight} 
\begin{align}
 \boxed{w = 2\cos(2\pi\beta P)}\ .
\end{align}

Conjecture: polynoms (modulo subracting poles). 

Properties: permutation etc.

Compare reference struct. cst to A-series, D-series (quickly done!).
