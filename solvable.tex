...
\right.
\end{align}
where the diagonal fields take all possible momentums that correspond to the loop weight $w_s=2\cos(2\pi \beta P_s)$ \eqref{wP}. 

space of solutions to be defined more precisely incl. channel spectrums. 

The conjecture is that each combinatorial map corresponds to a solution, and that the resulting solution form a basis of 

All independent. Predict dimension of space of solutions. 

Remaining puzzles: interpret extra parameters (not one to one), relate individual maps. 
