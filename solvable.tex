...
\begin{tikzpicture}[baseline=(base), scale = .25]
 ...
 \end{tikzpicture}
 $
has signature $(\sigma_s,\sigma_t,\sigma_u)=(0,2,2)$. The conjectured criterion is that the corresponding solution obeys $r<\sigma_x\implies D^{(x)}_{(r,s)}=0$. In some cases, including our example, this uniquely determines the solution.
...
\begin{tikzpicture}[baseline=(base), scale = .25]
 ...
 \end{tikzpicture}
 $.
 In the $O(n)$ model, all diagonal fields are degenerate, and their fusion rules would forbid this solution, since $V^d_{\langle 1,s\rangle}\notin V_{(\frac32,s_1)}V_{(\frac12,s_2)}$. In the Potts model on the other hand, the spectrum includes degenerate fields of weights $n,-n$ and diagonal fields of weight $0$. In some 4-point functions, diagonal fields with different weights can propagate in the same channel, leading to more solutions than maps. 
...
then map to $n$-plane more messy. From dilute go to another $n$-plane and meet dense at marginality line. 

not discuss ground state but could be $V_{(2,0)}, V^d_{\langle 1,1\rangle}, V_{P_{(0,\frac12)}}$.
...
Case of momentum: region not known...
