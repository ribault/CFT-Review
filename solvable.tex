...
\subsubsection{Loops and vertices}
...
\begin{align}
 ...
 \end{tikzpicture}
\end{align}
The Potts model is the same as the $U(n)$ model, except that the fundamental geometrical objects are clusters instead of loops, with loops forming the boundaries of clusters. In general, loops and clusters are equivalent: in the above example, the puncture of valency 4 is the point where two different clusters meet. We have banned open loops that can be pulled inside punctures, and such loops would indeed lead to a miscounting of clusters, as in the following configuration where we have a squeezed cluster rather than the meeting of two different clusters:
\begin{align}
 \begin{tikzpicture}[baseline=(current  bounding  box.center), scale = .4]
  \clip (-4, -4) rectangle (4, 4);
 ...
 \end{tikzpicture}
\end{align}
However, there is a subtle difference between loops and clusters in the case of a puncture of valency 2. In the Potts model, we would like this puncture to sit at the boundary of a cluster, which allows configurations such as the following: 
\begin{align}
 \begin{tikzpicture}[baseline=(current  bounding  box.center), scale = .4]
  \filldraw[thick, fill = red!30] (0, 0) to [out = -90, in = -80] (3, -.5) to [out = 100, in = 55] (0, 0);
   \draw (0, 0) node[fill, circle, minimum size = 2mm, inner sep = 0]{};
 \end{tikzpicture}
\end{align}
To ensure that our puncture is at the boundary of a cluster, the correct condition is no closed loop goes around the puncture. Therefore, the Potts model has punctures with valencies $2r\in 2(\mathbb{N}+2)$, and weight-changing punctures of weight $w=0$.
...
How do we see that a field is degenerate in the loop model? In the CFT deg. fields have special OPEs. But in the loop model their weight is a special value of a continuous parameter: what happens to the argument that single-valuedness excludes $V_{\langle 1,2\rangle}^d$ from the $O(n)$ model? 
