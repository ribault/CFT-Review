...
\begin{align}
 \underset{P=P_{(r,s)}}{\operatorname{Res}} \mathcal{F}_P(z) = R_{r,s}\mathcal{F}_{P_{(r,-s)}}(z)\ ,
 \label{resf}
\end{align}
where the coefficient $R_{r,s}$ is called a \textbf{Virasoro block residue}, and we use the momentum $P$ rather than the conformal dimension $\Delta$. 

By definition, the Virasoro block residue is a combination of correlation functions of descendant states, 
\begin{align}
 R_{r,s} = \frac{g^{L_{\langle r,s\rangle}}_{P_{(r,s)},P_1,P_2}\,
 g^{L_{\langle r,s\rangle}}_{P_{(r,s)},P_3,P_4}}{ \left.\frac{\partial}{\partial P} \frac{\left<L_{\langle r,s\rangle} V_P L_{\langle r,s\rangle} V_P \right>}{\left<V_PV_P\right>}\right|_{P=P_{(r,s)}}}\equiv \frac{c^{r,s}(P_1,P_2)c^{r,s}(P_3,P_4)}{b^{r,s}}\ , 
\end{align}
where $g^L$ was defined in Eq. \eqref{gl}, and we recall from Section \ref{sec:nvvm} that $L_{\langle r,s\rangle}$ is the creation operator such that $V_{P_{(r,-s)}}\propto L_{\langle r,s\rangle}V_{P_{(r,s)}}$ is the null vector of the reducible Verma module $\mathcal{V}_{P_{(r,s)}}$. Now, since $\Delta_{(1,1)}=0$, $c^{r,s}(P_{(1,1)},P)$ is in fact a ratio of 2-point functions, which can be used for writing 
\begin{align}
 b^{r,s} = c^{r,s}\left(P_{(1,1)}, P_{(r,-s)}\right)  \left.\frac{\partial}{\partial P} c^{r,s}\left(P,P_{(1,1)}\right)\right|_{P=P_{(r,s)}}\ . 
\end{align}
This reduces the problem to determining $c^{r,s}$. To do this, let us find how $c^{r,s}$ behaves under $r\to r+1$ from the fusion transformation \eqref{fsfft} of degenerate 4-point conformal blocks. The idea is to compare the fusion transformations for 4-point functions of the type $\left<V^d_{\langle 2,1\rangle}V_{P_{(r,-s)}}V_{P_1}V_{P_2}\right>$ and $\left<V^d_{\langle 2,1\rangle}V_{P_{(r,s)}}V_{P_1}V_{P_2}\right>$. More precisely, let us compare the coefficients $F_{++}$ and $\widetilde{F}_{++}$ that relate the following conformal blocks:
\begin{align}
...
To solve the shift equation \eqref{cros}, and the analogous equation for $\frac{c^{r,s+1}}{c ^{r,s}}$, we first determine $c^{1,1}(P_1,P_2) =\prod_\pm (P_1\pm P_2) $ from Eq. Need to assemble block to first order right after Eq. \eqref{fsl}. Sign of shift equation?

Either double Gamma or big product. Write $c^{r,s}$ deduce $b^{r,s}$. 
