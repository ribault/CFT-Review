...
In statistical physics, \textbf{loop models}\index{loop model} are a class of 2d lattice models whose states can be described in terms of configurations of non-intersecting loops. (See \cite{jac16} for a review.) Starting from the Ising model, many statistical models can indeed be reformulated in terms of loops, or directly constructed from loops.
...
12- No change. Fields are not defined as operators on a space, they are defined as vectors in representations of the Virasoro algebra. So there is no ambiguity, except for readers who have other texts and other formalisms in mind. Referring to these other formalisms would risk confusing the other readers.

13- No change. $\mathcal{S}$ is just some set of fields. I do not want to give it a name, because it no longer appears later, and the name OPE spectrum is reserved for a set of primary fields.
...
25- These days, in this field, journal data are not needed for finding papers, and do not give much useful information in terms of quality or else. A journal can be a venue for improving a paper as we are doing right now, but this does not imply that journal data should be displayed to readers in the list of citations. This said, I can add the data if the editors require it.
...
2- For lattice loop models, I have added reference [35]. As suggested in the introduction, the Coulomb gas approach is only of historical interest, and irrelevant to current research. Results from that approach are rederived in a simpler way in this text.
...
4- Other approaches to Liouville theory are outside the scope. See the Wikipedia article on Liouville theory (which I mostly wrote) and references therein.
