...
\begin{align}
 \boxed{S_{(r,s)}= rs} \ . 
\end{align}
Using Kac indices as parameters simplifies OPEs with degenerate fields: according to the fusion rules \eqref{rvvp}, the only primary fields that can appear in the OPE $V^d_{\langle 2,1\rangle}V_{(r,s)}$ are 
\begin{align}
 V^d_{\langle 2,1\rangle}V_{(r,s)} \subset \sum_\pm V_{(r\pm 1,s)} + \sum_\pm V_{(r,s\pm \beta^2)}\ . 
\end{align}
However, assuming all fields have integer spins, the two terms on the right-hand side cannot coexist. Consider indeed the spin differences:
\begin{align}
 S_{(r\pm 1,s)} - S_{(r,s)} = \pm s \quad , \quad S_{(r,s\pm \beta^2)} - S_{(r,s)} = \pm r\beta^2
\end{align}
If $s\in\mathbb{Z}$ and $r\beta^2\in\mathbb{Z}$ then $rs\beta^2=S_{(r,s)}\beta^2\in\mathbb{Z}$, therefore $\beta^2\in\mathbb{Q}$. Keeping the central charge generic, 

then assume generic c results still hold at rational c. 




Closure under degenerate fusion implies that fields exist. Stronger that closure under OPE. We now require terms to appear in OPE. Problem: diagonal vs non-diagonal? can formulate it using $(r,s)$ parametrization.

end with table of possible fields  and their dimensions 
