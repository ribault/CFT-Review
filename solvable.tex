...
We start with a few general remarks, before considering specific CFTs.
...
\begin{align}
 ...
 \label{rcon}
\end{align}
The first Kac index must therefore be half-integer. Moreover, since $V^d_{\langle 1,2\rangle}$ is assumed to exist, we may always  consider the correlation function $\left<V^d_{\langle 1,2\rangle}\prod_{i=1}^NV_{(r_i,s_i)}\right>$, if only in an extended CFT. 
The monodromy of $V^d_{\langle 1,2\rangle}$  around $V_{(r_i,s_i)}$ is $e^{2\pi ir_i}\in \{1,-1\}$, as determined by the differences of spins \eqref{sdiff2}. Each one of these monodromies may be nontrivial, but their product must be $1$. Therefore, the first Kac index is conserved modulo integers,
\begin{align}
