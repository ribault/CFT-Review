...
\begin{align}
 ...
 \label{sh012}
\end{align}
The Shapovalov matrix at level $N$ is degenerate if and only if there is a null vector at a level $\leq N$, equivalently if and only if $\Delta\in \left\{\Delta_{(r,s)}\right\}_{rs\leq N}$. The inverse matrix has simple poles for $\Delta\in \left\{\Delta_{(r,s)}\right\}_{rs\leq N}$, and can be computed in terms of $\left\{L_{\langle r,s\rangle}\right\}_{rs\leq N}$ \cite{fqs24}.
