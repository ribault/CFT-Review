...
Let us describe the behaviour of the Virasoro block $\mathcal{F}_\Delta(z)$ as $\Delta\to \infty$, before explaining how this behaviour can be derived. We replace the cross-ratio $z$ with the \myindex{nome}\index{nome} $q$, a function of $z$ built from the hypergeometric function: 
\begin{align}
...
\end{align}
The Virasoro block may then be rewritten as a prefactor, times a power series $H_\Delta(q)$, which has a trivial asymptotic behaviour:
\begin{align}
