...
\begin{align}
\mathcal{V}_{P+i\beta +j\beta^{-1}}=\mathcal{V}_{P+i'\beta +j'\beta^{-1}}\implies i\beta +j\beta^{-1} = i'\beta +j'\beta^{-1} \implies 2(i-i')q = 2(j'-j)p\ . 
\end{align}
But parity forbids $(i,j)=(i',j')$, since $p$ or $q$ is odd. And we have $2|i-i'|<p$ and $2|j-j'|<q$, so our equality cannot be satisfied. Therefore,
\begin{align}
...
Derive fusion from MM fusion rules. (Does not work from GMM, it seems.) Then claim limit from Liouville.
