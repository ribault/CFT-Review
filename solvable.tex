...
\begin{itemize}
 \item If $\mathcal{R}=\mathcal{R}_{\langle r,s\rangle}$ is degenerate, the condition that $V_{\langle r,s\rangle}$ may appear in the OPE $V_{\Delta_1}(z_1)V_{\Delta_2}(z_2)$ leads to polynomial equations of order $rs$ on $\Delta_1,\Delta_2$, just as we saw in the case $(r,s)=(2,1)$. 
...
\end{align}
We write $V_{\Delta,\bar\Delta}(z)$ for field that is primary with respect to the conformal algebra, with the left dimension $\Delta$ and the right dimension $\bar\Delta$. The difference of these dimensions is called the \textbf{conformal spin}
\begin{align}
 S = \Delta -\bar\Delta\ . 
 \label{sdd} 
\end{align}
A \textbf{spinless field} $V_\Delta(z)=V_{\Delta,\Delta}(z)$ is a primary field whose spin vanishes. Spinless fields are often called diagonal, but in Section \ref{sec:sesc} we will define diagonal fields as a subclass of spinless fields. 
...
\begin{align}
\boxed{ \Big< V_{\Delta_1,\bar\Delta_1}(z_1)V_{\Delta_2,\bar\Delta_2}(z_2) \Big> = B_{12} \left|\delta_{\Delta_1,\Delta_2} z_{12}^{-2\Delta_1}\right|^2} \ ,
\label{2ptb}
\end{align}
...
\begin{align}
 \boxed{  \left< \prod_{i=1}^3 V_{\Delta_i,\bar\Delta_i}(z_i) \right> = C_{123} \left|z_{12}^{\Delta_3-\Delta_1-\Delta_2} z_{13}^{\Delta_2-\Delta_1-\Delta_3} z_{23}^{\Delta_1-\Delta_2-\Delta_3}\right|^2 }\ ,
 \label{3ptc}
\end{align}
where $C_{123}$ is the \textbf{3-point structure constant}. (We are labelling structure constants by indices rather than by conformal dimensions, because there could exist several fields with the same conformal dimensions, although this is not manifest in our notation $V_{\Delta,\bar\Delta}$.) Single-valuedness of the 2-point and 3-point functions implies that spins obey
\begin{align}
 S_i\in\frac12 \mathbb{Z} \quad , \quad S_1+S_2+S_3 \in\mathbb{Z}\ . 
\end{align}
Now we have assumed that fields commute \eqref{comm}, but the $z_i$-dependent factors of the 2-point and 3-point functions are not invariant under permutations. This leads to the non-trivial behaviour of structure constants,
\begin{align}
...
\subsubsection{Calculation of conformal blocks}
