...
When a representation $\mathcal{R}$ has a null vector $|\chi\rangle$, this allows us to define a quotient representation $\frac{\mathcal{R}}{\mathcal{R}_{|\chi\rangle}}$ by setting the null vector to zero. 
...
set NV to zero: by abuse of terminology we say that the rep. has a vanishing null vector. 

not fully deg reps. play more important role with extended sym. alg, we will say deg. for full deg. 
