...
Nevertheless, the existence of relevant fields in the CFT influences
the behaviour of the statistical models along renormalization group flows. A primary field is called \textbf{relevant} if $\Re(\Delta+\bar\Delta)<2$, \textbf{marginal} if $\Re(\Delta+\bar\Delta)=2$, or \textbf{irrelevant} if $\Re(\Delta+\bar\Delta)>2$. Typically, the critical limit exists only provided some fields are irrelevant. Since the 
dimensions of primary fields depend on the parameter $n$ or $Q$, there exist \textbf{critical regions} in the complex $n$-plane or $Q$-plane where a given model has a critical limit, and the boundary of these regions are \textbf{marginality lines}, where a field becomes marginal. Crossing a marginality line does not always spell doom for the critcial limit however: in particular, a relevant field may be prevented from derailing the flow by some symmetry. 
...
This is one reason why the same CFT can describe various phases of various statistical models. For example, the $O(n)$ model's dense phase is usually defined for $\beta^2\in (0,1)$, and the dilute phase for $\beta^2\in (1,2)$, with $\beta^2>2$ corresponding to multicritical phases. These phases can be extended to the complex $\beta^2$-plane, by taking marginality lines as phase boundaries. Let us now plot the marginality lines in the $n$-plane that corresponds to $0<\Re \beta^2<1$:
\newcommand{\margs}[2]{
...
\end{tikzpicture}
\end{align}
Numerical investigations of the zeros of partition functions suggest that the gray region within the criticality line $M_{(1,3)}$ is indeed the critical region of certain lattice loop models \cite{bjjz22}. 

\textcolor{red}{Ground state could be $V_{(2,0)}, V^d_{\langle 1,1\rangle}, V_{P_{(0,\frac12)}}$. Influence on critical limit?}
...
We consider a 4-point function of the type $\left<\prod_{i=1}^4 V_{(r_i,s_i)}\right>$ with $r_1+r_2\equiv 0\bmod 1$. The corresponding $s$-channel spectrum \eqref{ss} includes a series of diagonal fields with momentums in $P_s+\beta^{-1}\mathbb{Z}$. 

remember that fct of $w_s$ but technically simpler to study dependence on $P_s$. 

Decompose into blocks, normalize using ref. 4pt structure constant.

Assume full analyticity in $w_s$ and also in $P_s$, natural from loop model but two caveats: not quite clear why it does not apply to other weights, and it applies term by term in channel expansion.

Look at poles of blocks and structure constants. 

Focus on some block. Residues of double poles must cancel in diag. terms already. Get formula for block residues! There remain simple poles, can be cancelled against non-diag. blocks. Get formula for log. blocks! 
