...
In the bootstrap approach, which is axiomatic rather than constructive, fields need not be constructed from other objects. Rather, we use fields as a convenient notation for stating properties of correlation functions, which we will introduce in Section \ref{sec:cor}.
...
V^f_{\langle r,s\rangle} = V^d_{\langle r,s\rangle}\ .
 \label{mm-gmm}
\end{align}
In particular, the doubly degenerate primary field $V^f_{\langle r,s\rangle}$ has vanishing singular vectors at levels $rs$ and $(p-r)(q-s)$ by Eq. \eqref{prqs}. The second singular vector disappears as its level tends to infinity, and the field becomes simply degenerate.
