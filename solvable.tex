...
In the analytic bootstrap approach of Section \ref{sec:ab}, we have used the crossing symmetry of 4-point functions that include degenerate fields, and deduced constraints on 2-point and 3-point structure constants. These constraints uniquely determined the structure constants in CFTs with 2 degenerate fields but not in loop models. In all cases, the existence of consistent CFTs requires the crossing symmetry of arbitrary 4-point functions. In the absence of degenerate fields, crossing symmetry equations involve infinite sums and are hard to deal with analytically. We will thus need to solve them numerically. This will not prevent us from deducing exact results for structure constants. 

\subsection{Crossing symmetry of 4-point functions}

\subsubsection{Achievements and limitations of analytic approaches}

Liouville proofs: Teschner, proba.

MM proof: Dotsenko--Fateev integrals? 

Formal proofs based on F-matrix

Then take limits \eqref{lims} 

Case of our CFTs: proof in Liouville theory, minimal models. Take limits, deduce RWT, Liouville $c\leq 1$, GDMM. F-matrix \cite{ebe23}, relation to 3-point \cite{rib14, run99}. 

Various issues and limitations: on the central charge in particular. Direct numerical check easy...

Here: numerics. Some cases inaccessible to analytic methods. 
Formulas for blocks (or sums thereof) complicated.
Get numbers for correlation functions. 
