...
Exact solvability relies on local conformal symmetry, and on the existence of degenerate fields. We show how these assumptions constrain the spectrum and correlation functions. In particular, we review the Ward identities and Virasoro blocks that follow from local conformal symmetry. We formalize the existence of degenerate fields in terms of an interchiral algebra and interchiral blocks.  
...
We discuss how crossing symmetry equations can be solved analytically or numerically. ana then num? shift equations. infinite-dim linear system with finite-dim space of sol. 


This leads to analytic formulas for structure constants, involving the double Gamma function. 
...
\begin{align}
 ...
 \label{sdec}
\end{align}
where we introduce the $s$-\myindex{channel spectrum}\index{spectrum!channel---} $\mathcal{S}^{(s)}= \mathcal{S}^{12}\cap \mathcal{S}^{34}$, and the $s$-channel \myindex{Virasoro blocks}\index{block!Virasoro---}, which are known (in principle) functions of $z$ as well as the central charge and conformal dimensions:
\begin{align}
...
\end{tikzpicture}
 ...
 \label{stu}
\end{align}
The commutativity of the product of fields \eqref{comm} implies that 4-point function \eqref{4pt} are invariant under permutations of the 4 fields, and this determines how Virasoro blocks behave under permutations. In particular, $t$-channel and $u$-channel blocks can be deduced from $s$-channel blocks via the relations 
\begin{align}
...
\end{align}
As another example, let us compare the diagonal and non-diagonal minimal models AMM$_{8,5}$ and DMM$_{8,5}$, which both have $c=-\frac{7}{20}$. The Virasoro representations that appear in the spectrums of both models belong to the Kac table $K_{8,5}$. Each entry in the table correspond to a primary field of AMM$_{8,5}$. Since $p=8\equiv 0\bmod 4$, the diagonal sector of DMM$_{8,5}$ is built from representations $\mathcal{R}^f_{\langle r,s\rangle}$ with $r\in 2\mathbb{N}+!$ (boxed), while the non-diagonal sector is built from $\mathcal{R}^f_{\langle r,s\rangle}$ with $r\in 2\mathbb{N}$ (highlighted):
\begin{align}
...
\end{align}
Here $g^L$ was defined in Eq. \eqref{gl}, and we recall from Section \ref{sec:nvvm} that $L_{\langle r,s\rangle}$ is the creation operator such that $V_{\Delta_{(r,-s)}}\propto L_{\langle r,s\rangle}V_{\Delta_{(r,s)}}$ is the null vector of the reducible Verma module $\mathcal{V}_{\Delta_{(r,s)}}$. Now, since $\Delta_{(1,1)}=0$, $c^{r,s}(P_{(1,1)},P)$ is in fact a ratio of 2-point functions, which can be used for writing 
\begin{align}
...
Therefore, the derivation of the $\Delta\to\infty$ asymptotics of Virasoro blocks is surely easier on the pillow than on the sphere. When it comes to asymptotics, 4-point blocks on the pillow are very similar to 1-point blocks on the torus. 
Moreover, the infinite product in Eq. \eqref{fpillow} might naturally emerge from a calculation using the oscillator basis. More fundamentally, it would be interesting to understand why blocks simplify in the pillow geometry, and whether this is the unique geometry with this property. 
...
\subsection{Global symmetries in $d=2$ and $d>2$}
