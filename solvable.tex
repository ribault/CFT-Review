...
For example, the following map corresponds to a solution with one $s$-channel diagonal field:
$
\begin{tikzpicture}[baseline=(base), scale = .25]
 \vertices
  \draw (0, 0) -- (0, 3);
...
\begin{align}
 \frac{R_{r,s}\bar{R}_{r,s}}{4P_{(r,s)}^2} + \underset{P=P_{(r,-s)}}{\operatorname{Res}^{(2)}} D_P = 0 \ .
\end{align}
...
\end{align}
Finally, let us consider the simple pole of $\left|\mathcal{F}_{P}\right|^2$ at $P=P_{(r,s)}$ with $r,s\in \mathbb{N}^*$. The residue at this pole involves terms in $\mathcal{F}_{P_{(r,s)}}\bar{\mathcal{F}}_{P_{(r,-s)}}$ and $\mathcal{F}_{P_{(r,-s)}}\bar{\mathcal{F}}_{P_{(r,s)}}$. From $D_{P+s\beta^{-1}} \left|\mathcal{F}_{P+s\beta^{-1}}\right|^2$, we also have terms in $\left.\frac{\partial}{\partial P}\left|\mathcal{F}_{P}\right|^2\right|_{P=P_{(r,-s)}}$ and $\left|\mathcal{F}_{P_{(r,-s)}}\right|^2$. All these terms must cancel against $\underset{P=P_{(r,s)}}{\operatorname{Res}} \left( D_{(r,s)}(P)\mathcal{G}_{(r,s)} + D_{(r,-s)}(P)\mathcal{G}_{(r,-s)}\right)$. However, there is no canonical way to separate these terms into contributions to $\mathcal{G}_{(r,s)}$ and $\mathcal{G}_{(r,-s)}$. Rather, their sum is a logarithmic block, which sums up the contributions of all the states in one indecomposable representation that includes both primary fields $V_{(r,s)}$ and $V_{(r,-s)}$. By convention we call $\mathcal{G}_{(r,s)}$ this logarithmic block, and set $\mathcal{G}_{(r,-s)}=0$. Our holomorphicity assumption determines $\mathcal{G}_{(r,s)}$, and requires 
\begin{align}
