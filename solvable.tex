...
For example, let us set $L_{-1}V_0=0$ in $\mathcal{V}_0$. The resulting quotient representation has a basis made of states $LV_0$ such that the creation operator $L\in\mathcal{L}$ does not involve $L_{-1}$. Let us sketch this basis up to the level $N=4$, for comparison with a Verma module \eqref{verma}:
 ...
 \label{r11}
\end{align}
Similarly, if we set a level $N$ null vector to zero, the creation operators that do not involve $L_{-N}$ provide a basis of the resulting quotient representation.
...
In loop CFTs, the existence of non-diagonal fields together with $V^d_{\langle 1,2\rangle}$ leads to the half-plane constraint $\Re c<13$ \eqref{rbp} on the central charge. And the asymmetry between the 2 Kac indices $r,s$ means that loop CFTs really depend on $\beta^2$ \eqref{cb}, not on $c$. The known constraints on fusion rules are:
\begin{itemize}
...
\end{itemize}
We now add an assumption that has 2 different formulations:
\begin{enumerate}
...
We therefore define a \textbf{loop CFT}\index{loop CFT} as a non-diagonal CFT whose spectrum is a subset of the extended spectrum $\widehat{S}^\text{loop}$ \eqref{sloop}, and whose OPEs are discrete.
