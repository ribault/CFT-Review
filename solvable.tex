...
The monodromy of $V^d_{\langle 1,2\rangle}$  around $V_{(r_i,s_i)}$ is $e^{2\pi ir_i}\in \{1,-1\}$, as determined by the OPE \eqref{prope} and the differences of spins \eqref{sdiff2}. Each one of these monodromies may be nontrivial, but their product must be $1$. Therefore, the first Kac index is conserved modulo integers, and we find 
\begin{align}
...
\subsection{Diagonal CFTs with 2 degenerate fields}
...
\end{align}
This spectrum is manifestly closed under the degenerate fusion rules \eqref{rrsr}. The set of conformal dimensions is in general not bounded from below:
\begin{align}
...
\begin{align}
 ...
 \label{vplim}
\end{align}
We assume that the resulting fields are only characterized by their conformal dimensions. This implies that $V_P$ does not depend on how exactly we take the limit, and also that $V_P$ and $V_{-P}$ are equivalent, since they are related by reflection. The study of correlation functions in Section \eqref{sec:dfpf} will justify this assumption.
...
\begin{align}
 ...
 \label{idpv}
\end{align}
where the factor $\frac12$ is here because $V_P$ and $V_{-P}$ are equivalent. Now, remember from Section \ref{sec:csope} that OPE coefficients have poles when the momentum $P$ becomes degenerate, and these poles are dense in the real line. The integral in the OPE $V_{P_1}V_{P_2}$ is therefore formally divergent. We may fear that the integral is only a fancy notation for indicating that the OPE $V^d_{\langle r_1,s_1\rangle}V^d_{\langle r_2,s_2\rangle}$ has a finite limit that does not depend on how we send the indices to infinity. However, it turns out that the integral can be regularized without going back to degenerate fields, by simply shifting the integration line into the complex plane:
\begin{align}
...
\begin{align}
 ...
 \label{sliou}
\end{align}
The regularized OPE \eqref{ireg} requires that we extend the spectrum to $\frac12 \left\{ V_P\right\}_{P\in \mathbb{C}}$. This will turn out to be possible because correlation functions are analytic in $P$. 
...
We also assume that correlation functions are analytic in momentums and in $c\in\mathbb{C}-(-\infty,1]$, and that degenerate fields exist, so that we have the extended spectrum
\begin{align}
...
Let us start with the case $\beta^2=-\frac{q}{p}<0$ with $p,q\in\mathbb{N}^*$ coprime. The representation $\mathcal{R}^d_{\langle 2, 1\rangle}$ is simply degenerate. Repeatedly fusing it with itself, do we obtain finitely many representations? The answer is no, for two reasons: 
\begin{enumerate}
 \item We do not obtain fully degenerate representations.
 \item Even if we did, this would not prevent us from generating infinitely many representations.
\end{enumerate}
To see this, let us observe that $r=p+1$ is the lowest index such that the Verma module $\mathcal{V}_{P_{(r,1)}}$ has two null vectors, and gives rise to the doubly degenerate quotient
\begin{align}
 \mathcal{R}^f_{\langle p+1,1\rangle}=\mathcal{R}^f_{\langle 1,q+1\rangle}\ .
\end{align}
From Eq. \eqref{rrsvp}, the fusion product of this representation with a generic Verma module is 
\begin{align}
 \mathcal{R}^f_{\langle p+1,1\rangle} \times \mathcal{V}_P = \sum_{\pm} \mathcal{V}_{P\pm \frac12\sqrt{pq}}\ ,
\end{align}
where $\{P\pm \frac12\sqrt{pq}\}_\pm = \{P+i\beta\}_{i=-\frac{p}{2},\cdots, \frac{p}{2}} \cap \{P+j\beta\}_{j=-\frac{q}{2},\cdots, \frac{q}{2}}$. However, by associativity, the fusion product $\mathcal{R}^d_{\langle 2, 1\rangle}\times \mathcal{R}^d_{\langle p, 1\rangle}\times \mathcal{V}_P$ is a sum of $2p$ Verma modules, and cannot agree with $\left(\mathcal{R}^d_{\langle p-1, 1\rangle}+\mathcal{R}^f_{\langle p+1, 1\rangle}\right)\times \mathcal{V}_P$, which would be a sum of $p+1$ modules. In fact, using Eq. \eqref{drms}, we find 
\begin{align}
\Delta_{(p-1,1)}-\Delta_{(p+1,1)} = \Delta_{(-1,q+1)}-\Delta_{(1,q+1)}=q+1 \in \mathbb{N}\ ,
\end{align}
which suggests that the product $\mathcal{R}^d_{\langle 2, 1\rangle}\times \mathcal{R}^d_{\langle p, 1\rangle}$ is in fact a logarithmic representation, see Section \ref{sec:log}. What if we started with $\mathcal{R}^f_{\langle p+1,1\rangle}$? Fusing it with itself, we obtain $\mathcal{R}^f_{\langle p+1,1\rangle}\times \mathcal{R}^f_{\langle p+1,1\rangle} = \mathcal{R}^d_{\langle 1,1\rangle} + \mathcal{R}^f_{\langle p+1,q+1\rangle}$. 
% Rk: Identity rep is apparently absent from \mathcal{R}^f_{\langle p+1,1\rangle}\times \mathcal{R}^f_{\langle 1,q+1\rangle} if we trust generic c degenerate fusion rules, but in fact it is present! 
Again, the two resulting representations have dimensions that differ by an integer, so they might in fact be combined into a single logarithmic representation. Anyway, since $\mathcal{R}^f_{\langle p+1,q+1\rangle} = \mathcal{R}^f_{\langle 2p+1,1\rangle}$, we see that repeated fusion produces conformal dimensions of the type $\Delta_{(r,1)}$, with no bound on $r$. 
...
\begin{align}
 ...
 \label{kac}
\end{align}
This amounts to showing that $\mathcal{R}^f_{\langle r_1,s_1\rangle} \times \mathcal{R}^f_{\langle r_2,s_2\rangle}$ only involves representations with indices in the Kac table, and that these representations are fully degenerate. We will use the 
$\mathbb{Z}_2$ symmetry of the Kac table,
\begin{align}
 \mathcal{R}^f_{\langle r,s\rangle} = \mathcal{R}^f_{\langle p-r,q-s\rangle}\ , 
 \label{prqs}
\end{align}
which implies that $\mathcal{R}^f_{\langle r,s\rangle}$ has vanishing null vectors at levels $rs$ and $(p-r)(q-s)$. These vanishing null vectors imply the fusion rules \eqref{rrsr}
\begin{align}
...
We therefore define the A-series minimal model MM$_{p,q}$ by its spectrum, made of fully degenerate fields in the Kac table:
\begin{align}
 \boxed{\mathcal{S}^{\text{MM}_{p,q}} = \frac12\left\{ V^f_{\langle r,s\rangle} \right\}_{(r,s)\in [1, p-1] \times [1, q-1]} \quad \text{with} \quad 2\leq p<q\ \text{ coprime}}\ .
\end{align}
The factor $\frac12$ accounts for the $\mathbb{Z}_2$ symmetry \eqref{prqs}. We assume $p<q$ because MM$_{p, q}=$MM$_{q,p}$, and $2\leq p$ for the Kac table to be non-empty.


\subsubsection{Runkel--Watts-type CFTs}\label{sec:rwt}

Let us consider a limit of A-series minimal models where $\frac{q}{p}\to \beta_0^2>0$. The Kac table becomes infinite, and 
\begin{align}
 \lim_{\frac{q}{p}\to \beta_0^2} \mathcal{S}^{\text{MM}_{p,q}} = \mathcal{S}^\text{GMM}\ . 
\end{align}
This makes it possible to simultaneously send the momentums $P_{(r,s)}$ to arbitrary values $P\in\mathbb{R}$, just as in Eq. \eqref{vplim}. Therefore, Liouville theory with $c\leq 1$ is not only a limit of generalized minimal models, but also a limit of minimal models. However, if $\beta_0^2\in\mathbb{Q}$, it is also possible to fine-tune the behaviour of $p,q,r,s$ such that constraints from the minimal model's fusion rules survive in the limit. As a result, we can obtain not only Liouville theory with its trivial OPE \eqref{idpv}, but also another diagonal CFT with a continuous spectrum, called a \textbf{Runkel--Watts-type theory} after the original example at $c=1$, discovered by Runkel and Watts \cite{rw01}. 
...
This leads to 
\begin{align}
 \lim P_{(r,s)} =  \frac{1}{2\sqrt{p_0q_0}}\left(n+f\right)\quad \text{with} \quad \left\{\begin{array}{l} n =r_0q_0 - s_0p_0 \in \mathbb{Z} \ , \\ f =  \lim\frac{r}{p} = \lim\frac{s}{q} \in (0, 1) \ . \end{array}\right.
\end{align}
...
both $\frac{r}{p}$ and $\frac{s}{q}$, and therefore also $f=\frac12\left(\frac{r}{p}+\frac{s}{q}\right)$, are evenly distributed in the interval $(|f_1-f_2|, \min(f_1+f_2,2-f_1-f_2))$. Writing the primary fields as $V_{n,f} = \lim V_{(r,s)}^f$, we thus find the OPE
\begin{align}
 \boxed{V_{n_1,f_1}V_{n_2,f_2} \sim \sum_{n\in n_1+n_2+p_0+q_0+2\mathbb{Z}}
\int_{|f_1-f_2|}^{\min (f_1+f_2,2-f_1-f_2)} df\ V_{n, f}}\ .
\end{align}
Of course, $V_{n,f}$ is just another notation for the diagonal field $V_P$, with $n,f$ the integer and fractional parts of $2\sqrt{p_0q_0}P$. The reflection relation is now $(n,f)\to (-n-1,1-f)$. 
The resulting Runkel--Watts-type CFT has the same spectrum as Liouville theory \eqref{sliou} $
 \mathcal{S}^{\text{RWT}_{p_0,q_0}} = \frac12 \left\{ V_P\right\}_{P\in \mathbb{R}}$. In contrast to Liouville theory, the momentums cannot be continued to complex values, where the OPE would no longer make sense. Notice however that if $f_1\neq f_2$, the OPE does not produce fields with degenerate momentums, i.e. fields of the type $V_{n,0}$. So there is no need to regularize the integral over $f$. Alternatively, the OPE may be written in a manifestly reflection-invariant way using the momentums $P_i$ \cite{rs15},
 \begin{align}
...
New subsection for summarizing arrows? Copy figure from \cite{rib14}. Change color for arrows that depend on structure constants, including stating that we can get deg. fields from Liouville theory. 

Limit of MM from GMM subtle at the level of correlation functions, could get log-MM. Add log-MM to diagram?  
...
See whether in Liouville theory fields with deg. dimensions are actually deg. For $c\leq 1$, guess not deg. from MM limit. For other $c$, need to know about poles of struct. csts. So answer is probably no. Postpone the problem to analysis of correl. 
