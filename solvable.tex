...
\subsubsection{Degenerate structure constants}
...
\begin{align}
 ...
 \label{cbcb}
\end{align}
where the second equation is obtained from the first one, applied to $i$ and $i^+$. (For $s_i\in\mathbb{Z}+\frac12$ the sign prefactor comes from resolving the sign ambiguity of $(-)^{s_i}$.)
...
\end{align}
Now this ratio differs from Eq. \eqref{sh-mp} with $\bar P_1=P_1$ by an overall sign. We now cancel this sign by doing the field renormalization $V^d_{\langle r,s\rangle}\to (-)^{\frac{r+1}{2}} V^d_{\langle r,s\rangle}$ in  D-series minimal models.  
This renormalization leaves the other shift equations unchanged. (Eq. \eqref{sh-bb} would be changed, but it does not make sense in a D-series minimal model.) We therefore obtain a sign prefactor in the diagonal 3-point structure constant:
\begin{align}
...
In the loop models' extended spectrum $\widetilde{S}^\text{loop}$ \eqref{sloop}, it is clear that shift equations do not determine the structure constants. First consider the non-diagonal fields $V_{(r,s)}$. The spectrum $\widetilde{S}^\text{loop}$ does not include the degenerate field $V_{\langle 2,1\rangle}^d$, whose shift equations would constrain the dependence on $r$. We only have $V^d_{\langle 1,2\rangle}$, whose shift equations determine how structure constants behave under $s\to s+2$, whereas $s$ takes fractional values. Second, consider the diagonal fields $V_P$. The shift equations from $V^d_{\langle 1,2\rangle}$ determine how structure constants behave under $P\to P+\beta^{-1}$. 
...
\begin{align}
 C^\text{norm}_{P_1,(r_2,s_2),(r_3,s_3)} = (-)^{\frac{s_2+s_3}{2}} \prod_\pm \prod_{i=-\frac{|r_2\pm r_3|-1}{2}}^{\frac{|r_2\pm r_3|-1}{2}} 2\cos \pi(\beta P_1+\beta^2i) \ ,
\end{align}
where the product over $i$ runs by steps of $1$. Since $r_2,r_3\in\mathbb{Z}+\frac12$, there is one $i=0$ factor, which is $2\cos(\pi \beta P_1)$. The rest of the product can be written in terms of 
\begin{align}
 4\cos \pi(\beta P_1+\beta^2i)\cos \pi(\beta P_1-\beta^2i) = 2\cos(2\pi\beta P_1)+2\cos(2\pi i \beta^2)\ , 
\end{align}
which for $i\in\frac12\mathbb{Z}^*$ is polynomial in loop weights. 
...
Move to logarithmic section? Dismiss? 
