...
For any complex value of the central charge, we define a \textbf{\boldmath generalized minimal model}\index{minimal model!generalized---} by its spectrum,
\begin{align}
...
the momentums $P_{\langle r,s \rangle}$ and $P_{\langle r',s' \rangle}$ \eqref{ars} obey $P_{\langle r,s \rangle}=P_{\langle r',s' \rangle}$ or $P_{\langle r,s \rangle}=Q-P_{\langle r',s' \rangle}$, and $b^2$ must therefore be rational.
To obtain rational theories, it is enough to focus on the case 
\begin{align}
 P_{\langle r,s \rangle}+P_{\langle r',s' \rangle} = Q\ .
\end{align}
...
According to eq. \eqref{rtv}, the representation $\mathcal{V}_{P'} $ can appear in the fusion product $\mathcal{R}\times \mathcal{V}_P$ only if there are integers $(i,j,i',j')\in [0,r-1]\times [0,s-1]\times [0,r'-1]\times [0,s'-1]$ such that 
\begin{align}
 \Delta(P') = \Delta\left(P+P_{\langle r,s \rangle} + ib+jb^{-1}\right) = \Delta\left(P+P_{\langle r',s' \rangle} +i'b+j'b^{-1}\right) \ .
\end{align}
Since $\Delta(P)$ \eqref{daq} is a quadratic function, we a priori have two types of solutions.
In the first type, we have $P+P_{\langle r,s \rangle} + ib+jb^{-1} = P+P_{\langle r',s' \rangle} +i'b+j'b^{-1}$, which implies $q(r-i+i')=p(s-j+j')$.
...
\begin{align}
 \left(P+P_{\langle r,s \rangle} + ib+jb^{-1}\right) + \left(P+P_{\langle r',s' \rangle} +i'b+j'b^{-1}\right) = Q\ .
\end{align}
...
\begin{align}
 P_{\langle r'',s'' \rangle}+P_{\langle p-r'',q-s'' \rangle}=Q\ ,
\end{align}
...
\begin{align}
 \left\langle V_{\langle r_1,s_1 \rangle} V_{\langle r_2,s_2 \rangle} \right\rangle = \delta_{r_1,p-r_2}\delta_{s_1,q-s_2} + R(P_{\langle r_1,s_1 \rangle}) \delta_{r_1,r_2} \delta_{s_1,s_2} \ .
\label{vvddp}
\end{align}
(Compare with the two-point function of a generalized minimal model \eqref{vvdd}.) This formula can be simplified by renormalizing the fields, so that the reflection coefficient $R(P_{\langle r,s\rangle})$ becomes one.
...
Defining
\begin{align}
 \boxed{\tilde{V}_{\langle r,s \rangle} = \frac{1}{\sqrt{R(P_{\langle r,s \rangle})}} V_{\langle r,s \rangle}}\ , 
\end{align}
where the reflection coefficient $R(P)$ was given in eq. \eqref{ram}, we now have 
\begin{align}
...
\end{align}
There is an additional subtlety in the case of the three-point function: if we want $\tilde{V}_{\langle 1,1 \rangle}$ to be the identity field, we need to renormalize the three-point structure constant with a factor $\sqrt{R(P_{\langle 1,1 \rangle})}$ on top of the factors coming from the field renormalizations, so that 
\begin{align}
 \tilde{C}_{\langle r_1,s_1\rangle ,\langle r_2,s_2\rangle ,\langle r_3,s_3 \rangle} = \sqrt{\frac{R(P_{\langle 1,1 \rangle})}{\prod_{i=1}^3 R(P_{\langle r_i,s_i \rangle})}}  C_{\langle r_1,s_1\rangle ,\langle r_2,s_2\rangle ,\langle r_3,s_3 \rangle}\ ,
\label{tcc}
\end{align}
...
\begin{enumerate}
 \item Show that for $z_1\to z_2$, the integral over $s$-channel momentums localizes near $P_s=\frac{Q}{2}$. Show that the three-point structure constants $C$ and $\hat C$ behave differently near this value of the momentum.
 \item Show that 
 \begin{align}
...
\begin{align}
 \tilde{C}_{\phi\phi\phi} = \sqrt{\frac{R(P_{\langle 1,1 \rangle})}{R(P_{\langle 2,1 \rangle})^3}} C_{\langle 2,1\rangle , \langle 2,1 \rangle,\langle 2,1 \rangle}\ .
\end{align}
Notice that $C_{\langle 2,1\rangle , \langle 2,1 \rangle,\langle 2,1 \rangle} = C_+(P_{\langle 2,1 \rangle})=1$ due to eqs. \eqref{vot} and \eqref{cco}.
...
\begin{align}
 \tilde{C}_{\sigma\sigma\epsilon} = \frac{1}{R(P_{\langle 2,1 \rangle})}\sqrt{\frac{R(P_{\langle 1,1 \rangle})}{R(P_{\langle 1,2 \rangle})}} C_{\langle 2,1\rangle , \langle 2,1 \rangle,\langle 1,2 \rangle}\ .
\end{align}
Setting $\mu=1$, compute
\begin{align}
