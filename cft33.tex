...
While we mostly present already known results, the following aspects may be original:
\begin{itemize}
...
We consider a two-dimensional space, and assume it comes equipped with a metric.
...
\end{center}
So, we assume that the symmetries include the infinitely many independent \textbf{\boldmath local conformal transformations}\index{conformal transformation!local---}, in addition to the global conformal transformations, which are parametrized by the six real coordinates of $PSL_2({\mathbb{C}})$.
Let us now describe the mathematical structure of the resulting space of all conformal transformations.
This space is an infinite-dimensional Lie group that includes $PSL_2({\mathbb{C}})$.
...
\begin{itemize}
 \item the linear dilaton theory, which like Liouville theory has a diagonal, continuous spectrum and is defined for any $c\in {\mathbb{C}}$, being unitary if $c\geq 1$.
...
\label{pur}
\end{align}
Let us introduce a basis $\mathcal{L}$ of $U(\mathfrak{V}^+)$, that is parametrized by ordered $p$-uples $(-n_1,\cdots -n_p)$ of decreasing, strictly negative integers:
\begin{align}
...
(See also Exercise \ref{exotbf}.) The corresponding solutions are called $s$-channel \textbf{degenerate conformal blocks}\index{conformal block!degenerate---}, denoted $\mathcal{F}^{(s)}_\pm(x)$, and depicted
\begin{align}
 \mathcal{F}^{(s)}_\pm(x)  =  
 \begin{tikzpicture}[baseline=(current  bounding  box.center), very thick, scale = .4]
...
We can also define the $t$-channel basis of solutions, whose elements are associated to the characteristic exponents at the regular singular point $x=1$.
...
\begin{enumerate}
 ...
 \item
 Write a computer program for computing singular vectors at arbitrary levels, and check that at level $N$ the number of Verma modules that have a singular vector is the number of factorizations of $N$ into two positive integers.
\end{enumerate}
\end{exo}
