...
We now assume that the fusion rules are associative, and proceed to deduce the existence of the higher degenerate representations $\mathcal{R}_{\langle r,s \rangle}$, and their fusion products. From the product $\mathcal{R}_{\langle 2,1 \rangle}\times \mathcal{V}_P$ and associativity, we first deduce 
\begin{align}
...
\end{align}
Since we obtain a sum of finitely many Verma modules, $\mathcal{R}_{\langle 2,1 \rangle}\times \mathcal{R}_{\langle 2,1 \rangle}$ must be a sum of finitely many degenerate representations. But we know that $\mathcal{R}_{\langle 2,1 \rangle}\times \mathcal{R}_{\langle 2,1 \rangle}$ is a sum of two representations with the momentums $\{P_{\langle 2,1 \rangle} \pm \frac{b}{2}\} = \{P_{\langle 1,1\rangle}, P_{\langle 3,1\rangle}\}$. 
...
\end{align}
Generalizing this reasoning, we can construct the degenerate representation $\mathcal{R}_{\langle r,s\rangle}$ and its fusion rules by recursion on $r,s\in\mathbb{N}^*$, starting from the known degenerate representations $\mathcal{R}_{\langle 1,1\rangle}, \mathcal{R}_{\langle 2,1\rangle}, \mathcal{R}_{\langle 1,2\rangle}$. 
In particular, the fusion rule 
\begin{align}
...
The sum in eq. \eqref{fsd} is over conformal dimensions $\Delta_s,\bar{\Delta}_s$, which stand for representations of the Virasoro algebra. The relative contributions of all states in a given irreducible representations are determined by Ward identities, and the sum over these states is included in the conformal block by definition. By definition, the sum has to converge whenever the OPE $V_{\sigma_1}(z_1)V_{\sigma_2}(z_2)$ converges. However, we will shortly see that conformal blocks can be defined for all positions $z_i$ by analytic continuation, and the sum actually converges everywhere in minimal models (because it is a finite sum) and in Liouville theory (see Section \ref{seceul}).
