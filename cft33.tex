...
\begin{align}
 ...
 \label{eq:2pt}
\end{align}
Here the symbol $\delta_{\Delta_1,\Delta_2}$ is a Kronecker delta or a Dirac delta function, depending on whether the conformal dimensions take discrete or continuous values. We have assumed that the fields have no multiplicities, i.e. two different primary fields cannot have the same dimensions. And we have introduced the \textbf{two-point structure constant}\index{structure constant!two-point---} $B_{1}$, which depends on $(\Delta_1,\bar\Delta_1)$, equivalently on $(\Delta_2,\bar\Delta_2)$. We could perform a field renormalization such that $B_{1}=1$, but this would be incompatible with the analyticity of the three-point structure constant in Liouville theory, as we will see in Section \ref{seccbe}.
...
\begin{align}
 C_{123} = \mu(\Delta_1) \nu(\Delta_2)\nu(\Delta_3)F_{\langle 1,1\rangle, \Delta_{1}} \begin{bmatrix} \Delta_2 & \Delta_3\\ \Delta_2 & \Delta_3\end{bmatrix}\ ,
\end{align}
...
\begin{align}
 V_{P_1}V_{P_2} \sim \frac12\int_{i{\mathbb{R}}} dP \frac{C_{P_1,P_2,P}}{B_P} V_P + \sum_{P_k\in \mathcal{P}_{P_1,P_2}} \frac{\underset{P=P_k}{\operatorname{Res}} C_{P_1,P_2,P}}{B(P_k)} V_{P_k}\ ,
 \label{acope}
\end{align}
