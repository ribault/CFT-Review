...
% NB: also mention right primaries, introduce V_{\Delta, \bar \Delta}
...
Having two Virasoro symmetry algebras $\mathfrak{V}$ and $\overline{\mathfrak{V}}$, we will consider fields $V_{\Delta,\bar\Delta}(z)$ that are primary with respect to both algebras. Then $\Delta$ and $\bar\Delta$ are respectively called the left and right conformal dimensions of $V_{\Delta,\bar\Delta}(z)$. Depending on the context, the notation $V_\Delta(z)$ will denote either a $\mathfrak{V}$-primary field, or more specifically a \textbf{diagonal primary field}\index{diagonal primary field} $V_\Delta(z)= V_{\Delta,\Delta}(z)$. 
...
Relaxing this axiom can be fruitful, in particular by allowing the existence of fermionic and parafermionic fields.
...
Let us first write the local Ward identities.
A spanning set of these identities can be obtained by taking $\epsilon(z) = \frac{1}{(z-z_i)^{n-1}}$ with $n\geq 2$ and $i=1, \cdots N$ in eq. \eqref{oiz}. 
...
Let us now write
the three global Ward identities. Taking $\epsilon(z)\in \{1,z,z^2\}$ in eq. \eqref{oiz}, we find 
\begin{align}
...
\label{slllz}
\end{align}
These equations express the global conformal invariance of correlation functions.
...
Using Axiom \eqref{ax:dvz}, the global Ward identities for an $N$-point function $\left\langle \prod_{i=1}^N V_{\Delta_i}(z_i)\right\rangle$ of quasi-primary fields reduce to a system of three differential equations, 
\begin{align}
...
Let us discuss the solutions of the global Ward identities \eqref{spz} for an $N$-point function of primary fields
$
...
This implies $\left\langle V_{\Delta_1}(z_1)\right\rangle$ is constant, and that this constant vanishes unless $\Delta_1=0$.
...
\end{align}
Setting
the $z_i$-independent factor to one by a choice of normalization of the fields, and
restoring the dependence on the right conformal dimensions, we find
\begin{align}
 \boxed{ \left\langle V_{\Delta_1,\bar{\Delta}_1}(z_1,\bar{z}_1) V_{\Delta_2,\bar{\Delta}_2}(z_2,\bar{z}_2)\right\rangle = \delta_{\Delta_1,\Delta_2}\delta_{\bar{\Delta}_1,\bar{\Delta}_2} z_{12}^{-2\Delta_1}\bar z_{12}^{-2\bar \Delta_1} }\ ,
 \label{eq:2pt}
\end{align}
where the symbol $\delta_{\Delta_1,\Delta_2}$ is a Kronecker delta or a Dirac delta function, depending on whether the conformal dimensions take discrete or continuous values.
...
Restoring the dependence on the right conformal dimensions, we therefore have 
\begin{align}
 \left\langle \prod_{i=1}^3 V_{\Delta_i,\bar{\Delta}_i}(z_i) \right\rangle = C_{\Delta_1,\bar{\Delta}_1,\Delta_2,\bar{\Delta}_2,\Delta_3,\bar{\Delta}_3}
 \left|\mathcal{F}^{(3)}(\Delta_1,\Delta_2,\Delta_3|z_1,z_2,z_3)\right|^2\ ,
\label{cff}
\end{align}
where the modulus square notation means $|f(\Delta,z)|^2 = f(\Delta,z)f(\bar{\Delta},\bar{z})$, and the factor
$C_{\Delta_1,\bar{\Delta}_1,\Delta_2,\bar{\Delta}_2,\Delta_3,\bar{\Delta}_3}$ is called the \textbf{\boldmath three-point structure constant}\index{three-point structure constant}.
...
\label{zgg}
\end{align}
where we built the prefactor as a combination of three-point functions, such that each variable $z_i$ has the total exponent $-2\Delta_i$.
...
\end{align}
\end{itemize}
In order for the three-point function \eqref{cff} to be single-valued as required by Axiom \ref{ax:svcf}, we need $\Delta_i-\bar\Delta_i \in\mathbb{Z}$. Therefore, any primary field $V_{\Delta,\bar{\Delta}}(z,\bar{z})$ must obey
\begin{align}
 \boxed{\Delta-\bar{\Delta} \in {\mathbb{Z}}}\ .
\label{dbdz}
\end{align}
The number $\Delta-\bar{\Delta}$ is called the \textbf{conformal spin}\index{spin (conformal)} of the field $V_{\Delta,\bar\Delta}(z,\bar z)$, as it controls how that field behaves under rotations, $T_{\left(\begin{smallmatrix} e^{i\theta} & 0 \\ 0 & e^{-i\theta} \end{smallmatrix}\right)} V_{\Delta,\bar{\Delta}}(z,\bar{z}) = e^{2i\theta(\Delta-\bar\Delta)} V_{\Delta,\bar{\Delta}}(e^{2i\theta} z,e^{-2i\theta}\bar{z})$. A primary field is diagonal if and only if its conformal spin is zero.


\subsection{Operator product expansions \label{secope}}

Let us study how local conformal symmetry constrain OPEs. We start with a generic OPE of the type $V_{\sigma_1}(z_1)V_{\sigma_2}(z_2) = \sum_\sigma C_{\sigma_1,\sigma_2}^\sigma(z_1,z_2) V_\sigma(z_2)$. Let us insert $\oint_C dz (z-z_2)^{n+1} T(z)$ on both sides of the OPE, where the contour $C$ encircles both $z_1$ and $z_2$. Assuming $n\geq -1$, this yields the \textbf{OPE Ward identity}\index{Ward identity!---for OPEs}
\begin{align}
 \left(L_n^{(z_2)}+\sum_{m=-1}^{n}\binom{m+1}{n+1} z_{12}^{n-m}L_{m}^{(z_1)}\right)V_{\sigma_1}(z_1)V_{\sigma_2}(z_2) = \sum_\sigma C_{\sigma_1,\sigma_2}^\sigma(z_1,z_2) L_n V_\sigma(z_2)\ .
\end{align}
...
Using Axiom \ref{ax:dvz}, the $n=-1$ identity becomes 
 \begin{align}
  \left(\frac{\partial}{\partial z_1} +\frac{\partial}{\partial z_2}\right) V_{\sigma_1}(z_1)V_{\sigma_2}(z_2) = \sum_\sigma C_{\sigma_1,\sigma_2}^\sigma(z_1,z_2) \frac{\partial}{\partial z_2} V_\sigma(z_2)\ .
 \end{align}
Using the OPE again on the left-hand side, this implies $\left(\frac{\partial}{\partial z_1} +\frac{\partial}{\partial z_2}\right) C_{\sigma_1,\sigma_2}^\sigma(z_1,z_2)=0$ -- translation invariance of the OPE coefficients. 
Assuming the fields $V_{\sigma_1}, V_{\sigma_2}$ and $V_\sigma$ are  $L_0$-eigenvectors with the respective dimensions $\Delta_{\sigma_1}, \Delta_{\sigma_2}$ and $\Delta_\sigma$, the $n=0$ identity becomes 
\begin{align}
 \left(z_{12}\frac{\partial}{\partial z_1} + \Delta_{\sigma_1}+\Delta_{\sigma_2}\right) V_{\sigma_1}(z_1)V_{\sigma_2}(z_2) = \sum_\sigma C_{\sigma_1,\sigma_2}^\sigma(z_1,z_2) \Delta_\sigma V_\sigma(z_2)\ .
\end{align}
...
\begin{align}
 C_{\sigma_1,\sigma_2}^\sigma(z_1,z_2) = C_{\sigma_1,\sigma_2}^\sigma 
 z_{12}^{\Delta_{\sigma}-\Delta_{\sigma_1}-\Delta_{\sigma_2}}
 \bar z_{12}^{\bar\Delta_{\sigma}-\bar\Delta_{\sigma_1}-\bar\Delta_{\sigma_2}}
 \ ,
 \label{eq:coz}
\end{align}
...
Let us for simplicity specialize to an OPE of two primary fields $V_{\Delta_1}(z_1)V_{\Delta_2}(z_2)$. Using the form \eqref{eq:coz} of the OPE coefficients, and
omitting the dependence on right-moving quantities $\bar z_i,\bar \Delta_i$, the local Ward identities read 
\begin{align}
 \sum_\sigma C^\sigma_{\Delta_1,\Delta_2}(\Delta_\sigma + n\Delta_1-\Delta_2) z_{12}^{\Delta_{\sigma}-\Delta_1-\Delta_2}z_{12}^n V_\sigma(z_2) = \sum_\sigma C^\sigma_{\Delta_1,\Delta_2} z_{12}^{\Delta_{\sigma}-\Delta_1-\Delta_2}  L_nV_\sigma(z_2)\ .
\end{align}
Focussing on the right-hand side terms that come from descendents $V_\sigma= LV_\Delta$ of a given primary field at a level $|L|=N\geq n$, we find the identities of elements of $U(\mathfrak{V}^+)$ 
\begin{align}
 \sum_{|L|=N-n} C^{(\Delta,L)}_{\Delta_1,\Delta_2}(\Delta+N-n+n\Delta_1-\Delta_2)L = \sum_{|L|=N} C^{(\Delta,L)}_{\Delta_1,\Delta_2}[L_n, L]\ .
 \label{eq:lwo}
\end{align}
This system of linear equations determines the coefficients $C^{(\Delta,L)}_{\Delta_1,\Delta_2}$ of descendent fields in terms of the coefficient $C^{(\Delta,1)}_{\Delta_1,\Delta_2}$ of the primary field,
\begin{align}
 C^{(\Delta,L)}_{\Delta_1,\Delta_2} = C^{(\Delta,1)}_{\Delta_1,\Delta_2} f^{\Delta,L}_{\Delta_1,\Delta_2}\ ,
\end{align}
where  $f^{\Delta,L}_{\Delta_1,\Delta_2}$ are universal coefficients such that $f^{\Delta,1}_{\Delta_1,\Delta_2} =1$.
Let us show this for $|L|\leq 2$. The cases $(N,n)=(1,1),(2,2),(2,1)$ of \eqref{eq:lwo} respectively yield 
\begin{align}
 \Delta + \Delta_1-\Delta_2 & = 2\Delta f^{L_{-1}} \ ,
 \label{flfo}
 \\
 \Delta+ 2\Delta_1-\Delta_2 & = 6\Delta f^{L_{-1}^2} + (4\Delta+\tfrac{c}{2})f^{L_{-2}}\ ,
 \\
 (\Delta+1+\Delta_1-\Delta_2) f^{L_{-1}} & = 2(2\Delta+1) f^{L_{-1}^2} + 3 f^{L_{-2}}\ ,
\end{align}
where we temporarily use the notation $f^L=f^{\Delta,L}_{\Delta_1,\Delta_2}$.
The first equation determines $f^{L_{-1}}$, unless $\Delta=0$. The next two equations determine
$f^{L_{-1}^2}$ and $f^{L_{-2}}$, unless $\Delta \in \{\Delta_{\langle 1,2 \rangle}, \Delta_{\langle 2,1 \rangle}\}$.
More generally, given a value of the level $N\geq 2$, the equations with $n=1,2$ determine $\{f^L\}_{|L|=N}$, unless $\Delta\in\{\Delta_{\langle r,s \rangle}\}_{rs=N}$. (See Exercise \ref{exohf}.) If $\Delta=\Delta_{\langle r,s \rangle}=$, then for generic values of $\Delta_1,\Delta_2$ there is no solution for $\{f^L\}$, which means that $V_\Delta$ cannot appear in the OPE $V_{\Delta_1}V_{\Delta_2}$.


\subsubsection{Structure of the OPE}

Still omitting right-moving quantities, our OPE of two primary fields can now be written as 
\begin{align}
 V_{\Delta_1}(z_1) V_{\Delta_2}(z_2) = \sum_\Delta C^{(\Delta, 1)}_{\Delta_1,\Delta_2} 
 z_{12}^{\Delta-\Delta_1-\Delta_2} \Big( V_\Delta(z_2) + O(z_1-z_2)\Big)\ .
\end{align}
Inserting this in a three-point function, and using the two-point function \eqref{eq:2pt}, this leads to
\begin{align}
 \left<  V_{\Delta_1}(z_1) V_{\Delta_2}(z_2) V_{\Delta_3}(z_3) \right> = C^{(\Delta_3,1)}_{\Delta_1,\Delta_2} z_{12}^{\Delta_3-\Delta_1-\Delta_2} \Big( z_{23}^{-2\Delta_3} + O(z_1-z_2) \Big)\ .
\end{align}
...
\begin{align}
 C^{(\Delta_3,1)}_{\Delta_1,\Delta_2} = C_{\Delta_1,\Delta_2,\Delta_3}\ .
 \label{cftt}
\end{align}
Let us now restore the dependence on right-moving quantities, and write the complete expression of the OPE of two primary fields:
\begin{align}
 \boxed{ V_{\Delta_1,\bar\Delta_1}(z_1) V_{\Delta_2,\bar\Delta_2}(z_2) = \sum_{\Delta,\bar\Delta} C_{\Delta_1,\bar\Delta_1,\Delta_2,\bar\Delta_2,\Delta,\bar\Delta} \left| z_{12}^{\Delta-\Delta_1-\Delta_2}\sum_{L\in\mathcal{L}} z_{12}^Lf^{\Delta,L}_{\Delta_1,\Delta_2} L \right|^2 V_{\Delta,\bar\Delta}(z_2)
 }\ ,
 \label{vvs}
\end{align}
...
That OPE Ward identities uniquely determine the contributions of the descendents of a given primary field, means that fusion multiplicities are trivial in fusion products of highest-weight representations of the Virasoro algebra. By definition, the multiplicity of the Verma module $\mathcal{V}_\Delta$ in the fusion product $\mathcal{V}_{\Delta_1}\times \mathcal{V}_{\Delta_2}$ indeed coincides with the dimension of the space of solutions of the OPE Ward identities, and that dimension is one or zero.
