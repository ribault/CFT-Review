...
The difference between generalized minimal models and minimal models is more apparent in universal quantities that reflect the nature of the involved representations: degenerate in the former case, doubly degenerate in the latter case.
...
\label{tqz}
\end{align}
which depend on a parameter $Q$ called the \textbf{\boldmath $\hat{\mathfrak{u}}_1$ background charge}\index{background charge!u1@$\hat{\mathfrak{u}}_1$---}. 
...
We define a \textbf{\boldmath $\hat{\mathfrak{u}}_1$-primary field}\index{primary field!u1@$\hat{\mathfrak{u}}_1$---} $V_{\alpha}(z_0,\bar{z}_0)$ with the \textbf{\boldmath $\hat{\mathfrak{u}}_1$ momentum}\index{momentum!u1@$\hat{\mathfrak{u}}_1$---} $\alpha$ by its OPE with $J(y)$,
\begin{align}
...
Actually it is the $x$-basis calculation which is misleading: analyticity conditions on the $x_i$-dependence of $\left\langle \prod_{i=1}^3 \Phi^{j_i}_{x_i} \right\rangle$ in general allow the existence of two solutions that differ globally, although they are locally identical \cite{rib09}.
...
\end{align}
For any value of the level, we can build a diagonal spectrum from these representations,
\begin{align}
...
Therefore, it is natural to conjecture that correlation functions of the (generalized) $SU_2$ WZW model can be deduced from $H_3^+$ correlation functions by analytically continuing the spins (and the level) to discrete values, in the same way as (generalized) minimal model two- and three-point functions are deduced from Liouville theory. 
For the first step of recovering degenerate fusion rules from the $H_3^+$ OPE, see Exercise \ref{exodrfrh}.
...
After the $H_3^+$ model, the \textbf{$\widetilde{SL}_2(\mathbb{R})$ WZW model}\index{WZW model!sl2tilder@$\widetilde{SL}_2(\mathbb{R})$---} is our second example of a non-rational model with an $\widehat{\mathfrak{sl}}_2$ symmetry algebra.
