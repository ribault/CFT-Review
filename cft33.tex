...
And we will now show that the solution of this equation can be written in terms of the fusion matrix. 
...
The particular fusing matrix that we need can actually be computed as a limit of the general expression \eqref{eq:fm},
\begin{align}
 ...
 \\
  ...
  \label{eq:fdozz}
\end{align}
where we neglect factors that can be absorbed into field renormalizations $V_{\Delta}(z)\to \lambda(\Delta)V_\Delta(z)$. (See Exercise \ref{exoflfg}.) This  coincides with the DOZZ formula \eqref{caaa} for the three-point function of Liouville theory, given the relation $\Upsilon_b(x) = \frac{1}{\Gamma_b(x)\Gamma_b(Q-x)}$ between the Upsilon function that appears in the DOZZ formula, and the double Gamma function.
...
Rather than an appealing way of solving Liouville theory, the present discussion is a demonstration of its fundamental nature, as its three-point structure constant is a special case of the fusing matrix of Verma modules. Actually, the full fusing matrix coincides (up to simple factors) with Liouville theory's boundary three-point structure constant \cite{pt01}, and is sometimes called the Liouville fusing matrix. 




define Upsilon function (index), postpone properties to later. Use its name in Liouville part. 


Universality of Liouville theory... also emphasize in section 0/1.
