...
\label{upup}
\end{align}
These equations define a unique (up to a constant factor) function $\Upsilon_b(x)$, which will turn out to have an analytic continuation to the domain $\{\Re b > 0 \}$. 
...
\end{align}
The solution $\hat\Upsilon_b(x)$ of these equations can actually be constructed from the function $\Upsilon_b(x)$ as
\begin{align}
 \boxed{ \hat{\Upsilon}_b(x) = \frac{1}{\Upsilon_{ib}(-ix+ib)} }\ ,
\label{tub}
\end{align}
and this function therefore has an analytic continuation to the domain $\{\Im b < 0 \}$. 
For generic values of the central charge $c$, we can choose $\Re b>0$ and $\Im b<0$, and both functions $\Upsilon_b(x)$ and $\hat\Upsilon_b(x)$ exist. 
...
But is this solution unique? and is the resulting OPE associative in general, when degenerate fields are not necessarily involved? These are the questions of the uniqueness and existence of Liouville theory, as defined by the axioms of Section \ref{secspe}.
...
domain, which would contradict our considerations on uniqueness.
