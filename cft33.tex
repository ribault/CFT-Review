...
$N$-point functions of descendent fields are of the type $\left\langle \prod_{i=1}^N \left(\left(\prod_{j=1}^{p_i} L_{-n_{i,j}}\prod_{\bar j=1}^{\bar p_i} \bar L_{-\bar n_{i,\bar j}}\right)V_{\Delta_i,\bar\Delta_i}(z_i)\right) \right\rangle$, with $p_i,\bar p_i\geq 0$ and $n_{i,j},\bar n{i,\bar j}\geq 1$. 
In order to derive equations that constrain and relate these correlation functions, we will now explain how the action of conformal symmetry is encoded in a field, the energy-momentum tensor.
...
\end{hyp}
\noindent
The value of $\frac{\partial}{\partial z} V_\sigma(z)$ is suggested by the interpretation of $L_{-1}$ as a generator of translations.

From this axiom we can immediately deduce how $L_n^{(z)}$ depends on $z$. Applying the axiom to the two fields $V_\sigma(z)$ and $L_n^{(z)}V_\sigma(z)$, we find 
\begin{align}
...
Now this relation is equivalent to $\frac{\partial}{\partial z} T(y) = 0$, where we define the \textbf{\boldmath energy-momentum tensor}\index{energy-momentum tensor} $T(y)$ as 
\begin{align}
...
$\left(\begin{smallmatrix} T_{yy} = T(y) & T_{y\bar y} \\ T_{\bar y y} & T_{\bar y \bar y} = \bar T(\bar y) \end{smallmatrix}\right)$.
...
\subsection{Global conformal symmetry}
...
\end{align}
\end{itemize}
Axiom on single-valuedness / absence of monodromy?
...
\subsection{Null vector equations, and the fusion rules}\label{secfr}
...
\section{Consequences of conformal symmetry}
