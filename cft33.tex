...
Let us investigate the fusion rules involving doubly degenerate representations. 
Let us start with the fusion product $\mathcal{R}\times \mathcal{V}_P$ of a doubly degenerate representation $\mathcal{R}$ with a Verma module, which encodes rules that apply to fusion products of $\mathcal{R}$ with 
arbitrary highest-weight representations. We will see that this fusion product is zero for almost all values of $P$. Whenever $\mathcal{R}\times \mathcal{V}_P=0$, we consider that $\mathcal{R}$ cannot coexist with a representation of momentum $P$, as correlation functions that involve both representations would vanish. 
