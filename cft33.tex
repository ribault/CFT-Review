...
\end{align}
By closure under fusion, the vector $(\alpha,\bar\alpha)$ must take values in a set that is closed under addition. For $Q-\alpha-\bar\alpha$ to take nonzero values, 
the values of $\alpha-\bar\alpha$ 

Let $R$ be the smallest number (in modulus) such that $\alpha-\bar\alpha\in \frac{i}{R}\mathbb{Z}$, then we must have $\alpha+\bar\alpha-Q\in iR\mathbb{Z}$. 


In the spectrum $S_R^0$ that we wish to extend, we have $\alpha+\bar{\alpha}\in\frac{i}{R}{\mathbb{Z}}$, so that the largest possible extension is obtained by assuming $\alpha-\bar{\alpha}\in iR{\mathbb{Z}}$.
