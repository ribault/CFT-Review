...
\begin{align}
 ...
 \label{nop}
\end{align}
where $\mu$ is an arbitrary parameter called the cosmological constant.
The resulting expression for the three-point structure constant is
\begin{align}
 C_{P_1,P_2,P_3} =  \frac{\mu^{-\frac{Q}{2}-P_1-P_2-P_3}\Upsilon_b'(0)\prod_{i=1}^3 \Upsilon_b(-2P_i) }{\prod_{\pm,\pm} \Upsilon_b\left(\tfrac{Q}{2}+P_1\pm P_2 \pm P_3\right)} \ ,
\label{caaa}
\end{align}
...
up to a redefinition of the cosmological constant. These normalizations are such that the later equation \eqref{crs} will hold. 
...
In particular, momentums can reach values that are related by the reflection $P\to -P$. But since multip. one they carry same info.

Let us discuss how correlation functions behave under the reflection $P\to -P$. Since the reflection leaves conformal dimensions invariant, normalization-independent quantities must be invariant too. However, the fields $V_P(z)$ and $V_{-P}(z)$ can differ by a normalization-dependent factor $R(P)$ called the \textbf{\boldmath reflection coefficient}\index{reflection!---coefficient}, so that we have the \textbf{\boldmath reflection relation}\index{reflection!---relation}
\begin{align}
V_P(z)= R(P) V_{-P}(z)\ .
\label{vrv}
\end{align}
...
\end{align}
where $B(P)=R(P)^2B(-P)$. Moreover, we must have $N(P)=R(P)N(-P)$ and $C_{P_1,P_2,P_3} = R(P_1) C_{-P_1,P_2,P_3}$.

We now focus on the particular normalization \eqref{nop}, such that the three-point function is given by the DOZZ formula. This normalization obeys $N(P)N(-P) = \left(N^2B^{-1}\right)(P)$, which implies $R(P)=B(P)$. 
...
\subsection{Degenerate fields in Liouville theory}

\subsubsection{Degenerate OPEs}
