...
\label{dyym} 
\end{align}
In particular, the two-point function can be deduced from the two-point function \eqref{vvss} of Liouville theory, 
\begin{align}
 \left\langle \Phi^{j_1}_{\mu_1} \Phi^{j_2}_{\mu_2}\right\rangle = \delta^{(2)}(\mu_1+\mu_2) |\mu_1|^2 b\, B\left(b^{-1}(j_1+\tfrac12)\right) \delta(j_2-j_1)\ ,
\end{align}
where we omitted the dependence on $z_i$, and $B(P)$ is the Liouville two-point structure constant.
...
\begin{align}
 \left\langle \prod_{i=1}^3\Phi^{j_i}_{\mu_i} \right\rangle = \delta^{(2)}(\textstyle{\sum}_{i=1}^3\mu_i) |\mu_2|^2 \sum_\pm C_\pm(P_1)C_{P_1\mp\frac{b}{2},P_2,P_3} \left|\mathcal{H}^{(s)}_\pm(-\tfrac{\mu_1}{\mu_2})  \right|^2\ , 
\label{sfpm}
\end{align}
...
$
 \mathcal{F}^{(s)}_\pm(x)  =  
\begin{tikzpicture}[baseline=(current  bounding  box.center), very thick, scale = .3]
\draw (-1,2) node [left] {$P_1$} -- (0,0) -- node [above] {$P_1\mp \frac{b}{2}$} (4,0) -- (5,2) node [right] {$P_2$};
\draw[dashed] (-1,-2) node [left] {$\langle 2,1 \rangle$} -- (0,0);
\draw (4,0) -- (5,-2) node [right] {$P_3$};
\end{tikzpicture}
$
...
\begin{align}
 C^{H_3^+}_{j_1,j_2,j_3} = \frac{\pi^2\prod_{i=1}^3\gamma(2j_i+1)}{d_\pm}C_{\pm}(P_1)C_{P_1\mp \frac{b}{2},P_2,P_3}\ .
\end{align}
...
Using eqs. \eqref{cpm} and \eqref{caaa} for $C_{\pm}(P_1)$ and $C_{P_1\mp \frac{b}{2},P_2,P_3}$ respectively, we find
\begin{align}
...
\subsubsection{Reflection relation}
...
\end{align}
which involves the Liouville reflection coefficient $R(P)$ \eqref{ram}.
