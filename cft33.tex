...
Proving such relations is not straightforward, because of subtleties with the analytic behaviour of correlation functions. In particular, the fusion rules of degenerate fields cannot be derived from Liouville theory, as we saw in Section \ref{secgmm}. Moreover, the analyticity of structure constants and conformal blocks as functions of the central charge does not imply the analyticity of four-point functions, as we saw in Section \ref{seceul}. Therefore, we need to understand the behaviour of four-point functions, and not just three-point functions. In Liouville theory and generalized minimal models, four-point functions are combinations of infinitely many conformal blocks, and their limits are not easy to control. Our statements on limits of CFTs will therefore only be conjectures.
...
\end{itemize}
The only known subtleties occur if $c_0$ is again of the type $c_{p_0,q_0}$ with $p_0,q_0\in \mathbb{Z}$. In this case, there are several inequivalent ways of defining the limit $\lim_{\substack{c_{p,q}\to c_0\\ P_{\langle r,s\rangle} \to P_0}}$. With some definitions, we recover Liouville theory. But in the case $c=1$, another definition leads to a CFT with the same spectrum as Liouville theory but a non-analytic structure constant, called Runkel--Watts theory \cite{rw01}. There is a Runkel--Watts-type theory for any central charges of the type $c_{p_0,q_0}$ \cite {rs15}, and we expect that this theory can again be reached as a particular limit of minimal models.

Need we give more details? rate at which $p/q$ approaches $p_0/q_0$? 

\subsubsection{From Liouville theory to minimal models}

First send momentums to be degenerate: can recover GMM in some cases. Then send central charge to be rational: can recover MM in some cases. 

Get RWT from Liouville also possible. 
