...
\\ \mathcal{F}^{(s)}_+(x)  =\left. \mathcal{F}^{(s)}_-(x) \right|_{P_1\to -P_1} \ ,
\end{array}\right. 
\label{gpm}
\end{align}
...
\end{hyp}
\noindent
Equivalently, we could assume that the two degenerate fields $V_{\langle 2,1\rangle}$ and $V_{\langle 1,2\rangle}$ exist, and construct the remaining degenerate fields by repeatedly performing OPEs with these two fields.
...
\begin{align}
 \boxed{V_{\langle r,s \rangle} V_P \sim \sum_{i=-\frac{r-1}{2}}^{\frac{r-1}{2}} \sum_{j=-\frac{s-1}{2}}^{\frac{s-1}{2}}  C_{i,j}^{\langle r,s \rangle}(P) \mathcal{V}_{P + ib+jb^{-1}}}\ .
\label{vrsv}
\end{align}
In analogy with eq. \eqref{cec}, the OPE coefficient $C_{i,j}^{\langle r,s \rangle}(P)$ is related to a three-point structure constant  as 
\begin{align}
 C_{i,j}^{\langle r,s \rangle}(P) = C_{\langle r,s \rangle\, ,\, P\, ,\, Q-P-P_{\langle r,s \rangle}-ib-jb^{-1}}\ , 
\end{align}
where $C_{\langle r,s \rangle, P_2,P_3}$ is invariant under the permutation of $P_2$ and $P_3$, and covariant under reflections of $P_2$ and $P_3$.
...
\begin{align}
 C_+(P) &= C_+(Q-P+\tfrac{b}{2})\ ,
\label{cpcp}
\\
C_-(P) & = \frac{R(P)}{R(P+\tfrac{b}{2})} C_+(P+\tfrac{b}{2})\ . 
\label{cmcp}
\end{align}
...
\boxed{V_{\langle 1,2 \rangle} V_P \sim \tilde{C}_+(P) V_{P-\frac{1}{2b}} + \tilde{C}_-(P) V_{P+\frac{1}{2b}}}\ ,
 \label{vto}
\end{align}
and relations involving the coefficients $\tilde{C}_+(P)$ and $\tilde{C}_-(P)$. 
