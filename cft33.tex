...
We have determined the normalization-independant combinations of structure constants, and this is enough for solving Liouville theory in the conformal bootstrap approach. However, in order to compare different approaches to Liouville theory, it can be convenient to consider specific a field normalization. 
...
\begin{align}
 N(P) = \left[b^{\frac{2}{b}-2b}\mu\right]^{-P}\Upsilon_b(-2P)\ ,
\end{align}
...
\begin{align}
 C_{P_1,P_2,P_3} =  \frac{\left[b^{\frac{2}{b}-2b}\mu\right]^{-\frac{Q}{2}-P_1-P_2-P_3}\Upsilon_b'(0)\prod_{i=1}^3 \Upsilon_b(Q+2P_i) }{\prod_{\pm,\pm} \Upsilon_b\left(\tfrac{Q}{2}+P_1\pm P_2 \pm P_3\right)} \ ,
\label{caaa}
\end{align}
...
up to a redefinition of the cosmological constant. In addition, our normalizations are such that the later equations \eqref{crs} and \eqref{clc} will hold. -- revise in light of later stuff?
...
=
\left\{\begin{array}{ll}
\left[b^{\frac{2}{b}-2b}\mu\right]^{Q-2P}\frac{\Upsilon_b(2P)}{\Upsilon_b(2P-Q)}
& \quad \text{with} \quad R(\tfrac{Q}{2}) = -1 \ ,
\\
\left[(ib)^{\frac{2}{b}-2b}\mu\right]^{Q-2P}\frac{\hat{\Upsilon}_b(2P)}{\hat{\Upsilon}_b(2P-Q)}
& \quad \text{with} \quad R(\tfrac{Q}{2}) = 1 \ .
\end{array}\right.
\end{align}
(Notice that $R(\frac{Q}{2})=-1$ implies $V_\frac{Q}{2}=0$, which is consistent with $C_{\frac{Q}{2},P_2,P_3}=0$. )
