...
\end{align}
where the normalization factor $N(P)$ transforms as $N(P)\to \lambda(P)N(P)$, and the renormalized three-point structure constant $C'_{P_1,P_2,P_3}$ is invariant. Then the invariant quantities that we want to determine are $C'_{P_1,P_2,P_3}$, and the combination $N^2B^{-1}(P)$. As a check that these quantities are all we need, let us write the crossing symmetry equation for a four-point function of the type $\langle V_{P_1}V_{P_2}V_{P_3}V_{P_4}\rangle$. Adapting the crossing symmetry equation \eqref{csd} to the 
...
up to a redefinition of the cosmological constant, and a reflection of the momentums. Our normalizations are such that the later equation \eqref{crs} will hold.
...
\label{hc}
\end{align}
Then $\hat C_{P_1,P_2,P_3}$ has the same behaviour as $C_{P_1,P_2,P_3}$ under the shifts $P_1\to P_1+b$ and $P_1\to P_1+\frac{1}{b}$.
