...
Then, if $|\alpha\rangle$ is the $\hat{\mathfrak{u}}_1$-primary state of $\mathcal{V}_\alpha$, we find $\left< J_{-1}\alpha\middle|J_{-1}\alpha\right> = \frac12 \operatorname{sign}(Q^2) \langle \alpha|\alpha\rangle$.
So, for the scalar product to be positive definite, we need $Q\in \mathbb{R}$.
Then $J_0$ is an operator such that $J_0^\dagger = Q-J_0$, with the eigenvalue $\alpha$. This implies $\bar\alpha = Q-\alpha$. 
...
The $\hat{\mathfrak{u}}_1$ fusion rules can be derived by 
...
The OPE then reads 
\begin{align}
\boxed{V_{\alpha_1}(z_1)V_{\alpha_2}(z_2) 
= 
|z_1-z_2|^{-4\alpha_1\alpha_2} \big( V_{\alpha_1+\alpha_2}(z_2) + O(z_1-z_2)\big)}\ ,
\label{vvoo}
\end{align}
...
\begin{align}
 V_{\alpha_1}(z_1)V_{\alpha_2}(z_2) = |z_1-z_2|^{-4\alpha_1\alpha_2}\exp\left(-\sum_{p=1}^\infty \frac{2}{p}\alpha_1 \left(z_{12}^pJ_{-p}+\bar{z}_{12}^p \bar{J}_{-p}\right) \right) V_{\alpha_1+\alpha_2}(z_2) \ .
\end{align}
...
This means that conformal spins must be integer, which leads to $(\alpha-\bar{\alpha})(\alpha+\bar{\alpha})\in {\mathbb{Z}}$.
