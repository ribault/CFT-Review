...
\begin{align}
 \boxed{ \mathcal{K}\left\langle \prod_{i=1}^N \Phi^{j_i}_{X_i}(z_i)\right\rangle \sim \delta(Y_1) Y_2 \Theta^{\frac12 b^2} \left\langle \prod_{i=1}^N V_{\alpha_i}(z_i)\prod_{j=1}^{N-2}V_{\langle 2,1\rangle}(y_j)\right\rangle}\ .
\label{dyy}
\end{align}
This equation means that the differential equations obeyed by both sides coincide.
Solutions of the KZ equations, in other words $N$-point \textbf{\boldmath $\widehat{\mathfrak{sl}}_2$ conformal blocks}\index{sl2hat conformal blocks@$\widehat{\mathfrak{sl}}_2$ conformal block}, are therefore related to certain Virasoro $(2N-2)$-point conformal blocks. 
...
Since Liouville theory exists for all complex values of the central charge, we expect that the $H_3^+$ model exists for all values of the level except $k=-2$, when the Sugawara construction breaks down and the limit of the model is no longer a conformal field theory.
...
\begin{align}
 ...
 \left|{\textstyle\sum_{i=1}^N\mu_iz_i}\right|^2 |\Theta|^{-k-2} 
...
\begin{align}
 U_g\Phi^j_{x} = |cx+d|^{4j}\Phi^j_{\frac{ax+b}{cx+d},\frac{\bar{a}\bar{x}+\bar{b}}{\bar{c}\bar{x}+\bar{d}}} \quad \text{with} \quad g = \left(\begin{array}{cc} a & b \\ c & d \end{array}\right) \in SL_2({\mathbb{C}})\ .
\label{ugp}
\end{align}
...
\end{align}
This suggests that we identify the large level limit of the field $\Phi^j_{x}$ with the function $\Phi^j_{x}(h)$. 
...
In the large level limit, the OPE of the fields $\Phi^j_{x}$ must remain associative and commutative. 
...
$\mathcal{F}(H_3^+)$ which is spanned by the functions $\Phi^j_{x}$.
...
Calling $C^j$ the \textbf{\boldmath principal series representation of $\mathfrak{sl}_2({\mathbb{C}})$}\index{principal series representation of $\mathfrak{sl}_2({\mathbb{C}})$} of spin $j\in -\frac12 +i{\mathbb{R}}$, which is defined by the action of $\mathfrak{sl}_2({\mathbb{C}})$ on $\Phi^j_{x}$, we obtain
\begin{align}
...
Notice that the large level limit of the $H_3^+$ $N$-point function is formally identical to the light asymptotic limit of the Liouville $N$-point function \eqref{zih}, whose interpretation is however quite different as it depends on positions $z$ instead of isospin variables $x$.
