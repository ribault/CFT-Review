...
\item in Section \ref{secltf}, the systematic discussion of the limits of Liouville theory and (generalized) minimal models,
...
\end{hyp} 
\noindent
The commutativity axiom implies that the OPE is \textbf{associative}\index{associativity (of the OPE)} and commutative. However, we wrote the OPE in a way that makes commutativity nontrivial, by using fields $V_\sigma(x_2)$. Had we used $V_\sigma(\frac{x_1+x_2}{2})$ instead, commutativity would have reduced to the simple condition $C_{\sigma_1,\sigma_2}^{\sigma}(x_1,x_2) = C_{\sigma_2,\sigma_1}^{\sigma}(x_2,x_1)$.
...
\end{align}
 We then summarize limits where the momentums or the central charge go from generic to discrete values. The limits are denoted as $\sim$ when they hold only for a subset of four-point functions. 
\begin{align}
...
\end{align}
Much work remains to be done in order to fully understand these limits.
