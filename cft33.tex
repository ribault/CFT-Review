...
\label{somr}
\end{align}
Here we have decomposed $S$ as a combination of some basic representations $R$ of the symmetry algebra, and the number $m_R \in {\mathbb{N}}\cup\{\infty\}$ is the \textbf{\boldmath multiplicity}\index{multiplicity (of a representation)} of the representation $R$ in the spectrum $S$.
...
In particular, for the dependence on $v_i$ to be completely determined by symmetry, $R_i$ must be an \textbf{\boldmath indecomposable representation}\index{indecomposable representation}, i.e. a representation that cannot be written as a direct sum of smaller representations.
\begin{align}
 R \ \ \text{indecomposable} \quad \iff \quad \nexists R',R'' \neq 0 \quad | \quad R = R'\oplus R''\ .
\end{align}
...
\begin{align}
 R \ \ \text{irreducible} \quad \iff \quad \nexists R' \neq 0,R \quad | \quad R'\subset R\ .
\end{align}
...
\label{ope}
\end{align}
where the \textbf{\boldmath OPE coefficients}\index{OPE!---coefficient} $C_{\sigma_1,\sigma_2}^{\sigma}(x_1,x_2)$ are $\mathbb{C}$-valued functions, and the sum runs over a basis of the spectrum $S$. 
...
\begin{align}
 \left\langle \prod_{i=1}^4 V_{\sigma_i}(x_i) \right\rangle = \sum_{\sigma\in S} C_{\sigma_1,\sigma_2}^{\sigma}(x_1,x_2)\sum_{\sigma'\in S} C_{\sigma,\sigma_3}^{\sigma'}(x_2,x_3)\Big\langle V_{\sigma'}(x_3)V_{\sigma_4}(x_4)\Big\rangle\ .
\end{align}
...
\label{clcf}
\end{align}
where $C_{R_1,R_2}^{R_3,\lambda}(x_1,x_2)$ is a model-dependent structure function, and 
...
\begin{align}
V_{\sigma_1}(x_1)V_{\sigma_2}(x_2)  =  \sum_{\sigma\in S} \Big\langle \sigma^* \Big| V_{\sigma_1}(x_1)V_{\sigma_2}(x_2)  \Big\rangle  V_{\sigma}(x_2) \ .
\end{align}
...
To do this, use the decomposition $R=\oplus_\Delta R_\Delta$ where $R_\Delta$ is the generalized eigenspace associated with the eigenvalue $\Delta$.
...
\begin{enumerate}
 \item Construct the action of the Virasoro algebra on the spectrum $S\otimes S'$ of the product theory by 
\begin{align}
...
\label{lvlv}
\end{align}
Since $R$ is irreducible, $R$ coincides with the subrepresentation that is generated by $|\Delta\rangle$. We denote this subrepresentation as $U(\mathfrak{V})|\Delta\rangle$, where $U(\mathfrak{V})$ is the universal enveloping algebra of the Virasoro algebra, i.e. the associative algebra that is generated by the Virasoro generators $L_n$.
...
If a Verma module has several singular vectors, then it also has several corresponding subrepresentations, and therefore a number of possible cosets, depending on which subrepresentation is chosen as the denominator representation $R'$ in eq. \eqref{rvrp}.
...
Exercise \ref{exosv}.) We define the \textbf{\boldmath maximally degenerate representation}\index{degenerate representation!maximally---} $R_{\langle r,s \rangle}$ as the coset of $\mathcal{V}_{\Delta_{\langle r,s \rangle}}$ by the subrepresentation generated by all the singular vectors. 
...
Given a basis $\{v_i\}$ of the level-$N$ eigenspace $R_N$, the scalar product on $R_N$ is characterized by the \textbf{Gram matrix}\index{Gram matrix} $M^{(N)}$ defined by $M^{(N)}_{ij}=\langle v_i|v_j\rangle$, and in particular the scalar product is positive definite if and only if all the eigenvalues of $M^{(N)}$ are strictly positive. This implies in particular $\det M^{(N)}>0$.
...
\begin{align}
\boxed{ R_{\langle 2,1 \rangle}\times \mathcal{V}_P = \sum_\pm \mathcal{V}_{P \pm \frac{b}{2}} }\quad , \quad 
\boxed{ R_{\langle 1,2 \rangle}\times \mathcal{V}_P = \sum_\pm \mathcal{V}_{P \pm \frac{1}{2b}} }\ .
\label{rot}
\end{align}
...
We now assume that the fusion rules are associative, and proceed to deduce $R_{\langle r,s \rangle}\times \mathcal{V}_P$.
...
Then, using the associativity of the fusion products $R_{\langle 2,1\rangle}\times R_{\langle r,s\rangle}\times \mathcal{V}_P$ and $R_{\langle 1,2\rangle}\times R_{\langle r,s\rangle}\times \mathcal{V}_P$, a recursion on $r$ and $s$ leads to
\begin{align}
 \boxed{R_{\langle r,s \rangle}\times \mathcal{V}_P = \sum_{i=-\frac{r-1}{2}}^{\frac{r-1}{2}} \sum_{j=-\frac{s-1}{2}}^{\frac{s-1}{2}} \mathcal{V}_{P + ib+jb^{-1}}}
 \ ,
\label{rtv}
\end{align}
...
\begin{exo}[Singular vectors that are not of the type $|\chi_{\langle r,s \rangle}\rangle$]
 ~\label{exosv}
Let us consider the Virasoro algebra with the Liouville coupling constant $b^2=-\frac{q}{p}$ where $p,q$ are strictly positive integers.
\begin{enumerate}
 \item 
...
\begin{enumerate}
 \item 
Assume that $R$ is indecomposable but reducible: what does this mean for the action of $L_0$ on $E$, and the corresponding matrix? 
\item
...
\begin{align}
 \boxed{ S= \frac12\int_{i{\mathbb{R}}} dP\ \mathcal{V}_P \otimes \overline{\mathcal{V}}_P =\int_{i{\mathbb{R}_+}} dP\ \mathcal{V}_P \otimes \overline{\mathcal{V}}_P = \int_{\frac{c-1}{24}}^\infty d\Delta\ \mathcal{V}_\Delta\otimes \overline{\mathcal{V}}_\Delta} \ ,
\label{sad}
\end{align}
...
\end{align}
where the function $N(P)$ transforms as $N(P)\to \lambda(P)N(P)$, and the renormalized three-point structure constant $C'_{P_1,P_2,P_3}$ is invariant. Then the invariant quantities that we want to determine are $C'_{P_1,P_2,P_3}$, and the combination $N^2B^{-1}(P)$. As a check that these quantities are all we need, let us write the crossing symmetry equation for a four-point function of the type $\langle V_{P_1}V_{P_2}V_{P_3}V_{P_4}\rangle$. Adapting the crossing symmetry equation \eqref{csd} to the 
...
Our decomposition of the three-point structure constant does not completely define $N(P)$ and $C'_{P_1,P_2,P_3}$: we fix this ambiguity by assuming that the shift equation \eqref{eq:shift} splits into the two equations 
\begin{align}
...
We have determined the normalization-independant combinations of structure constants, and this is enough for solving Liouville theory in the conformal bootstrap approach. However, in order to compare different approaches to Liouville theory, it can be convenient to consider specific field normalizations. 
...
up to a redefinition of the cosmological constant. These normalizations are such that the later equation \eqref{crs} will hold, and such that $C_-(P)=1$. (See Exercise \ref{exodoc}.)
...
\begin{align}
 \hat C_{P_1,P_2,P_3} =  \frac{\mu^{-\frac{Q}{2}-P_1-P_2-P_3}\hat\Upsilon_b(0)\prod_{i=1}^3 \hat\Upsilon_b(-2P_i) }{\prod_{\pm,\pm} \hat\Upsilon_b\left(\tfrac{Q}{2}+P_1\pm P_2 \pm P_3\right)} \ .
\label{hc}
\end{align}
Then $\hat C_{P_1,P_2,P_3}$ has the same behaviour as $C_{P_1,P_2,P_3}$ under the shifts $P_1\to P_1+b$ and $P_1\to P_1+\frac{1}{b}$. Moreover, we have $\hat C_{-\frac{Q}{2},-\frac{Q}{2},\frac{Q}{2}} = 1$.
...
\end{align}
where $B(P)=R(P)^2B(-P)$. Moreover, we must have $N(P)=R(P)N(-P)$ and $C_{P_1,P_2,P_3} = R(P_1) C_{-P_1,P_2,P_3}$. On the other hand, normalization-independent quantities are invariant under reflection.
...
\subsection{Analytic continuation}\label{secacl}

We will now sketch how OPEs and correlation functions behave when momentums are analytically continued beyond the imaginary axis. In particular, we will study what happens if momentums reach the values that correspond to degenerate fields, and this will allow us to determine the three-point structure constants of minimal models.

\subsubsection{Analytic continuation of the OPE}
...
\end{align}
When we analytically continue $P_1,P_2$ beyond the imaginary axis, the behaviour of this OPE depends on the behaviour of the singularities of the OPE coefficient  as a function of $P$. In the case $c\in ]-\infty, 1]$, the poles of the OPE coefficient $\frac{\hat C_{P_1,P_2,P}}{\hat B(P)}$ do not depend on $P_1,P_2$, and our OPE remains valid as written for any $P_1,P_2\in\mathbb{C}$ such that $\hat N(P_1)$ and $\hat N(P_2)$ are finite.
On the other hand, in the case $c\notin ]-\infty,1]$, the three-point structure constant has $P_1,P_2$-dependent simple poles due to the normalization-independent factor $C'_{P_1,P_2,P}$ \eqref{cp}. 
Let us plot these poles in the complex $P$-plane, assuming $P_1,P_2\in i\mathbb{R}$. The poles lie on cones that we draw in the fashion of eq. \eqref{lines},
and we also draw the line  of integration $P\in i{\mathbb{R}}$, together with the two lines $P\in \pm\frac{Q}{2}+i\mathbb{R}$:
\begin{align}
 \newcommand{\polewedge}[3]{
\begin{scope}[#1]
\node[blue, draw,circle,inner sep=1pt,fill] at (0, 0) {};
...
\end{scope}
 ...
 \end{tikzpicture}
\end{align}
If we continue $P_1,P_2$ such that $|\Re(P_1+P_2)|>\frac{Q}{2}$ or $|\Re(P_1-P_2)|>\frac{Q}{2}$, then some poles cross the line of integration. Calling $\mathcal{P}_{P_1,P_2}$ the finite set of these poles, the OPE becomes
\begin{align}
 V_{P_1}V_{P_2} \sim \frac12\int_{i{\mathbb{R}}} dP \frac{C_{P_1,P_2,P}}{B(P)} V_P + \frac12 \sum_{P_k\in \mathcal{P}_{P_1,P_2}} \frac{\underset{P=P_k}{\operatorname{Res}} C_{P_1,P_2,P}}{B(P_k)} V_{P_k}\ .
\end{align}
For an example of the explicit determination of the set $\mathcal{P}_{P_1,P_2}$, see Exercise \ref{exoaur}.




Then go to degenerate fields, we have been generic


\subsubsection{Analytic continuation to degenerate momentums}
...
This behaviour depends on the value of the central charge $c$, because the structure constants $C$ and $\hat C$ have very different analytic properties. 
...
So the OPE remains an integral over the Liouville spectrum, and nothing particular happens. 
...
We define an \textbf{A-series minimal model}\index{minimal model!A-series---} as a conformal field theory such that
\begin{itemize}
...
We would obtain \textbf{\boldmath generalized minimal models}\index{minimal model!generalized---}, whose spectrums contain all degenerate representations, and which exist for arbitrary values of the central charge $c$. 

What if we insist on the original assumption of rationality? We will see that there are finite sets 
of degenerate representations that close under fusion.
Such sets are made of
\textbf{\boldmath doubly degenerate representations}\index{degenerate representation!doubly---}, that is maximally degenerate representations of the type $R_{\langle r,s \rangle}=R_{\langle r',s' \rangle}$ with $\langle r,s \rangle\neq \langle r',s' \rangle$.
...
But we already found the correct regularization when we expressed degenerate OPE coefficients in terms of $C$ \eqref{crs}, after sending one momentum to a degenerate value. 
...
\label{clc}
\end{align}
For these equalities to hold, we need the three degenerate fields to obey the fusion rules \eqref{rrrsss}.
...
Some of the minimal model structure constants however correspond to values of $\langle r_i,s_i\rangle$ that violate the fusion rules of the generalized minimal models -- for instance, $C_{\langle 1,1\rangle,\langle 1,1\rangle,\langle p-1,q-1\rangle}$. 

The two-point function of a minimal model is 
\begin{align}
...
\end{align}
There is an additional subtlety in the case of the three-point function: if we want $\tilde{V}_{\langle 1,1 \rangle}$ to be the identity field, we need to renormalize the three-point structure constant with a factor $\sqrt{R(\alpha_{\langle 1,1 \rangle})}$ on top of the factors coming from the field renormalizations, so that 
\begin{align}
...
\begin{exo}[Fusion rules of all unitary representations if $c>1$]
 ~\label{exoaur}
Assuming $Q>0$, compute the analytic continuation of the Liouville OPE to momentums $\Pi\in ]0,\frac{Q}{2}[$, which correspond to unitary representation that do not belong to the spectrum \eqref{sad}. 
Show that this leads to the following fusion rules:
\begin{align}
 \mathcal{V}_\Pi \times \mathcal{V}_P  
&= \frac12 \int_{i\mathbb{R}} dP'\ \mathcal{V}_{P'}\ ,
\\
...
+ \frac12 \int_{i\mathbb{R}} dP'\ \mathcal{V}_{P'}\ .
\end{align}
In particular, show that $\mathcal{V}_\Delta \times \mathcal{V}_\Delta$ belongs to the Liouville spectrum if $\Delta \geq \frac{c-1}{32}$, and has discrete terms if $\Delta \in ]0,\frac{c-1}{32}[$.
\end{exo}
...
\begin{align}
 ...
 \label{amba}
\end{align}
Since any two vectors $\vec\alpha_1,\vec\alpha_2$ in the spectrum obey this condition, there must be an element of $\mathbb{Z}\vec\alpha_1 + \mathbb{Z}\vec\alpha_2$ that is diagonal. And actually, the compactified free boson spectrum $S_R$ \eqref{sr} is made of states with integer spins provided
\begin{align}
...
Given a representation $R$ of $\mathfrak{g}$, we define an \textbf{\boldmath affine primary field}\index{primary field!affine---} $\Phi^{R}(z_0)$ by its OPE with $J^a(y)$,
\begin{align}
...
\label{lmp}
\end{align}
Let us now consider the states $|v^R\rangle$ that correspond to the $R$-valued field $\Phi^R(z_0)$ by the state-field correspondence.
...
Acting on the affine primary states with creation operators $J^a_{n<0}$ generates affine descendent states, which form an \textbf{\boldmath affine highest-weight representation}\index{highest-weight representation!affine---} $\hat{R}$ of the affine Lie algebra $\hat{\mathfrak{g}}$.
...
For generic representations $R_i$, the multiplicity $m_{R_1,R_2}^{R_3}$ is the number of linearly independent solutions of eq. \eqref{dddc} in the absence of further constraints, namely
\begin{align}
...
\end{align}
The action $U_g$ is characteristic of the \textbf{\boldmath principal series representation of $\mathfrak{sl}_2({\mathbb{C}})$}\index{principal series representation!of sl2c@---of $\mathfrak{sl}_2({\mathbb{C}})$} $C^j$. 
...
a WZW model can be associated with a particular Lie group $G$, such that its spectrum $S$ obeys
\begin{align}
...
\end{align}
Here $\hat{R}$ is the affine highest-weight representation of $\hat{\mathfrak{g}}$ that is built from $R$ by acting with the creation modes and removing the null vectors, and the finite subset $\mathcal{R}_k$ of $\mathcal{R}$ is defined by certain $k$-dependent conditions.
The resulting representations $\hat{R}$ are called the integrable highest-weight representations of $\hat{\mathfrak{g}}$. 
We have $\underset{k\to\infty}{\lim} \mathcal{R}_k =\mathcal{R}$ and $\underset{k\to \infty}{\lim} \hat{R} = R$, so that the spectrum $S$ has the desired large level limit $\mathcal{F}(G)$.
...
corresponding affine highest-weight representations $\hat R^{\langle 0,s\rangle}$ of $\widehat{\mathfrak{sl}}_2$ are degenerate, with level zero null vectors.
Their fusion rules are 
\begin{align}
 \hat{R}^{\langle 0, s_1\rangle}\times\hat{R}^{\langle 0, s_2\rangle} = \sum_{s\overset{2}{=}|s_1-s_2|+1}^{s_1+s_2-1} \hat{R}^{\langle 0, s\rangle}\ .
\end{align}
For any value of the level $k\in\mathbb{C}-\{-2\}$, we can build a diagonal spectrum from these representations,
\begin{align}
 S = \bigoplus_{s=1}^\infty \hat{R}^{\langle 0, s\rangle}\otimes \bar{\hat{R}}^{\langle 0, s\rangle}\ ,
\end{align}
...
As we did in the case of Virasoro minimal models, let us consider multiply degenerate representations of the type $\hat{R}^{\langle 0, s\rangle}$. 
...
\begin{align}
 S = \bigoplus_{s=1}^{k+1} \hat{R}^{\langle 0, s\rangle}\otimes \bar{\hat{R}}^{\langle 0, s\rangle}\ .
\end{align}
Fusion is constrained by both null vectors, and becomes 
\begin{align}
 \hat{R}^{\langle 0, s_1\rangle}\times\hat{R}^{\langle 0, s_2\rangle} = \sum_{s\overset{2}{=}|s_1-s_2|+1}^{\min(s_1+s_2-1,2k+3-s_1-s_2)} \hat{R}^{\langle 0, s\rangle}\ .
\end{align}
...
\label{ror}
\end{align}
where $N_{R_1,R_2,R_3}$ is an integer-valued function that obeys $N_{R_1^*,R_2^*,R_3^*}=N_{R_1,R_2,R_3}$ and is symmetric under permutations.
...
\end{align}
Given a representation $R$ of $\widehat{\mathfrak{sl}}_2$, i.e. an action of the generators $J^a_n$ on some vectors $|v\rangle$, we define the \textbf{\boldmath spectrally flowed representation}\index{spectrally flowed representation} $\rho_w(R)$ by the action $\rho_{-w}(J^a_n)|v\rangle$.  
...
\end{align}
where the coefficients $N_{R_1,R_2,R_3}$ are a permutation-symmetric, integer-valued fusion multiplicities such that $N_{R_1^*,R_2^*,R_3^*}=N_{R_1,R_2,R_3}$.
From eq. \eqref{rwr}, we must then have 
\begin{align}
w_1+w_2+w_3=0 \quad \implies \quad N_{\rho_{w_1}(R_1),\rho_{w_2}(R_2),\rho_{w_3}(R_3)}=N_{R_1,R_2,R_3}\ .
\label{nrrr} 
\end{align}
...
\begin{exo}[Associativity of the $J^aJ^b\Phi^R$ OPE]
 ~\label{exojjp}
Let us check the associativity of the $J^aJ^b\Phi^R$ OPE, by performing two different computations of the behaviour near $y_2=z_0$ of 
\begin{align}
...
In the second term, use the $J^a(y_1)\Phi^R(z_0)$ OPE, and show that
\begin{align}
...
\item
Use $J^a(y_1) t^b \Phi^R(z_0) =t^b J^a(y_1)  \Phi^R(z_0)$, and check that you recover eq. \eqref{oftp}.
