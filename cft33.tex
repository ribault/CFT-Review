...
It turns out that the second-order BPZ equation is the only meromorphic second-order differential equation with three regular singular points at $x=0,1,\infty$, and characteristic exponents of the type \eqref{lpm}. So this equation can in principle be derived from the fusion rules, without knowing the level two null vectors. 

rigid Fuchsian system
...
\\ 
\iff\quad  \text{hypergeometric\ equation\ for\quad }&  \mathcal{G}(x)= x^{-bP_1}(1-x)^{-bP_3}\mathcal{F}(x) 
\\
...
\begin{align}
\renewcommand{\arraystretch}{1.3}
\left\{\begin{array}{l}   A = \frac12 + b(P_1+P_3-Q) + b(P_2-\tfrac{Q}{2}) \ , \\
      B = \frac12 + b(P_1+P_3-Q) - b(P_2-\tfrac{Q}{2}) \ , \\
      C = 1 + b(2P_1-Q) \ ,
\end{array}\right. 
\label{abc}
\end{align}
or equivalently $P_i = -b^{-1}j_i +\frac{b}{2}$.
...
\begin{align}
\left\{\begin{array}{l}  \mathcal{F}^{(s)}_+(x) = x^{bP_1} (1-x)^{bP_3} F(A,B,C,x)\ ,
\\ \mathcal{F}^{(s)}_-(x)  =x^{b(Q-P_1)} (1-x)^{bP_3} F(A-C+1,B-C+1,2-C,x)= \left. \mathcal{F}^{(s)}_+(x) \right|_{P_1\to Q-P_1} \ ,
\end{array}\right. 
\label{gpm}
\end{align}
...
\begin{align}
 \left\{\begin{array}{l}  \mathcal{F}^{(t)}_+(x) = x^{bP_1} (1-x)^{bP_3} F(A,B,A+B-C+1,1-x) \ ,
\\ \mathcal{F}^{(t)}_-(x) =x^{bP_1} (1-x)^{b(Q-P_3)} F(C-A,C-B,C-A-B+1,1-x)\ ,
\end{array}\right.  
\end{align}
...
\begin{align}
 \mathcal{F}^{(t)}_\pm(x)  =  
 \begin{tikzpicture}[baseline=(current  bounding  box.center), very thick, scale = .4]
 \draw (-2,3) node [left] {$P_1$} -- (0,2) -- node [left] {$P_3\mp \frac{b}{2}$} (0,-2);
...
\draw[dashed] (0, -2) -- node [below left] {$\langle 2,1 \rangle$} (-2, 0);
\draw (-2, 0) -- node [below] {$P_3\mp \frac{b}{2}$} (2, 0); 
\end{tikzpicture}
\ \ .
\label{tpic}
\end{align}
