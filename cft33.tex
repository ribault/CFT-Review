...
$N$-point functions of the descendent fields are of the type $\left\langle \prod_{i=1}^N \left(\left(\prod_{j=1}^{p_i} L_{-n_{i,j}}\right)V_{\Delta_i}(z_i)\right) \right\rangle$, with $p_i\geq 0$ and $n_{i,j}\geq 1$. 
...
\begin{hyp}[Dependence of fields on the position]
 ...
 \begin{align}
 \boxed{{\frac{\partial}{\partial z} V_\sigma(z)} = L_{-1} V_\sigma (z) }  \ .
\label{lvpv}
\end{align}
\end{hyp}
\noindent
...
\end{align}
Up to a $z_i$-independent factor, the solution is 
\begin{align}
 \mathcal{F}^{(2)}(\Delta_1|z_1,z_2) = z_{12}^{-2\Delta_1}\ .
\label{fzz}
\end{align}
The $z_i$-independent factor can be set to one by a choice of normalization of the fields $V_{\Delta,\bar{\Delta}}(z,\bar{z})$, so that 
\begin{align}
...
and we use the modulus square notation in the sense $|f(\Delta,z)|^2 = f(\Delta,z)f(\bar{\Delta},\bar{z})$.
