...
Let us for simplicity specialize to an OPE of two primary fields $V_{\Delta_1}(z_1)V_{\Delta_2}(z_2)$. Omitting the dependence on right-moving quantities $\bar z_i,\bar \Delta_i$, the local Ward identities read 
\begin{align}
...
\end{align}
Focussing on the right-hand side terms that come from descendents $V_\sigma= LV_\Delta$ of a given primary field at a level $|L|=N$, we find the identities of elements of $U(\mathfrak{V}^+)$ 
\begin{align}
...
Let us determine these coefficients for $|L|\leq 2$, temporarily abbreviating them as $f^L$. The cases $(N,n)=(1,1),(2,2),(2,1)$ of \eqref{eq:lwo} respectively yield 
\begin{align}
 f^1(\Delta + \Delta_1-\Delta_2) & = 2\Delta f^{L_{-1}} \ ,
 \\
 f^1(\Delta+ 2\Delta_1-\Delta_2) & = 6\Delta f^{L_{-1}^2} + (4\Delta+\tfrac{c}{2})f^{L_{-2}}\ ,
 \\
 (\Delta+1+\Delta_1-\Delta_2) f^{L_{-1}}L_{-1} & = \left[2(2\Delta+1) f^{L_{-1}^2} + 3 f^{L_{-2}}\right] L_{-1}\ .   
\end{align}
In the case

Further assuming that the fields $V_\sigma$  $V_\sigma = LV_\Delta$ with $L\in U(\mathfrak{V}^+)$ is a descendent field, 

Local Ward identities determine the OPE coefficients of descendent fields $V_\sigma$ in terms of the coefficients of the corresponding primary fields.
Before showing this in the case of low level descendent fields, let us see how it affects the structure of the OPEs. Taking for simplicity $V_{\sigma_1}$ and $V_{\sigma_2}$ to be primary fields, the OPE reads


This property implies that fusion multiplicities are trivial in fusion products of highest-weight representations. If Ward identities determined $f^L$ in terms of say $f^1$ and $f^{L_{-1}}$, then the fusion multiplicity would be two. 



The fields which appear in OPEs will then be denoted as $V_{s,\bar{s}}(z,\bar{z}) = L\bar{L} V_{\Delta,\bar{\Delta}}(z,\bar{z})$, where $s=(\Delta,L)$ is made of a conformal dimension $\Delta$, and an element $L$ of the basis $\mathcal{L}$ \eqref{lels} of $U(\mathfrak{V}^+)$.
...
\label{vvlv}
\end{align}
Constraints from Virasoro symmetry can be obtained by inserting 
