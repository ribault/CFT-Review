...
\label{ffm}
\end{align}
This formulation of crossing symmetry can be useful, because the fusing matrix is a simpler object than the conformal blocks, as we will see in a special case in Section \ref{sechcb}.
...
\begin{multline}
 V_{P_1}(z_1) V_{P_2}(z_2) = \int_{\frac{Q}{2}+i{\mathbb{R}_+}} dP\ C_{P_1,P_2}^P |z_{12}|^{2(\Delta-\Delta_1-\Delta_2)} 
\left|
...
\label{vvvs}
\\
 &\boxed{V_{P_1}V_{P_2} \sim \int_{\frac{Q}{2}+i{\mathbb{R}_+}} dP\ \frac{C_{P_1,P_2,P}}{B(P)} V_P}\ .
\label{vvi}
\end{align}
...
Let us now investigate the consequences of the associativity of the OPE, whose general form was given in eq. \eqref{cccc}. In order to constrain the three-point structure constant, we will study four-point functions that involve one degenerate field $V_{\langle 2,1\rangle}$ or $V_{\langle 1, 2\rangle}$. We will therefore need to constrain the OPE coefficients of these degenerate fields. 
To do this, we will consider four-point functions that involve two degenerate fields. 


\subsubsection{What we learn from $
 \left\langle V_{\langle 2,1 \rangle}(x) V_P(0) V_{\langle 2,1 \rangle}(\infty) V_{Q-P}(1) \right\rangle$}
...
-- node [left] {$P\mp \frac{b}{2}\!  $} (0, -2)
-- node [below right] {\!$Q-P$} (2, 0);
\end{tikzpicture}
\ .
\end{align}
The coefficients $c^{(s)}_\pm$ of this decomposition can be expressed in terms of the OPE structure constants
\begin{align}
 c^{(s)}_+ &= C_+(P)C_-(P-\tfrac{b}{2}) = \frac{R(P-\tfrac{b}{2})}{R(P)}\ ,
\\
c^{(s)}_- & = C_-(P)C_+(P+\tfrac{b}{2}) = \frac{R(P)}{R(P+\tfrac{b}{2})}\ ,
\end{align}
where we used the equations \eqref{cmcp} and \eqref{cco}.
The coefficients $c^{(s)}_+$ and $c^{(s)}_-$ are related by eq. \eqref{spsm}, with
\begin{align}
 \left\{\begin{array}{l}  P_1 = P \\ P_2 = -\frac{b}{2} \\ P_3 = Q-P \end{array}\right. \quad \implies \quad \left\{\begin{array}{l}  A = 1-bQ \\ B = bQ \\ C=1+b(2P-Q) \end{array}\right.  \ ,
\end{align}
so that $A+B=1$. We thus obtain
\begin{align}
 \frac{R(P-\frac{b}{2})R(P+\frac{b}{2})}{R(P)^2} 
 = \frac{\gamma(2bP)\gamma(b(Q-2P))}{\gamma(b(2Q-2P))\gamma(b(2P-Q))}\ .
\end{align}
...
\begin{align}
 \frac{R(P-\tfrac{1}{2b})R(P+\tfrac{1}{2b})}{R(P)^2} 
 = \frac{\gamma(\frac{1}{b}2P)\gamma(\frac{1}{b}(Q-2P))}{\gamma(\frac{1}{b}(2Q-2P))\gamma(\frac{1}{b}(2P-Q))}\ .
\end{align}
The general solution of these equations, together with \eqref{rrr}, is
\begin{align}
 \boxed{R(P) = R(\tfrac{Q}{2})\mu^{Q-2P} \frac{\Gamma(b(2P-Q))\Gamma(\frac{1}{b}(2P-Q))}{\Gamma(b(Q-2P))\Gamma(\frac{1}{b}(Q-2P))}} \quad \text{with} \quad R(\tfrac{Q}{2})\in \{1, -1\}  \ ,
\label{ram}
\end{align}
...
Using eq. \eqref{cmcp}, this determines in particular $C_-(P)$, so that we have
\begin{align}
 C_+(P)=1 \quad , \quad C_-(P) = \mu^b b^4 \frac{\gamma(b(2P-Q))}{\gamma(2bP)}\ .
\label{cpm}
\end{align}
We will need these OPE coefficients in order to determine the three-point structure constant. 
However, we would not need these OPE coefficients 
if we only wanted to determine the $P_s$-dependence of the combination $C_{P_1,P_2,Q-P_s}C_{P_s,P_3,P_4}$. (See Exercise \ref{exocas}.)
...
\mathcal{F}^{(s)}_\pm = 
\begin{tikzpicture}[baseline=(current  bounding  box.center), very thick, scale = .8]
\draw (2, 0) -- node [above right] {\!$P_2$} (0, 2);
\draw[dashed] (-2, 0)
-- node [below left] {$\langle 2,1 \rangle$\!} (0, -2); 
\draw (-2, 0) -- node [above left] {$P_1$\!} (0, 2) 
-- node [left] {$P_1\mp \frac{b}{2}\!  $} (0, -2)
-- node [below right] {\!$P_3$} (2, 0);
\end{tikzpicture}
\ .
\end{align}
The coefficients $c^{(s)}_\pm$ of this decomposition are now 
\begin{align}
 c_+^{(s)} & = C_+(P_1) C_{P_1-\frac{b}{2},P_2,P_3} \ ,
\\
c_-^{(s)} & = C_-(P_1) C_{P_1+\frac{b}{2},P_2,P_3}\ .
\end{align}
The ratio of these coefficients is determined by eq. \eqref{spsm}. 
Using moreover the formulas \eqref{cpm} for the degenerate structure constants $C_\pm(P_1)$, we obtain
\begin{align}
 \frac{C_{P_1+b,P_2,P_3}}{C_{P_1,P_2,P_3}} = \mu^{-b} b^{-4}\frac{\gamma(2bP_1)\gamma(2bP_1+b^2)}{\prod_{\pm,\pm} \gamma\left(bP_1\pm b(P_2-\frac{Q}{2})\pm b(P_3-\frac{Q}{2})\right)}\ .
\label{fcc}
\end{align}
...
The two equations prescribe how the three-point structure constant $C_{P_1,P_2,P_3}$ behaves under shifts of $P_1$ by $b$ and $\frac{1}{b}$.
Next, we will discuss their solutions.

Additional crossing symmetry equations: Exercise \ref{exonorm}
...
Since we
use the momentum $P$ instead of the conformal dimension $\Delta$ for parametrizing representations, we a priori have two notations $V_P(z)$ and $V_{Q-P}(z)$ for the primary field that corresponds to the representation $\mathcal{V}_P \otimes \overline{\mathcal{V}}_P$.
We must therefore have a \textbf{\boldmath reflection relation}\index{reflection!---relation}
\begin{align}
 \boxed{V_P(z)= R(P) V_{Q-P}(z)}\ ,
\label{vrv}
\end{align}
where $R(P)$ is the \textbf{\boldmath reflection coefficient}\index{reflection!---coefficient}, which must obey 
\begin{align}
 R(P)R(Q-P)=1\ ,
\label{rrr}
\end{align}
...
where the coefficient $c(P)$ must obey $c(P)=c(Q-P)$.
Since both $c(P)$ and $R(P)$ can be absorbed in a field renormalization, 

The three-point structure constant $C_{P_1,P_2,P_3}$ is constrained by the reflection relation, for instance
\begin{align}
 C_{P_1,P_2,P_3} = R(P_1) C_{Q-P_1,P_2,P_3}\ .
\label{crc}
\end{align}
...
It is tempting to use a similar calculation for deriving the fusion rules of the unitary Verma modules that do not belong to the spectrum. 
...
\begin{exo}[OPE coefficients of degenerate fields]
 ~\label{exocpcm}
...
\begin{exo}[Allowing finite multiplicities in Liouville theory]
 ~\label{exomul}
We want to generalize the analysis of Section \ref{seccbe}, by relaxing the assumption that the multiplies are at most one, and allowing a finite multiplicity $m\geq 2$ instead. 
\begin{enumerate}
 \item 
