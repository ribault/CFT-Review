...
$P_{\langle r,s \rangle}=\pm P_{\langle r',s' \rangle}$. Using eq. \eqref{ars} for $P_{\langle r,s\rangle}$, this implies
\begin{align} 
b^2=-\frac{q}{p} \in \mathbb{Q} \quad \Rightarrow \quad c\in ]-\infty, 1] \cup [25,\infty[\ .
\end{align}
For $b^2=-\frac{q}{p}$, we have the relation
\begin{align}
...
In order to recover the known minimal models, it is enough to consider rational values of $b^2$ that obey a few additional assumptions. We first assume that the relation between the momentums is 
\begin{align}
 P_{\langle r,s \rangle}+P_{\langle r',s' \rangle} = 0\ .
\end{align}
This leads to $b^2=-\frac{q}{p}$ where the integers $p,q$ are positive,
\begin{align}
 p = r+r' \quad , \quad q = s+s'\ ,
\end{align}
and we further assume that they are coprime. 
After we introduce the Kac table, we will finally need to assume $p,q\geq 2$ in order for that table to be non-empty, and we will finally recover the values $c_{p,q}$ \eqref{cpq} of the central charges of minimal models.

\subsubsection{Kac table}

Let us investigate fusion rules involving doubly degenerate representations. 
Let us start with the fusion product $\mathcal{R}\times \mathcal{V}_P$ of a doubly degenerate representation $\mathcal{R}$ with a Verma module, which encodes constraints that apply to the fusion of $\mathcal{R}$ with 
