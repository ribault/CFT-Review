...
The corresponding degenerate representations do not belong to the spectrum, but we assume that OPEs and correlation functions of such fields nevertheless make sense. 
...
Finally, we assume that the OPE (\ref{vrsv}) still holds if $V_\alpha$ is replaced by the degenerate field $V_{\langle r',s'\rangle}$, and that the coefficients are simply $C_{i,j}^{\langle r,s \rangle}(\alpha_{\langle r',s'\rangle})$ where $\alpha_{\langle r',s'\rangle}$ is given by eq. \eqref{ars}. Similarly, the reflection coefficient for $V_{\langle r',s'\rangle}$ is assumed to be $R(\alpha_{\langle r',s'\rangle})$.
...
Finally, let us mention that Liouville theory with $c> 25$ is known to be consistent on the torus \cite{hjs09}, although its partition function is infinite. 
