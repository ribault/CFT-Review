...
This field normalization  is such that $N(P_{\langle r,s\rangle})=0$, and therefore $\lim_{P_2 \to P_{\langle r,s\rangle}} C_{P_1,P_2,P}=0$.
...
To see this, let us follow how the poles behave when $P_2 \to P_{\langle r,s\rangle}$. 


If we analytically continue the OPE to a value $P_2=P_{\langle r,s \rangle}$, then poles from different sides of the line of integration must coincide, so some poles have to cross the line. 
...
\end{scope}
 ...
 \end{tikzpicture}
\end{align}
The integral along the deformed line goes to zero as $P_2\to P_{\langle r,s \rangle}$, while the residues of the poles have finite limits.
We obtain an OPE of the type of eq. \eqref{vrsv}, with 
\begin{align}
 \boxed{C_{i,j}^{\langle r,s \rangle}(P_1) = \underset{P_2\to P_{\langle r,s \rangle}}{\lim}\ \underset{P=P_1+P_2+ib+jb^{-1}}{\operatorname{ Res}}\ C_{P_1,P_2,Q-P}}\ .
\label{crs}
\end{align}
To summarize, 
\begin{align}
 \boxed{
 \underset{P\to P_{\langle r,s \rangle}}{\lim } V_P = 
 \left\{ \begin{array}{ll}
          V_{P_{\langle r,s \rangle}} \quad & \text{if\ } c\in ]-\infty, 1]\ ,
          \\
...
Let us finish the computation of the degenerate OPE coefficient $C_{i,j}^{\langle r,s \rangle}(P)$.
Using eq. \eqref{caaa}, we obtain 
\begin{multline}
 C_{i,j}^{\langle r,s \rangle}(P) = \left[b^{\frac{2}{b}-2b}\mu\right]^{ib+jb^{-1}} \frac{\Upsilon_b'(0)}{\Upsilon_b'(-ib-jb^{-1})} \frac{\Upsilon_b'(2P_{\langle r,s \rangle})}{\Upsilon_b'(2P_{\langle r,s \rangle}+ib+jb^{-1})}
\\ \times
 \frac{\Upsilon_b(2P)}{\Upsilon_b(2P+ib+jb^{-1})} \frac{\Upsilon_b(2P+2P_{\langle r,s \rangle}-Q+2ib+2jb^{-1})}{\Upsilon_b(2P+2P_{\langle r,s \rangle}-Q+ib+jb^{-1})} \ ,
\end{multline}
...
\\
\frac{\left[b^{\frac{2}{b}-2b}\mu\right]^{Q-P_1-P_2-P_3}\Upsilon_b'(0) \Upsilon_b'(2P_1) \Upsilon_b'(2P_2) \Upsilon_b'(2P_3)}{\Upsilon_b'(P_1+P_2+P_3-Q) \Upsilon_b'(P_1+P_2-P_3)\Upsilon_b'(P_2+P_3-P_1)\Upsilon_b'(P_3+P_1-P_2)}\ ,
\label{crisi}
\end{multline}
where we use the notation $P_i = P_{\langle r_i,s_i \rangle}$ for short.
...
\begin{align}
 \forall i\neq j\neq k\ , \quad \Upsilon_b(P_i+P_j-P_k) = 0\ .
\end{align}
...
\item If $1<c<25$, show that the set of unitary representations is not closed under fusion.
 \end{enumerate}
