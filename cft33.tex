...
\label{pll}
\end{align}
This is equivalent to $\frac{\partial}{\partial z_0} T(y) = 0$, where we define the \textbf{\boldmath Virasoro field}\index{Virasoro field} $T(y)$ as 
\begin{align}
...
\label{tvp}
\end{align}
The field $T(y)$ is a \textbf{\boldmath symmetry field}\index{symmetry field}: a field which encodes the action of symmetry transformations, but does not necessarily correspond to a state in the spectrum. 
...
\label{dbd}
\end{align}
The number $\Delta-\bar{\Delta}$ is called the \textbf{conformal spin}\index{spin (conformal)} of the field $V_{\Delta,\bar\Delta}(z,\bar z)$, as it controls how that field behaves under rotations, $T_{\left(\begin{smallmatrix} e^{i\theta} & 0 \\ 0 & e^{-i\theta} \end{smallmatrix}\right)} V_{\Delta,\bar{\Delta}}(z,\bar{z}) = e^{2i\theta(\Delta-\bar\Delta)} V_{\Delta,\bar{\Delta}}(e^{2i\theta} z,e^{-2i\theta}\bar{z})$.
...
We assume that the spectrum \eqref{sad} that we obtained for $c>1$ also holds for any $c\in\mathbb{C}$ by analyticity in $\alpha$. 
...
A subtlety occurs if the line $\alpha \in \frac{Q}{2}+i\mathbb{R}$ contains degenerate values $\alpha_{\langle r, s\rangle}$, which happens if and only if $c\leq 1$. 
...
\begin{align}
 \left\langle V_{\alpha_1}(z_1,\bar{z}_1) V_{\alpha_2}(z_2,\bar{z}_2)\right\rangle = c(\alpha_1) \Big\{ \delta(Q-\alpha_1-\alpha_2) + R(\alpha_1)\delta(\alpha_1-\alpha_2)\Big\} |z_{12}|^{-4\Delta(\alpha_1)}\ , 
\label{vvc}
\end{align}
...
\label{tub}
\end{align}
which is therefore defined for $\Im b < 0$. 
For any $c\notin ]-\infty, 1] \cup [25,\infty[$, there is a choice of $b$ such that $\Re b>0$ and $\Im b<0$, and both functions $\Upsilon_b(x)$ and $\hat\Upsilon_b(x)$ exist. 
Since they obey essentially identical shift equations, their ratio is essentially an elliptic function \cite{zam05}. 
...
\item obey the the reflection relation \eqref{crc}, as follows from the identities \eqref{upq} and 
\begin{align}
 R(\alpha)
...
\subsection{Existence and uniqueness of Liouville theory}
...
This makes it impossible to get a four-point function for $c\in ]-\infty, 1]$ by taking a limit from the rest of the complex plane, or conversely to continue the four-point function from $c\in ]-\infty, 1]$ to a larger region.
...
For the other values of $c$, we have $b^2\notin\mathbb{R}$, and we can find alternative solutions of the equations \eqref{upup}, by multiplying $\Upsilon_b(x)$ with elliptic functions -- meromorphic functions $\theta(x)$ such that $\theta(x+b)=\theta(x+\frac{1}{b}) = \theta(x)$.
This leads to alternative solutions of the shift equations.
But these alternative structure constants have no reason to lead to crossing symmetric four-point functions, and it seems likely that Liouville theory is unique for any $c\in\mathbb{C}$, although this would be difficult to prove. 
...
These poles lie on cones, which we draw in the fashion of eq. \eqref{lines}, together with the line $\alpha\in \frac{Q}{2}+i{\mathbb{R}}$ of integration of the OPE, 
\begin{align}
 \newcommand{\polewedge}[3]{
\begin{scope}[#1]
\node[blue, draw,circle,inner sep=1pt,fill] at (0, 0) {};
...
To avoid the divergences, we deform the integration line to the right, while picking residues at poles that cross the line,
\begin{align}
...
\end{scope}
 ...
 \end{tikzpicture}
\end{align}
The integral along the deformed line of integration now remains convergent, and goes to zero as $\alpha_2\to \alpha_{\langle r,s \rangle}$.
...
\label{vla}
\end{align}
The behaviour for $c\notin ]-\infty, 1]$ is quite violent from a representation-theoretic point of view, as it suggests that the limit of a Verma module is a degenerate representation. 
Nevertheless, that behaviour is natural in the functional integral approach to Liouville theory, to the point that the milder limit that is observed for $c\in ]-\infty, 1]$ has sometimes been considered puzzling or problematic \cite{rs15}. 
...
\label{vvdd}
\end{align}
The natural regularization of the three-point structure constant \eqref{caaa} is suggested by the observation that the degenerate OPE coefficient $C_{i,j}^{\langle r,s \rangle}(\alpha_1)$ \eqref{crs} is perfectly finite, and has a well-defined limit when $\alpha_1\to \alpha_{\langle r_1,s_1 \rangle}$.
This suggests that the three-point structure constant of a generalized minimal model is 
\begin{align}
 C_{\langle r_1,s_1\rangle ,\langle r_2,s_2\rangle ,\langle r_3,s_3 \rangle} = \underset{\alpha_1\to \alpha_{\langle r_1,s_1 \rangle}}{\lim}\ 
...
While many four-point functions of generalized minimal models are limits of Liouville theory four-point functions and are therefore crossing-symmetric, there are cases when the $\alpha_i\to \alpha_{\langle r_i,s_i\rangle}$ limit of the Liouville four-point function yields spurious terms in addition to the four-point function of the generalized minimal model, due to the non-vanishing of $C_{\langle r_1,s_1\rangle ,\langle r_2,s_2\rangle ,\langle r_3,s_3 \rangle}$ in cases when the degenerate fusion rules are violated.
...
Solutions of the KZ equations, which are called \textbf{\boldmath $\widehat{\mathfrak{sl}}_2$ conformal blocks}\index{sl2hat conformal blocks@$\widehat{\mathfrak{sl}}_2$ conformal block}, are therefore related to certain Virasoro $2N-2$-point conformal blocks: the relation between individual blocks can be obtained by studying their behaviours in the $z_i\to z_{i'}$ limits.
...
\end{scope}
 ...
 \end{tikzpicture}
\end{align}
Taking the limit $j\to J$ or $j\to J-1-\frac{k}{2}$, the OPE coefficient vanishes, and the only surviving contributions are from pairs of poles which pinch the line of integration. (See Section \ref{sectpf} for more explanations.) The resulting OPEs are schematically written as 
\begin{align}
