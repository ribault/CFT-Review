...
If $\nabla_z={\frac{\partial}{\partial z}} +2\alpha J(z)$ and $\nabla_{\bar z}={\frac{\partial}{\partial \bar z}} +2\alpha \bar J(z)$, then we have $\nabla_z V_{\alpha,\alpha}(z)=\nabla_{\bar z} V_{\alpha,\alpha}(z)=0$, neglecting the issue of regularizing the operator products.
If we then define $\nabla_\alpha = {\frac{\partial}{\partial \alpha}}+2\int^z dyJ(y)+2\int^{\bar z}d\bar{y} \bar J(y)$ so that $[\nabla_z,\nabla_\alpha]=[\nabla_{\bar z},\nabla_\alpha]=0$, then $\nabla_\alpha V_{\alpha,\alpha}(z)$ is annihilated by $\nabla_z,\nabla_{\bar z}$, and must be proportional to $V_{\alpha,\alpha}(z)$.
...
\begin{align}
 ...
 = -2 \int^z dy\, J(y)V_{\alpha,\alpha}(z)-2 \int^{\bar z} d{\bar y}\, \bar J(y)V_{\alpha,\alpha}(z)\ ,
\label{swp}
\end{align}
where the integrals denote the regularize values of particular primitives of the integrands.
...
\begin{align}
 ...
 = \frac{1}{2\pi i} \oint_{z} dy\, J(y)V_{\alpha,\alpha}(z)
 = \frac{1}{2\pi i} \oint_{\bar z} d\bar y\, \bar J(y)V_{\alpha,\alpha}(z)\ .
\label{swa}
\end{align}
Equations \eqref{swp} and \eqref{swa} show that the actions of ${\frac{\partial}{\partial \alpha}}$ and $\alpha$ on $V_{\alpha,\alpha}(z)$ can be obtained by integrating the one-form $J(y)dy$ along particular contours.
...
\label{dpzz}
\end{align}
Having a consistent spectrum is not always necessary: in the application to Liouville theory (Section \ref{seclld}), we will use free bosonic correlation functions whose momentums $\alpha_i$ do not belong to the spectrum of any consistent theory, although they obey $\sum_i\alpha_i=Q$.
