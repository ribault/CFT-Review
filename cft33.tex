...
Using the functions $\Upsilon_b(x)$ and $\hat\Upsilon_b(x)$, we can write solutions $C$ and $\hat C$ of the shift equations \eqref{fcc} for three-point structure constants, that are respectively valid for $c\notin ]-\infty, 1]$ and $c\notin [25,\infty[$,
\begin{align}
...
\item are invariant under $b\rightarrow \frac{1}{b}$, as follows from the invariance of $\Upsilon_b(x)$,
...
\end{align}
\end{itemize}
However, the sets of poles and zeros of $C$ and $\hat C$ are very different from one another.
We will see how this affects the properties of correlation functions under analytic continuation of momentums or the central charge.


\subsection{Existence and uniqueness of Liouville theory}

We derived the three-point structure constants $C$ and $\hat C$ by solving the equation \eqref{fcc} and its dual equation under $b\rightarrow \frac{1}{b}$, which are consequences of crossing symmetry of four-point functions involving degenerate fields. 
...
It turns out that four-point functions based on $C$ or $\hat C$ are crossing symmetric, depending on the value of the central charge $c\in\mathbb{C}$:
\begin{align}
\renewcommand{\arraystretch}{1.3}
 \begin{tabular}{|l||c|c|}
  \hline
...
difference between $C$ and $\hat C$ from analytic properties of integrand of 4pt function...

Actually, Liouville theory with $c> 25$ is known to be consistent also on the torus \cite{hjs09}, although its partition function is infinite. 
