...
\subsubsection{Linear dilaton theory}

We first consider the diagonal, continuous spectrum
\begin{align}
 \boxed{S = \int_{\frac{Q}{2}+i{\mathbb{R}}} d\alpha \ \mathcal{V}_\alpha \otimes \bar{\mathcal{V}}_\alpha}\ .
\label{sz}
\end{align}
This spectrum is obtained from the Liouville spectrum \eqref{sad} by doubling each Virasoro representation $\mathcal{V}_\alpha\otimes \bar{\mathcal{V}}_\alpha$ (except for $\alpha=\frac{Q}{2}$), i.e. replacing it with a sum $\mathcal{V}_\alpha\otimes \bar{\mathcal{V}}_\alpha \oplus \mathcal{V}_{Q-\alpha}\otimes \bar{\mathcal{V}}_{Q-\alpha}$ of two $\hat{\mathfrak{u}}_1 \times \bar{\hat{\mathfrak{u}}}_1$ representations that are equivalent as Virasoro representations. 

The correlation functions are 
\begin{align}
 \left\langle \prod_{i=1}^N V_{\alpha_i}(z_i)\right\rangle = \delta\left({\textstyle \sum}_{i=1}^N\alpha_i-Q\right)\ \prod_{i<j} |z_i-z_j|^{-4\alpha_i\alpha_j} \ .
\label{dpzz}
\end{align}
...
\subsubsection{Compactified free bosons}

Let us look for free bosonic theories with discrete spectrums and trivial multiplicities, while insisting that the spectrum closes under fusion \eqref{vvp}, and allowing only bosonic fields.
In terms of the left and right momentums $\alpha$ and $\bar \alpha$, the condition that the conformal spin is integer reads 
\begin{align}
 (\alpha-\bar\alpha)(\alpha+\bar\alpha-Q)\in\mathbb{Z}\ . 
\end{align}
Moreover, by closure under fusion, the vector $\vec\alpha = (\alpha,\bar\alpha)$ must take values in a set that is closed under addition.

Let us first deal with the case $Q=0$. If two vectors $\vec\alpha_1$ and $\vec\alpha_2$ belong to the spectrum, then $\vec\alpha_1,\vec\alpha_2$ and $\vec\alpha_1+\vec\alpha_2$ must have integer spins.
...
This leaves us with a free continuous parameter if $N=1,2$. But any spectrum with $N=1$ generator can be extended by adding a second generator, so generic maximal spectrums have $N=2$ generators.

Let us focus on generic maximal spectrums that contain diagonal states. 
Let us parametrize a diagonal generator as  $\vec\alpha_1=(\frac{i}{2R},\frac{i}{2R})$. By integrality of the spins, the second generator $\vec\alpha_2$ must obey $\frac{i}{R}(\alpha_2-\bar\alpha_2)\in\mathbb{Z}$, and without loss of generality we assume $\alpha_2-\bar\alpha_2=iR$. This implies $\alpha_2+\bar\alpha_2 \in \frac{i}{R}\mathbb{Z}$, and we can always assume $\alpha_2+\bar\alpha_2=0$ by doing a redefinition $\vec\alpha_2\to \vec\alpha_2+n\vec\alpha_1$ with $n\in\mathbb{Z}$. To summarize, generic maximal spectrums are of the type 
\begin{align}
...
For $Q\neq 0$ it turns out that compactified free bosons still exist, and are actually the most general free bosonic theories with discrete spectrums and trivial multiplicities. 
The condition that $\vec\alpha$ and $2\vec\alpha$ have integer spins indeed implies 
\begin{align}
...
\begin{align}
 \left\langle \prod_{i=1}^N V_{n_i,w_i}(z_i) \right\rangle = \delta_{\sum_{i=1}^N n_i,0}\ \delta_{\sum_{i=1}^N w_i,0}\ \prod_{i<j} (z_i-z_j)^{-2\alpha_i\alpha_j}(\bar{z}_i-\bar{z}_j)^{-2\bar{\alpha}_i\bar{\alpha}_j}\ , 
\end{align}
where we defined
\begin{align}
\renewcommand{\arraystretch}{1.3}
...
Assuming the spectrum is still $S_R$ \eqref{sr}, the fields are bosonic (i.e. have integer conformal spins) provided 
\begin{align}
 iQR\in {\mathbb{Z}} \ .
\end{align}
...
Let us relax the assumption that $J(z)$ (and therefore $W_1^{(0)}(z)$) be locally holomorphic, and assume there is a function $\phi(z)$ such that 
\begin{align}
...
We write these solutions as $\psi(z) = \left[\begin{smallmatrix} z \\ 1 \end{smallmatrix}\right]^\dagger h \left[\begin{smallmatrix} z \\ 1 \end{smallmatrix}\right]$, where $h$ is a constant matrix such that $h=h^\dagger$.
Now how do we choose $h$? Global conformal symmetry \eqref{vtv} suggests that we should integrate over $h$, so that
\begin{align}
\left\langle\prod_{i=1}^N V_{\alpha_i}(z_i)\right\rangle\ \underset{\text{light\ asymptotic\ limit}}{\propto}\ \int_{H^+_3} dh\ \prod_{i=1}^N \left( \left[\begin{smallmatrix} z \\ 1 \end{smallmatrix}\right]^\dagger h \left[\begin{smallmatrix} z \\ 1 \end{smallmatrix}\right] \right)^{-2\eta_i}\ ,
\label{zih}
\end{align}
...
\begin{align}
 \boxed{ \mathcal{K}\left\langle \prod_{i=1}^N \Phi^{j_i}_{X_i,\bar{X}_i}(z_i)\right\rangle = \delta^{(2)}(Y_1) |Y_2|^2 |\Theta|^{b^2} \left\langle \prod_{i=1}^N V_{\alpha_i}(z_i)\prod_{j=1}^{N-2}V_{\langle 2,1\rangle}(y_j,\bar{y}_j)\right\rangle}\ .
\label{dyy}
\end{align}
...
\begin{align}
 \underset{k\to \infty}{\lim} \left\langle \prod_{i=1}^N \Phi^{j_i}_{x_i,\bar{x}_i}(z_i)\right\rangle \propto \int_{H_3^+} dh\ \prod_{i=1}^N \Phi^{j_i}_{x_i,\bar{x}_i}(h)\ , 
\end{align}
...
Notice that the large level limit of the $H_3^+$ $N$-point function is formally identical to the light asymptotic limit of the Liouville $N$-point function \eqref{zih}, whose interpretation is however quite different as it depends on positions $z$ instead of isospin variables $x,\bar{x}$.
