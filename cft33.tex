...
\begin{align}
 ...
 \label{eq:shiftd}
\end{align}
This is all that we will need to know about the degenerate OPE coefficients $C_\pm(P)$. An analogous equation can be derived for $\tilde{C}_\pm (P)$, using a  four-point function of the type  $
 \left\langle V_{\langle 1,2 \rangle} V_P V_{P} V_{\langle 1,2 \rangle} \right\rangle$ instead of $
 \left\langle V_{\langle 2,1 \rangle} V_P V_{P} V_{\langle 2,1 \rangle} \right\rangle$. That analogous equation is
 \begin{align}
...
Using a four-point function of the type  $
 ...
 \left\langle V_{\langle 2,1 \rangle} V_{P_1}V_{P_2}V_{P_3} \right\rangle$, we would obtain another equation for the shift $P_1\to P_1+\frac{1}{b}$. That other equation is obtained from the above equation by the substitutions $C_\pm \to \tilde{C}_\pm$ and $b\to \frac{1}{b}$.
 

\subsubsection{Normalization-invariant combinations} 
 
It may seem that we can eliminate the degenerate OPE coefficients $C_\pm(P)$ from our shift equation \eqref{eq:shift}, by using  eq. \eqref{eq:shiftd}. However, in order to do this we would need 
\begin{itemize}
 \item either to take the square-root of the latter equation, thereby introducing branch cuts whereas we want our correlation functions to be meromorphic,
 \item or to take the square of the former equation, thereby losing track of the sign of the three-point structure constant $C_{P_1,P_2,P_3}$. 
\end{itemize}
We will evade this dilemma by distinguishing quantities that are invariant under field renormalizations \eqref{vlv} and must be unambiguously determined, from quantities that are not invariant and can therefore be ambiguous and/or ill-behaved.   

see minimal lectures

introduce $N$ factors here, mention 4pt structure constant. Separate equations for $N$, $C'$. Separate solutions in next subsection. Invariance under renormalization.
