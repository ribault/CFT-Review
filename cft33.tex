...
Just like free field theories, conformal field theories are simpler and more tractable approximations of generic field theories, and can serve as starting points for perturbative computations in generic field theories. However, conformal field theories are more complicated than free field theories, and also more often of independent interest, for example when they describe critical points of condensed matter systems. Therefore, it is worthwhile to study conformal field theories in their own right, leaving aside the applications to non-conformal field theories.
...
Even later will come free theories, which do not play any fundamental role in the bootstrap approach.
...
Let us argue that this text fills an available ecological niche, by considering which neighbouring niches are already filled: 
\begin{itemize}
\item 
...
\end{itemize}
In contrast to most of these texts, we will treat both rational and non-rational theories.
...
A much abridged version is also available \cite{rib16}. With its two articles on \href{https://en.wikipedia.org/wiki/Two-dimensional_conformal_field_theory}{Two-dimensional conformal field theory} and \href{https://en.wikipedia.org/wiki/Liouville_field_theory}{Liouville theory}, Wikipedia provides an even briefer summary.
...
Differential operators such as $\frac{\partial}{\partial z}$, that act on functions on ${\mathbb{C}}^2$, will appear at intermediate steps of calculations, whereas correlation functions only involve functions that are well-defined on $\mathbb{C}$, for example $\sqrt{|z|}$ but not $\sqrt{z}$. 
...
\label{lvlv}
\end{align}
Since $R$ is irreducible, $R$ coincides with the subrepresentation $U(\mathfrak{V})|\Delta\rangle$ that is generated by $|\Delta\rangle$. Here we introduced the universal enveloping algebra $U(\mathfrak{V})$ of the Virasoro algebra, i.e. the associative algebra that is generated by the Virasoro generators $L_n$.
...
new parameter $\alpha$ called the \textbf{\boldmath Liouville momentum}\index{momentum!Liouville---}, such that 
\begin{align}
...
This conjugation rule allows us to deduce scalar products of descendent states, from the square norm of the primary state, which we normalize to $1$ and write  $\langle \Delta|\Delta\rangle =1$. 
For example,
\begin{align}
